...
System::System(python::object _pyobj) {
  ...
  // __pyx_obj_6mpi4py_3MPI_Comm * pyMPIComm = (__pyx_obj_6mpi4py_3MPI_Comm *)
  // pyobj;
...
void System::setSkin(real _skin) {
  ...
  // storage -> decompose();  // it's not nessesary because at the end of
  // cellAdjust()
...
void System::scaleVolume(real s, bool particleCoordinates) {
  // the size of the system should be modified first because of the cell size
  // recalculation
  // in xDecomposition
  bc->scaleVolume(s);
...
// Scale all coordinates of the system, anisotropic case (rectangular
// system!!!).
// Now the scale parameter is vector (Real3D). It is the case of the anisotropic
// extension of the
// system. Certainly it should be modified for triclinic box. Scale parameter s
// should be not
// Real3D but tensor.
