...
// gammaVariate(*boostRNG, gamma_distribution< real >(1, 1.0)), //TODO this line is nonsense:
// alpha=1 is trivial
{
}
...
long RNG::get_seed() { return seed_; }
...
real RNG::normal() { return normalVariate(); }
...
Real3D RNG::uniformOnSphere() { return uniformOnSphereVariate(); }

std::shared_ptr<RNGType> RNG::getBoostRNG() { return boostRNG; }
...
void RNG::loadState(const char* fname)
{
    ...
    {
        std::cerr << "# Warning: The number of processes does not match the number of saved random "
                     "number generators."
                  << std::endl
                  << "           The old states are not restored.\n";
        return;
    }
