...
RNG::RNG(long _seed)
    : boostRNG(make_shared<RNGType>(_seed + mpiWorld->rank())),
      ...
      seed_(_seed)
// gammaVariate(*boostRNG, gamma_distribution< real >(1, 1.0)), //TODO this line
// is nonsense: alpha=1 is trivial
{}
...
real RNG::gamma(unsigned int alpha) {
  ...
  variate_generator<RNGType&, gamma_distribution<real> > gamma_var(*boostRNG,
                                                                   gamma_dist);
  return gamma_var();
}
...
shared_ptr<RNGType> RNG::getBoostRNG() { return boostRNG; }
