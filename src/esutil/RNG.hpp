...
public:
    ...
    {
        return gamma(1);
    };  // this would look nicer with an optional argument, but it did not work (for boost)
...
private:
    ...
    typedef variate_generator<RNGType&, normal_distribution<real> > NormalVariate;

    //      typedef
    //      variate_generator< RNGType&, gamma_distribution< real > >
    //      GammaVariate;

    typedef variate_generator<RNGType&, uniform_on_sphere<real, Real3D> > UniformOnSphereVariate;
