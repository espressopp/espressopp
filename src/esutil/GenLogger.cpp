...
Logger& Logger::getInstance(string name) { return Logger::createInstance<GenLogger>(name); }
...
{
    LOG4ESPP_DEBUG(myLogger, "logger " << getFullName()
                                       << ", level = " << getLevelString(getEffectiveLevel())
                                       << ", set = " << setFlag);
...
static int read_config(char* fname)
{
    ...
    if (buf_len > 0) no_entries += eval_entry(buffer, buf_len, fname);
...
{
#ifdef HAVE_UNISTD_H
    fprintf(stderr, "%s:%d: %s in \"%s\", PID %d: %s\n", loc.filename, loc.line, level,
            loc.funcname, getpid(), msg.c_str());
#else
    fprintf(stderr, "%s:%d: %s in \"%s\": %s\n", loc.filename, loc.line, level, loc.funcname,
            msg.c_str());
#endif
    fflush(stderr);
}
...
void GenLogger::trace(Location loc, const string& msg) { log("TRACE", loc, msg); }

void GenLogger::debug(Location loc, const string& msg) { log("DEBUG", loc, msg); }

void GenLogger::info(Location loc, const string& msg) { log("INFO", loc, msg); }

void GenLogger::warn(Location loc, const string& msg) { log("WARN", loc, msg); }

void GenLogger::error(Location loc, const string& msg) { log("ERROR", loc, msg); }

void GenLogger::fatal(Location loc, const string& msg) { log("FATAL", loc, msg); }
