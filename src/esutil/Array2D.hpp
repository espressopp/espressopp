...
#ifndef _OLTL_OUTLIER_MODE
#define _OLTL_OUTLIER_MODE
  ...
  */
  ...
  class Array2D : private vector< T > {
    // {{{ 
  ...
  public:
    ...
    */
    ...
    reference at(size_type i, size_type j) 
      ...
      if (i >= n) throw out_of_range("Index i out of bounds.");
      if (j >= m) throw out_of_range("Index j out of bounds.");
      return operator() (i,j); 
    ...
    \exception out_of_range \p i is out of range of the Array2D */ 
    template< class Vector >
    reference at(const Vector& pos) throw (out_of_range) 
    ...
    bool operator==(const Array2D< T, exception > &a) {
      return static_cast< vector<T> >(*this) == static_cast< vector<T> >(a);
    }
