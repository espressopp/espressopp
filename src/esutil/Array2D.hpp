...
public:
    ...
    */
    ...
    reference operator()(size_type i, size_type j) { return Super::operator[](i + j * n); }
    ...
    reference at(size_type i, size_type j)
    {
        if (i >= size_n() || j >= size_m())
        {
            ...
            for (size_type k = 0; k < old.size_n(); k++)
                for (size_type l = 0; l < old.size_m(); l++) (*this)(k, l) = old(k, l);

            // init the new elements with the prototype
            for (size_type k = old.size_n(); k < size_n(); k++)
                for (size_type l = 0; l < size_m(); l++) (*this)(k, l) = prototype;

            for (size_type k = 0; k < old.size_n(); k++)
                for (size_type l = old.size_m(); l < size_m(); l++) (*this)(k, l) = prototype;
        }
        return Super::at(i, j);
    }
