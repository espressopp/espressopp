    ...
    : storage(_storage), globalLists()
{
    ...
    con1 = storage->beforeSendParticles.connect(std::bind(
        &FixedListComm::beforeSendParticles, this, std::placeholders::_1, std::placeholders::_2));
    con2 = storage->afterRecvParticles.connect(std::bind(
        &FixedListComm::afterRecvParticles, this, std::placeholders::_1, std::placeholders::_2));
    con3 = storage->onParticlesChanged.connect(std::bind(&FixedListComm::onParticlesChanged, this));
}
...
bool FixedListComm::add(pvec pids)
{
    ...
    // add(tmp);
    if (pids.size() == 2)
        this->PairList::add(tmp.at(1), tmp.at(0));
    else if (pids.size() == 3)
        this->TripleList::add(tmp.at(0), tmp.at(2), tmp.at(1));
    else if (pids.size() == 4)
        this->QuadrupleList::add(tmp.at(3), tmp.at(0), tmp.at(1), tmp.at(2));
    tmp.clear();
...
void FixedListComm::beforeSendParticles(ParticleList& pl, OutBuffer& buf)
{
...
void FixedListComm::afterRecvParticles(ParticleList& pl, InBuffer& buf)
{
...
void FixedListComm::onParticlesChanged()
{
    ...
    for (; it != globalLists.end(); ++it)
    {
        ...
        // add the particles
        if (size == 2)
            this->PairList::add(pK, tmp.at(0));
        else if (size == 3)
            this->TripleList::add(pK, tmp.at(0), tmp.at(1));
        else if (size == 4)
            this->QuadrupleList::add(pK, tmp.at(0), tmp.at(1), tmp.at(2));
        tmp.clear();
    }
