...
DomainDecompositionNonBlocking::DomainDecompositionNonBlocking(std::shared_ptr<System> _system,
    ...
    : DomainDecomposition(_system,
                          _nodeGrid,
                          _cellGrid,
                          1 /*in DD nonblocking we do not allow halfcell at the moment*/),
        ...
        for (int coord = 0; coord < 3; ++coord)
        {
            ...
            if (nodeGrid.getGridSize(coord) > 1)
            {
                ...
                if (nodeGrid.getNodePosition(coord) % 2 == 0)
                {
                    ...
                    reqs[1] = irecvParticles_initiate(inBufferR,
                                                      nodeGrid.getNodeNeighborIndex(2 * coord + 1));
                    reqs[2] = isendParticles(outBufferR, sendBufR,
                                             nodeGrid.getNodeNeighborIndex(2 * coord + 1));
                    reqs[3] = irecvParticles_initiate(inBufferL,
                                                      nodeGrid.getNodeNeighborIndex(2 * coord));
                }
                else
                {
                    reqs[0] = irecvParticles_initiate(inBufferR,
                                                      nodeGrid.getNodeNeighborIndex(2 * coord + 1));
                    reqs[1] = isendParticles(outBufferL, sendBufL,
                                             nodeGrid.getNodeNeighborIndex(2 * coord));
                    reqs[2] = irecvParticles_initiate(inBufferL,
                                                      nodeGrid.getNodeNeighborIndex(2 * coord));
                    reqs[3] = isendParticles(outBufferR, sendBufR,
                                             nodeGrid.getNodeNeighborIndex(2 * coord + 1));
                }
            ...
            else
            {
                ...
                if (sizesFirst)
                {
                    ...
                    if (nodeGrid.getNodePosition(coord) % 2 == 0)
                    {
                        ...
                        reqs[0] = getSystem()->comm->isend(nodeGrid.getNodeNeighborIndex(dir),
                    ...
                    else
                    {
                        ...
                        reqs[0] = getSystem()->comm->irecv(
                            nodeGrid.getNodeNeighborIndex(oppositeDir), DD_COMM_TAG,
                            &(recvSizes[0]), recvSizes.size());
                        reqs[1] = getSystem()->comm->isend(nodeGrid.getNodeNeighborIndex(dir),
                                                           DD_COMM_TAG, &(sendSizes[0]),
                                                           sendSizes.size());
                    }
                                                            ...
                                                            longint node)
{
    LOG4ESPP_DEBUG(logger,
                   "initiate non blocking isend " << list.size() << " particles to " << node);
    data.reset();
    int size = list.size();
    data.write(size);
