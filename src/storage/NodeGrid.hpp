...
public:
    ...
    Real3D getMyLeft() const { return Real3D(getMyLeft(0), getMyLeft(1), getMyLeft(2)); }

    /// calculate end of local box
    real getMyRight(int axis) const { return (nodePos[axis] + 1) * localBoxSize[axis]; }
    Real3D getMyRight() const { return Real3D(getMyRight(0), getMyRight(1), getMyRight(2)); }
    ...
    longint getNodeNeighborIndex(int dir) const { return nodeNeighbors[dir]; }
    longint getNodeNeighborIndex(Directions dir) const { return getNodeNeighborIndex(dir); }

    /// where to fold particles that leave the box in direction i (ToRight, 0, ToLeft)
    int getBoundary(int dir) const { return boundaries[dir]; }
    int getBoundary(FoldDirections dir) const { return getBoundary(dir); }
