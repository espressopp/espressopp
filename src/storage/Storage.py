...
class StorageLocal(object):
    ...
    def addParticles(self, particleList, *properties):
        if not (pmi._PMIComm and pmi._PMIComm.isActive()
            ...
            # we should check at the begin whether all the particles do not
            # exist.
            ...
            for particle in particleList:
                # verify that each particle has enough entries, avoids index
                # errors
                ...
                if index_adrAT >= 0:
                    ...
                    # print "%d:  addParticle %d, last_pos=pos %f, %f,
                    # %f"%(pmi._MPIcomm.rank,id,pos[0], pos[1], pos[2])
                    storedParticle = self.cxxclass.addParticle(
                        self, id, pos, True)

                if storedParticle is not None:
                    self.logger.debug(
                        "Processor %d stores particle id = %d" %
                        (pmi.rank, id))
                    self.logger.debug(
    ...
    def printRealParticles(self):
        if not (pmi._PMIComm and pmi._PMIComm.isActive()
            ...
            for pid in self.getRealParticleIDs():
                ...
                print(
                    "CPU %-3i ID %-5i TYPE %-3i POS(%8.3f, %8.3f, %8.3f)" %
