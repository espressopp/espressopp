...
Storage::Storage(shared_ptr<System> system)
    : SystemAccess(system), inBuffer(*system->comm), outBuffer(*system->comm) {
  ...
  if (!weak || localParticles.find(p->id()) == localParticles.end()) {
    ...
    if (oldp && fixedtupleList) {
        ...
        if (it != fixedtupleList->end()) {
            ...
            //std::cout << "updateInLocalParticles: Particle not found in
    tuples!\n";
        }
    }
    */
...
int Storage::removeParticle(longint id) {
  ...
  // TODO particle should be removed from different particle groups and lists
  // too
}
...
// Particle *Storage::appendUnindexedAdrParticle(ParticleListAdr &l, Particle
// &part)
...
void Storage::savePosition(size_t id) {
  if (getSystemRef().comm->size() == 1) {
    ...
    stringstream msg;
    msg << " At the moment it works only for one CPU. One can not store old "
           "positions"
           " for several CPUs";
