...
void Storage::packPositionsEtc_LEBC(
    OutBuffer &buf, Cell &_reals, int extradata, const Real3D &shift, real offset)
{
    ParticleList &reals = _reals.particles;
    real cpy_tmp;
...
void Storage::copyRealsToGhosts_LEBC(Cell &_reals,
                                     Cell &_ghosts,
                                     int extradata,
                                     const Real3D &shift)
{
         ...
         src != end; ++src, ++dst)
    {
        if (shift[2] != 0.0)
        {
            ...
            {
                while (src->position()[0] > xref_cell + Lx / 2.0)
                    src->position()[0] -= Lx;  // mcutoff
            }
            dst->copyAsGhost(*src, extradata, shift);
            src->position()[0] = cpy_tmp;
        }
