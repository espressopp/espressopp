...
namespace espresso {
  ...
  // we are not interested in the return value of addParticle, so we need
  // a wrapper class around
  class StorageWrap : public Storage {
  public:
    void addParticle(longint id, ConstReal3DRef pos) {
      Storage::addParticle(id, pos);
    }
  };
