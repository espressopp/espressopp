...
DomainDecompositionAdress::DomainDecompositionAdress(std::shared_ptr<System> _system,
        ...
        if (it != fixedtupleList->end())
        {
            ...
            for (std::vector<Particle *>::iterator itv = atList.begin(); itv != atList.end(); ++itv)
            {
                ...
                /*std::cout << getSystem()->comm->rank() << ":  ---> " << at.id() << "-" <<
                   at.ghost() << " (" << at.position() << ") type " << at.type() << " \n";*/
    ...
    for (ParticleList::iterator dst = ghosts.begin(), end = ghosts.end(); dst != end; ++dst)
    {
        ...
        if (it != fixedtupleList->end())
        {
            ...
            for (std::vector<Particle *>::iterator itv = atList.begin(); itv != atList.end(); ++itv)
            {
                Particle &atg = **itv;
                buf.read(atg,
                         extradata);  // we force extradata to 1 (although not necessary here...)

                /*std::cout << getSystem()->comm->rank() << ":  <1--- " << atg.id() << "-" <<
                   atg.ghost() << " (" << atg.position() << ") type " << atg.type() <<
                                " extradata: " << extradata << "\n";*/
            }
        }
        else
        {
            ...
            for (int i = 1; i <= numAT; ++i, ++itv2)
            {
                ...
                /*std::cout << getSystem()->comm->rank() << ":  <2--- " << atg.id() << "-" <<
                   atg.ghost() << " (" << atg.getPos() << ") type " << atg.getType() <<
                                    " extradata: " << extradata << "\n";*/
...
void DomainDecompositionAdress::copyRealsToGhosts(Cell &_reals,
                                                  Cell &_ghosts,
                                                  int extradata,
                                                  const Real3D &shift)
{
...
void DomainDecompositionAdress::unpackAndAddForces(Cell &_reals, InBuffer &buf)
{
    ...
    for (ParticleList::iterator dst = reals.begin(), end = reals.end(); dst != end; ++dst)
    {
        ...
        /*std::cout << getSystem()->comm->rank() << ": " << " for particle " << dst->id() << "
           unpacking force "
            << " and adding to " << dst->force() << "\n";*/
        ...
        */
        ...
        for (int lr = 0; lr < 2; ++lr)
        {
            ...
            if (nodeGrid.getGridSize(coord) == 1)
            {
                ...
                {
                    throw std::runtime_error(
                        "DomainDecompositionAdress::doGhostCommunication: send/recv cell structure "
