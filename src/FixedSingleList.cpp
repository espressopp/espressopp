    ...
    : storage(_storage), globalSingles()
{
    ...
    con1 = storage->beforeSendParticles.connect(std::bind(
        &FixedSingleList::beforeSendParticles, this, std::placeholders::_1, std::placeholders::_2));
    con2 = storage->afterRecvParticles.connect(std::bind(
        &FixedSingleList::afterRecvParticles, this, std::placeholders::_1, std::placeholders::_2));
    con3 =
        storage->onParticlesChanged.connect(std::bind(&FixedSingleList::onParticlesChanged, this));
}
...
bool FixedSingleList::add(longint pid1)
{
    bool returnVal = true;
...
void FixedSingleList::onParticlesChanged()
{
    ...
    for (GlobalSingles::const_iterator it = globalSingles.begin(); it != globalSingles.end(); ++it)
    {
        ...
        if (p1 == NULL)
        {
            ...
            msg << "onParticlesChanged error. Fixed Single List particle p1 " << *it
