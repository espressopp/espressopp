...
// ESPP_CLASS
#ifndef _VECTORIZATION_INTERACTION_LENNARDJONES_HPP
#define _VECTORIZATION_INTERACTION_LENNARDJONES_HPP
...
public:
    ...
    LennardJones() : epsilon(0.0), sigma(0.0)
    {
        setShift(0.0);
        setCutoff(infinity);
        preset();
    ...
    LennardJones(real _epsilon, real _sigma, real _cutoff) : epsilon(_epsilon), sigma(_sigma)
    {
        autoShift = false;
        setCutoff(_cutoff);
        preset();
...
{
    static boost::python::tuple getinitargs(LennardJones const& pot)
    {
        real eps;
        real sig;
        real rc;
