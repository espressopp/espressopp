...
VerletList::VerletList(std::shared_ptr<System> system,
    ...
    : SystemAccess(system), vec(vec)
{
    ...
    connectionResort =
        system->storage->onParticlesChanged.connect(std::bind(&VerletList::rebuild, this));
}
...
ParticleArray& VerletList::getParticleArray() { return vec->getParticleArray(); }
...
{
    {
        ...
        if (PACK_NEIGHBORS)
        {
            ...
            size_t vec_max_cell_size =
                ((max_cell_size + ESPP_VECTOR_WIDTH - 1) / ESPP_VECTOR_WIDTH) * ESPP_VECTOR_WIDTH;
...
#pragma ivdep
                ...
                // padding
                {
                    ...
                    int pad_end =
                        ((c_j_max + ESPP_VECTOR_WIDTH - 1) / ESPP_VECTOR_WIDTH) * ESPP_VECTOR_WIDTH;
...
#pragma ivdep
            ...
            if (c_j_max > int_c(c_j.size()))
                throw std::runtime_error("rebuild_p_nc_pack_stencil: Reserve size exceeded.");
...
#pragma ivdep
        ...
        for (size_t irow = 0; irow < numRealCells; irow++)
        {
            ...
            if (num_pairs + max_pairs_per_cell > (neighborList.nplist.size()))
            {
                ...
                LOG4ESPP_INFO(theLogger, "Resizing neighbor list. num_pairs="
                                             << num_pairs << " old_size=" << old_size
                                             << " new_size=" << new_size);
            }
            ///////////////////////////////////////////////////////////////////////////////////////////
...
#pragma ivdep
                        ...
                        if (num_pairs - prev_num_pairs_2) last_ncell = ncell_id;
        ...
        if (num_pairs > int_c(nplist_reserve))
        {
            LOG4ESPP_WARN(theLogger,
                          "Reserve size exceeded. "
                          "Expected "
                              << nplist_reserve << ". Got " << num_pairs);
        }
    }
...
int VerletList::localSize() const { return num_pairs; }
...
void VerletList::registerPython()
{
    ...
    class_<VerletList, std::shared_ptr<VerletList> >(
        "vectorization_VerletList",
        init<std::shared_ptr<System>, std::shared_ptr<Vectorization>, real, bool>())
