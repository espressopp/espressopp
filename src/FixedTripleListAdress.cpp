    ...
    : FixedTripleList(_storage), fixedtupleList(_fixedtupleList)
{
    ...
    sigBeforeSendAT = fixedtupleList->beforeSendATParticles.connect(
        std::bind(&FixedTripleListAdress::beforeSendATParticles, this, std::placeholders::_1,
                  std::placeholders::_2));

    sigAfterRecvAT = fixedtupleList->afterRecvATParticles.connect(
        std::bind(&FixedTripleListAdress::afterRecvParticles, this, std::placeholders::_1,
                  std::placeholders::_2));
...
bool FixedTripleListAdress::add(longint pid1, longint pid2, longint pid3)
{
    ...
    if (returnVal)
    {
        ...
        if (equalRange.first == globalTriples.end())
        {
            ...
            globalTriples.insert(std::make_pair(pid2, std::pair<longint, longint>(pid1, pid3)));
        }
        else
        {
...
void FixedTripleListAdress::beforeSendATParticles(std::vector<longint>& atpl, OutBuffer& buf)
{
...
void FixedTripleListAdress::beforeSendParticles(ParticleList& pl, OutBuffer& buf)
{
    // std::cout << storage->getSystem()->comm->rank() << ": beforeSendParticles() fixed pl (size "
    // << pl.size() << ")\n";
}
