...
{
    setDimension(_dim);
    for (int i = 0; i < _dim; i++) data[i] = v;
}

inline RealND::RealND(const RealND& v)
{
    setDimension(v.getDimension());
    for (int i = 0; i < v.getDimension(); i++) data[i] = v[i];
}
...
inline RealND::RealND(const int _dim, const std::vector<real>& v)
{
    ...
    setDimension(_dim);
    for (int i = 0; i < _dim; i++) data[i] = v[i];
}

inline RealND::RealND(const boost::python::list& l)
{
    setDimension(len(l));
    for (int i = 0; i < len(l); i++) data[i] = boost::python::extract<double>(l[i]);
}
...
inline real& RealND::operator[](int i) { return data[i]; }

inline const real& RealND::operator[](int i) const { return data[i]; }

inline real& RealND::at(int i)
{
    if (i < 0 || i > getDimension()) throw std::out_of_range("RealND::at");
    return (*this)[i];
}
...
inline void RealND::setItem(int i, real v) { this->at(i) = v; }

inline real RealND::getItem(int i) const { return this->at(i); }
...
inline RealND& RealND::operator+=(const RealND& v)
{
    ...
    for (int i = 0; i < dimension; i++) data[i] += v.data[i];
    return *this;
}
...
{
    for (int i = 0; i < dimension; i++) data[i] *= v;
    return *this;
}
...
{
    real v_1 = 1.0 / v;
    for (int i = 0; i < dimension; i++) data[i] *= v_1;
    return *this;
}
...
inline bool RealND::operator!=(const RealND& v) const { return !(*this == v); }

// elementwise binary operators
inline RealND RealND::operator+(const RealND& v) const { return RealND(*this) += v; }

inline RealND RealND::operator-(const RealND& v) const { return RealND(*this) -= v; }

inline RealND RealND::operator*(real v) const { return RealND(*this) *= v; }
...
inline real RealND::operator*(const RealND& v) const
{
    ...
    real res = 0.0;
    for (int i = 0; i < dimension; i++) res += data[i] * v.data[i];
...
{
    real res = 0.0;
    for (int i = 0; i < dimension; i++) res += data[i] * data[i];
    return res;
}

inline real RealND::abs() const { return sqrt(sqr()); }
...
//////////////////////////////////////////////////
// Global operators
inline RealND operator*(real s, const RealND& v) { return RealND(v) * s; }

inline std::ostream& operator<<(std::ostream& out, const RealND& v)
{
    for (int i = 0; i < v.getDimension(); i++) out << v[i] << ' ';
    return out;
}
...
inline RealND& RealNDs::operator[](int i) { return *data[i]; }

inline const RealND& RealNDs::operator[](int i) const { return *data[i]; }
...
inline void RealNDs::setItem(int i, const RealND& v) { this->at(i) = v; }

inline RealND RealNDs::getItem(int i) const { return this->at(i); }
...
inline std::ostream& operator<<(std::ostream& out, const RealNDs& v)
{
    for (int i = 0; i < v.getDimension(); i++)
    {
        for (int j = 0; j < v[i].getDimension(); ++j) out << v[i][j] << ' ';
        out << '\n';
    }
    return out;
}
