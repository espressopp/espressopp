...
def getSelfExclEnergyReactionField(
        system,
        exclusions,
        prefactor,
        epsilon1,
    ...
    for (pid1, pid2) in exclusions:
        if (pid1 in pidlist) and (pid2 in pidlist):
            ...
            if distSqr > rc2:
                print(
                    'Error, PBC problem in getSelfEnergyReactionField with particles ',
                    pid1,
                    pid2)
                quit()
            exclEnergy += part1.q * part2.q * \
                prefactor * -1.0 * (B1_half * distSqr + crf)
