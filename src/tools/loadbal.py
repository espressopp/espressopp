...
# This function sorts the nodeGrid, according to the index of the maximum
# size of the box (indMax) and its corresponding minimum (indMin)
...
# This function verifies if it is worthy to take care of special DD for
# inhomogeneous systems, otherwise the homogenous DD is triggered and
# returned as a xyz flags
...
# This function decomposes half of the box in one Dimension(1D) and it is
# assuming symmetry in the initial simulation box. Even with
# irregularities in the box dimension and cell-size relation it will find
# the half-Box-DD
...
def halfDecomp(
        ...
        idealGas,
        halfCellInt=1):
    # this value is only in case the Ideal Gas will in reality improve any
    # calculation or communication (i.e. Improve notoriously the sims
    # parallelization, which is not the case yet)
    pLoadIG = 1
    cellSizes = []
    usedCores = 0
    ...
    for i in range(halfCores1D):
        ...
        if idealGas:
            if i == 0:
                ...
                [cellSizes.append(halfCellInt * round((eh_size) / rc_skin / (halfCores1D - usedCores) - 0.5))
                 for i in range(usedCores, halfCores1D)]  # 2do: SuperCell stuff
                deltaCells = halfCellInt * round((eh_size) / rc_skin - 0.5) - halfCellInt * round(
                    (eh_size) / rc_skin / (halfCores1D - usedCores) - 0.5) * (halfCores1D - usedCores)  # 2do: SuperCell stuff
                # Both applies a benefit to the usedCores=1 in the sense that
                # will have one cell less loaded to CG-region. But also a
                # penalty to the usedCores=1 that will have to manage any cells
                # non distributed before
            ...
            if totNodesCG / 2. >= 2.:
                [cellSizes.append(halfCellInt *
                                  round((adrCenter1D -
                                         eh_size) /
                                        rc_skin /
                                        totNodesCG /
                                        2. -
                                        0.5)) for j in range(int(totNodesCG /
                                                                 2.))]  # 2do: SuperCell stuff
            else:
                # 2do: SuperCell stuff
                cellSizes.append(
                    halfCellInt *
            ...
            if totNodesEH / 2. >= 2.:
                [cellSizes.append(halfCellInt *
                                  round((eh_size) /
                                        ...
                                        (totNodesEH /
                                         2.) -
                                        0.5)) for i in range(int(totNodesEH /
                                                                 2.))]  # 2do: SuperCell stuff
            else:
                # 2do: SuperCell stuff
                cellSizes.append(
...
# This function decomposes the whole box in one Dimension(1D) and it is
# assuming symmetry in the initial simulation box. Even with
# irregularities in the box dimension and cell-size relation it will find
# the full-Box-DD
...
def fullDecomp(
    ...
    totCellsCG = cellsX - totCellsEH
    totNodesCG, totNodesEH = findNodesMS(
        fullCores1D, totCellsEH, totCellsCG, ratioMS, sizeX, eh_size, idealGas)
    for i in range(fullCores1D):
        ...
        if idealGas:
            if i == 0:
                ...
                cellSizes.append(
                    round(
                        (2. * eh_size) / rc_skin - 0.5) + pLoadIG)
                usedCores = 1     # For Ideal Gas purposes only 1 core covers the low-resolution region
            else:
                [cellSizes.append(round((2. * eh_size) / rc_skin / (fullCores1D - usedCores) - 0.5))
                 for i in range(usedCores, fullCores1D)]  # 2do: SuperCell stuff
                deltaCells = round((2. * eh_size) / rc_skin - 0.5) - round((2. * eh_size) / rc_skin / (
                    fullCores1D - usedCores) - 0.5) * (fullCores1D - usedCores)  # 2do: SuperCell stuff
                # Both applies a benefit to the usedCores=1 in the sense that
                # will have one cell less loaded to CG-region. But also a
                # penalty to the usedCores=1 that will have to manage any cells
                # non distributed before
            ...
            if totNodesEH >= 2.:
                [cellSizes.append(round((2. * eh_size) / rc_skin / (totNodesEH) - 0.5))
                 for i in range(int(totNodesEH))]  # 2do: SuperCell stuff
            else:
                # 2do: SuperCell stuff
                cellSizes.append(round((2. * eh_size) / rc_skin - 0.5))
            if totNodesCG >= 2.:
                [cellSizes.append(round((sizeX - 2. * eh_size) / rc_skin / totNodesCG - 0.5))
                 for j in range(int(totNodesCG))]  # 2do: SuperCell stuff
            else:
                # 2do: SuperCell stuff
                cellSizes.append(round((sizeX - 2. * eh_size) / rc_skin - 0.5))
            return cellSizes
    return cellSizes


# This function uses the previously decomposed half-box (from halfDecomp)
# and unfolds it to match the whole Neighbors List. If it does not match
# the whole neighbor, due to any asymmetry in the number of cores or the
# number of cells. It will be redistributed region by region by
# considering the whole simulation box size.
...
def addHsymmetry(
    ...
    if len(halfNeilListX) < node_gridX:
        if (node_gridX - len(halfNeilListX)
            ) == 1:  # Verifies if number of cores are even
            ...
            if sum(halfNeilListX) != cellsX:
                ...
                if any([v == 0 for v in halfNeilListX]
                       ):  # Recently added 138, 139 and 140
                    wholeNeilListX = reDistCells(
                        halfNeilListX,
                        cellsX,
                        eh_size,
            ...
            # I think, however, that it was just forgotten, so I put it here
            # like to all other calls to reDistCells
            wholeNeilListX = reDistCells(
                halfNeilListX,
                cellsX,
                eh_size,
        ...
        print(
            "HeSpaDDA message: The distributed cores are not matching the available ones",
            halfNeilListX)
...
# This function adapts the number of cells that go into each core into the
# data structure of left and right cell lists for example for 4 core and 8
# cells [3,4,5,8] to [0,3,4,5,8]
...
# This function distributes the cells into nodes as if they where
# homogeneous. It also applies to inhomogeneous system whenever there are
# less than 2 cores per direction: X, Y or Z.
...
def reDistCellsHom(node_gridX, sizeX, rc_skin, halfCellInt=1):
    ...
    if node_gridX % 2 == 0 and cellsX % 2 == 0:
        ...
        if int(cellsX - sum(wholeNeiListX)) != 0:
            ...
            print(
                "HeSpaDDA message: PASS appears...here, take a look at this value Px/Cx",
                round(
                    (cellsX) /
                    node_gridX -
                    0.5))
...
# This This function is matching proportion of cells to the cores on a
# dual resolution region basis
...
def reDistCells(
    ...
    print(
        "HeSpaDDA message: Cores in Both LR and HR, are:",
        totNodesCG,
        totNodesEH)
    if idealGas:          # This represents the Ideal Gas (IG)!!! (OJO)
        ...
        if totNodesEH % 2 == 0 and totCellsEH % 2 == 0:
            ...
            if int(totCellsEH - sum(wholeNeiListX_EH)) != 0:
                ...
                print(
                    "HeSpaDDA message IG: HR region: PASS appears...here, take a look at this value Px/Cx",
                    round(
                        totCellsEH /
                        totNodesEH -
                        0.5))
            ...
            if totNodesEH % 2 == 0 and totCellsEH % 2 != 0:
                ...
                # passing Delta cells to be redistributed semi-randomly (after
                # prioritizying the EH-region, additional cells should go to
                # the CG-region).
                wholeNeiListX_EH[0:totNodesEH] = redistDeltaRandomly(
                    wholeNeiListX_EH[0:totNodesEH], totCellsEH - sum(wholeNeiListX_EH[0:totNodesEH]), 0)
        ...
        if totNodesCG % 2 == 0 and totCellsCG % 2 == 0:
            ...
            if int(totCellsCG - sum(wholeNeiListX_CG)) != 0:
                ...
                print(
                    "HeSpaDDA message IG: LR region: PASS appears...here, take a look at this value Px/Cx",
                    round(
                        totCellsCG /
                        totNodesCG -
                        0.5))
            ...
            if totNodesCG % 2 == 0 and totCellsCG % 2 != 0:
                ...
                # passing Delta cells to be redistributed semi-randomly (after
                # prioritizying the EH-region, additional cells may come to the
                # CG-region).
                wholeNeiListX_CG[0:totNodesCG] = redistDeltaRandomly(
                    wholeNeiListX_CG[0:totNodesCG], totCellsCG - sum(wholeNeiListX_CG[0:totNodesCG]), 0)
    ...
    # @ not Ideal Gas
        ...
        if (totNodesCG + totNodesEH) > node_gridX:
            # minimum number of cores(nodes=cores) for the CG region version
            # 1.0
            if totNodesCG > 2:
            ...
            print(
                "HeSpaDDA message indexing: Nodes CG and Nodes EH are respectively,",
                totNodesCG,
                totNodesEH)
        if node_gridX % 2 == 0 and cellsX % 2 == 0:
            ...
            if totNodesCG % 2 == 0:
                ...
                # internal parameter which in case of dynamic LB, if more
                # weight of DD goes to the center by defaults (EH -region Flag
                # =1) and hence DD focus on any CG-region (default value
                # decomposes cells to CG).  preFactCen=ratioMS per default(OLD)
                centralFlagEH = 1
                ...
                if centralFlagEH > 0:  # H's WL as well    #NHEW
                    for i in range(int(ratioMS), int(cellsX + 1)):
                        ...
                        print(
                            "HeSpaDDA message indexing: the Ratio MS2Cellslot 'cells weight' in the CG region is:",
                            ratioMS2t)
                        ...
                        if totCellsEHtemp < totNodesEH and totCellsEHtemp > 0:
                            print(
                                "HeSpaDDA message indexing: Error with pass Factor MS-2-Cells, no worries HeSpaDDA will find another for you!",
                                totCellsEHtemp)
                        else:
                            ...
                            print(
                                "HeSpaDDA message indexing: The rescaling preFactor for the Cells distribution is...an optimized value, like this:",
                                preFactCen)
                            break
                            break  # the first value found for preFactCen takes you out of the for
                # Now redistributing the cells to nodes with a proper dimension
                # factor as a f(dimensions,ratioMS, coresEH, coresCG)
                for i in au1:
                    numRegBox = 3.  # 3. it is a region based parameter |CG|EH|CG| =3 fixed param, number of regions in the box
                    if cellsX > numRegBox * \
                            pow(ratioMS, 1. / 3.) and totNodesEH < cellsX - (pow(ratioMS, 1. / 3.) * totCellsCG):
                ...
                print(
                    "HeSpaDDA message LR: wholeNeiListX_CG is, ",
                    wholeNeiListX_CG)
                # Now the redist in the EH-region occurs | NHEW-> We still need
                # to check if the cells are enough for the EH cores
                ...
                if int(totCellsCG) % int(totNodesCG) == 0:  # NHEW
                    ...
                    # Punishing the last Node with an additional cell  NHEW
                    # (got rid of -1 in the index)
            ...
            if node_gridX % 2 == 0:
                ...
                wholeNeiListX = redistDeltaRandomly(
                    wholeNeiListX,
                    cellsX - sum(wholeNeiListX),
                    totNodesEH,
                    cellsX - sum(wholeNeiListX) - 1)
    # print "My Redist WholeNeiList is TODOs !:",wholeNeiListX
    return wholeNeiListX

# This function distributes the remaining DELTA cells into nodes as semi
# randomly. By default the biase applies to the CG-region and it assumes
# ther cannot be more than 3 extra cells to redistribute, because this is
# the total number of regions in the simulation box |CG|EH|CG| (by default
# the cg biased is left this could be updated in the dyn load balancing
# case!
...
# This function normalizes the number of cells to go to the EH and CG
# regions and find the ideal corresponding number of Nodes EH and CG
...
def findNodesMS(
    ...
    if idealGas:
        ...
        if node_gridX <= (totCellsEH + totCellsCG):
            ...
            print(
                "HeSpaDDA message: According to the theory of HV Guzman article P_{HR} is :",
                totNodesEH)
            totNodesCG = node_gridX - totNodesEH
            if (totNodesEH + totNodesCG) != node_gridX:
                # If there are more nodes than cells in EH=> redistribute nodes
                # to EH and CG
                ...
                # If there are more nodes than cells in LR=> redistribute nodes
                # to EH and CG
