...
#  
#  This file is part of espressopp++ -> Powered by HeSpaDDA algorithm developed by horacio.v.g@gmail.com
#  
#  espressopp++ is free software: you can redistribute it and/or modify
#  it under the terms of the GNU General Public License as published by
#  the Free Software Foundation, either version 3 of the License, or
#  (at your option) any later version.
#  
#  espressopp++ is distributed in the hope that it will be useful,
...
def halfDecomp(adrCenter1D,rc_skin,eh_size,halfCores1D,cellsX,ratioMS,sizeX,idealGas):
    ...
    for i in xrange(halfCores1D):
        ...
        if idealGas:
            if i==0:
                cellSizes.append(round((adrCenter1D-eh_size)/rc_skin-0.5)+pLoadIG)	#2do: SuperCell stuff
            ...
            if totNodesCG/2.>=2.:
                ...
                cellSizes.append(round((adrCenter1D-eh_size)/rc_skin-0.5))  #2do: SuperCell stuff
            if totNodesEH/2.>=2.:
                [cellSizes.append(round((eh_size)/rc_skin/(totNodesEH/2.)-0.5)) for i in xrange(int(totNodesEH/2.))]  #2do: SuperCell stuff
            else:
                cellSizes.append(round((eh_size)/rc_skin-0.5))  #2do: SuperCell stuff
            return cellSizes
    return cellSizes
...
def addHsymmetry(halfNeilListX,eh_size,rc_skin,node_gridX,cellsX,ratioMS,sizeX,idealGas):
    wholeNeilListX=[]
    aux=halfNeilListX[:]
    aux.reverse()	#unfolds halfDecomp and attempts to match the whole neighbor list.
    halfNeilListX.extend(aux)	#here half neighbor list X turns into whole neighbor list
    aux2=0
        ...
        if (node_gridX-len(halfNeilListX))==1:	# Verifies if number of cores are even
            ...
            if sum(halfNeilListX)!=cellsX:
                ...
                if any([ v == 0 for v in halfNeilListX]):	# Recently added 138, 139 and 140    
                    ...
                    print "HDD message: addHsymmetry all tests passed although not all cells were used"
                    wholeNeilListX=halfNeilListX[:]
            #wholeNeilListX=halfNeilListX[:]
        else:
            print "HDD message: The distributed cores are not matching the available ones (++ reDistCells())"
	    wholeNeilListX=reDistCells(halfNeilListX,cellsX,eh_size,rc_skin,node_gridX,ratioMS,sizeX,idealGas)	
	    # To be determined if additional reDitsCells should be called!	
    elif len(halfNeilListX)==node_gridX and aux2==0:
        if sum(halfNeilListX)!=cellsX:
            print "HDD message: The distributed cells are not matching the available ones"
            wholeNeilListX=reDistCells(halfNeilListX,cellsX,eh_size,rc_skin,node_gridX,ratioMS,sizeX,idealGas)
        else:
            if any([ v == 0 for v in halfNeilListX]):                    # Recently added 152, 153 and 154
                wholeNeilListX=reDistCells(halfNeilListX,cellsX,eh_size,rc_skin,node_gridX,ratioMS,sizeX,idealGas)
            else:    
                print "HDD message: addHsymmetry all tests passed although not all cells are used"
                wholeNeilListX=halfNeilListX[:]
    else:
	print "HDD message: The distributed cores are not matching the available ones",halfNeilListX
	...
	halfNeilListX.pop(len(halfNeilListX)-1)
        print "HDD message: During DD a core has been reduced"
	wholeNeilListX=reDistCells(halfNeilListX,cellsX,eh_size,rc_skin,node_gridX,ratioMS,sizeX,idealGas)
...
def adaptNeiList(neiListxin):
    ...
    print "HDD message: Your Cells Neighbor Lists is:",neiListx
    return neiListx
...
def reDistCellsHom(node_gridX,sizeX,rc_skin):
    ...
    if node_gridX%2==0 and cellsX%2==0:
        ...
        if int(cellsX-sum(wholeNeiListX))!=0:
            wholeNeiListX=redistDeltaRandomly(wholeNeiListX,cellsX-sum(wholeNeiListX),0) # passing Delta as cellsX-sum(wholeNeiListX)
        else:
	    print "HDD message: PASS appears...here, take a look at this value Px/Cx",round((cellsX)/node_gridX-0.5)
            pass
    else:
        if node_gridX%2==0 and cellsX%2!=0:
...
def reDistCells(halfNeilListX,cellsX,eh_size,rc_skin,node_gridX,ratioMS,sizeX,idealGas):
    #global preFactCen
    preFactCen=1.0		
    print "HDD message: Cells redistribution will improve whenever the Cells1D are at least twice as big as Nodes1D!"
    wholeNeiListX=[]
    totCellsEH=round(2.*eh_size/rc_skin-0.5)
    totCellsCG=cellsX-totCellsEH
    totNodesCG,totNodesEH=findNodesMS(node_gridX,totCellsEH,totCellsCG,ratioMS,sizeX,eh_size,idealGas)
    print "HDD message: Cores in Both LR and HR, are:", totNodesCG,totNodesEH
	...
	# High Resolution region
        if totNodesEH%2==0 and totCellsEH%2==0:
            ...
            print "HDD message IG: HR region: P and C are EVEN, given by:"
	    ...
	    [wholeNeiListX_EH.append(round(totCellsEH/totNodesEH-0.5)) for i in xrange(totNodesEH)]
	        ...
	        print "HDD message IG: HR region: PASS appears...here, take a look at this value Px/Cx",round(totCellsEH/totNodesEH-0.5)
                pass
        else:
            if totNodesEH%2==0 and totCellsEH%2!=0:
                ...
                print "HDD message IG: HR region: P and noC are EVEN"
            elif totCellsEH%2==0 and totNodesEH%2!=0:
                [wholeNeiListX_EH.append(round((totCellsEH)/totNodesEH-0.5)) for i in xrange(totNodesEH)]
                wholeNeiListX_EH[0:totNodesEH]=redistDeltaRandomly(wholeNeiListX_EH[0:totNodesEH],totCellsEH-sum(wholeNeiListX_EH[0:totNodesEH]),0) # passing Delta cells to be redistributed semi-randomly (after prioritizying the EH-region, additional cells should go to the CG-region).
                print "HDD message IG: HR region: noP and C are EVEN"
	#@@@ Low Resolution region
        if totNodesCG%2==0 and totCellsCG%2==0:
            ...
            print "HDD message IG: LR region: P and C are EVEN, given by:"
	    ...
	    [wholeNeiListX_CG.append(round(totCellsCG/totNodesCG-0.5)) for i in xrange(totNodesCG)]
	        ...
	        print "HDD message IG: LR region: PASS appears...here, take a look at this value Px/Cx",round(totCellsCG/totNodesCG-0.5)
                pass
        else:
            if totNodesCG%2==0 and totCellsCG%2!=0:
                ...
                print "HDD message IG: LR region: P and noC are EVEN"
                print wholeNeiListX_CG
            elif totCellsCG%2==0 and totNodesCG%2==0:
                [wholeNeiListX_CG.append(round((totCellsCG)/totNodesCG-0.5)) for i in xrange(totNodesCG)]
                wholeNeiListX_CG[0:totNodesCG]=redistDeltaRandomly(wholeNeiListX_CG[0:totNodesCG],totCellsCG-sum(wholeNeiListX_CG[0:totNodesCG]),0) # passing Delta cells to be redistributed semi-randomly (after prioritizying the EH-region, additional cells may come to the CG-region).
                print "HDD message IG: LR region: noP and C are EVEN"

        indCG1=int((len(wholeNeiListX_CG))/2) 	# Index of the middle LR region begin of HR
	print "HDD message indexing: The CG first subregion index is:",indCG1
        indEH1=indCG1+int(totNodesEH)		# Index of the start of the second LR region end of HR
	...
	if (totNodesCG+totNodesEH)>node_gridX:
	    if totNodesCG>2:  # minimum number of cores(nodes=cores) for the CG region version 1.0
		totNodesCG=totNodesCG+((totNodesCG+totNodesEH)-node_gridX) # Assign exceeding number of cores to LR
	        totNodesEH=node_gridX-totNodesCG
	    else:
		totNodesCG=2  ## At least use 2 core for the CG region
	    	totNodesEH=node_gridX-totNodesCG
	else:
            print "HDD message indexing: Nodes CG and Nodes EH are respectively,",totNodesCG,totNodesEH
        if node_gridX%2==0 and cellsX%2==0:
            ...
            if totNodesCG%2==0:
                ...
                au1.extend(range(indEH1,indEH1+indCG1)) 	# au1 contains the index of CG cells in the whole neighbor list
		centralFlagEH=1  # internal parameter which in case of dynamic LB, if more weight of DD goes to the center by defaults (EH -region Flag =1) and hence DD focus on any CG-region (default value decomposes cells to CG).	preFactCen=ratioMS per default(OLD)
		...
		if centralFlagEH>0: # H's WL as well    #NHEW
			for i in xrange(int(ratioMS),int(cellsX+1)):
				...
				print "HDD message indexing: the Ratio MS2Cellslot 'cells weight' in the CG region is:",ratioMS2t
				...
				if totCellsEHtemp<totNodesEH and totCellsEHtemp>0:
					print "HDD message indexing: Error with pass Factor MS-2-Cells, no worries HeSpaDDA will find another for you!",totCellsEHtemp
				else:
					preFactCen=pow(i,1./3.) #i was not the cubic root yet
					#if int(totCellsEHtemp)-1==int(totNodesEH):
					#	totCellsCG=totCellsCG+1											
					print "HDD message indexing: The rescaling preFactor for the Cells distribution is...an optimized value, like this:",preFactCen
					break
		...
		# Now redistributing the cells to nodes with a proper dimension factor as a f(dimensions,ratioMS, coresEH, coresCG)
			...
			print "HDD message LR: cells dist No IG if cells fit in 3 subregions..."
		    else:
			...
			print "HDD message LR: cells dist No IG if cells fit volRatio used..."
		totCellsEH=cellsX-sum(wholeNeiListX_CG)		
                print "HDD message LR: wholeNeiListX_CG is, ",wholeNeiListX_CG
                if totNodesEH%2==0 and totCellsEH>=totNodesEH: 	# Now the redist in the EH-region occurs | NHEW-> We still need to check if the cells are enough for the EH cores
                    ...
                    wholeNeiListX[indEH1-1]=wholeNeiListX_EH[indEH1-1]+1  # Punishing the last Node with an additional cell
		#print "Whole lists are as:",wholeNeiListX_EH,wholeNeiListX_CG
                ...
                au2.extend(range(indEH1,indEH1+indCG1+1))	#no assuming odd totNodesCG, before (indEH1,indEH1+indCG1))  
		print "HDD message: Cells CG wrong",totCellsCG	
		if int(totCellsCG)%int(totNodesCG)==0:	#NHEW
        	        ...
        	        wholeNeiListX_CG[indEH1+indCG1]=wholeNeiListX_CG[indEH1+indCG1]+1  # Punishing the last Node with an additional cell  NHEW (got rid of -1 in the index)
                if totNodesEH%2==0:
                    for i in xrange(indCG1,indEH1):
                        wholeNeiListX_EH[i]=round(1.0*totCellsEH/totNodesEH-0.5)
                else:
                    for i in xrange(indCG1,indEH1):
                        wholeNeiListX_EH[i]=round(1.0*(totCellsEH-1)/totNodesEH-0.5)
                    wholeNeiListX_EH[indEH1-1]=wholeNeiListX_EH[indEH1-1]+1  # Punishing the last Node with an additional cell
                for k in xrange(node_gridX):
            ...
            if node_gridX%2==0:
                ...
                wholeNeiListX[node_gridX-1]=wholeNeiListX[node_gridX-1]+1 	# Punishing the last Node with an additional cell
            elif cellsX%2==0:
                [wholeNeiListX.append(round((cellsX)/node_gridX-0.5)) for i in xrange(node_gridX)]
                wholeNeiListX=redistDeltaRandomly(wholeNeiListX,cellsX-sum(wholeNeiListX),totNodesEH,cellsX-sum(wholeNeiListX)-1)
    #print "My Redist WholeNeiList is TODO's !:",wholeNeiListX            
    return wholeNeiListX
...
def redistDeltaRandomly(wholeNeiListX,deltaCells,totNodesEH=0,biased=0):
    ...
    print "HDD message: This are the deltaCells",deltaCells
    if deltaCells>0.5:
    ...
    if totNodesEH==0:
	    ...
	    if biased>0 and biased<3: 	            # Left biased!  		
	        aux2=randint(0,index-totNodesEH-1)  # Left CG region | * |  |  |
	    	nIndMin=0
		...
		aux2=randint(totNodesEH+1,index)  # Right CG region |  |  | * |
...
def findNodesMS(node_gridX,totCellsEH,totCellsCG,ratioMS,sizeX,eh_size,idealGas,procsWEH=1.):
    ...
    if idealGas:
        if (node_gridX-2.)<=totCellsEH and node_gridX>totCellsEH:
            totNodesEH=round(totCellsEH/procsWEH-0.5)# 2do: Could be tuned for every case! SuperCell!
            ...
            if totNodesEH<1 and node_gridX>0:
                print "HDD message: You are using the minimum amount of cores!!!"
	    	totNodesEH=1
	    	totNodesCG=1
	    else:
                print "HDD message: Are you sure you need to use a Domain Decomposition? Verify that you are not trying to run this simulation on a single core"

    else:	# Applies to all other systems besides the Ideal Gas
        if node_gridX<=(totCellsEH+totCellsCG):
            totNodesEH=round(fRatioEH*node_gridX) 
	    print "HDD message: According to the theory of HV Guzman article P_{HR} is :",totNodesEH
            totNodesCG=node_gridX-totNodesEH
            if (totNodesEH+totNodesCG)!=node_gridX:
                if totNodesEH>(totCellsEH):  # If there are more nodes than cells in EH=> redistribute nodes to EH and CG
                    diffNodesCells=totNodesEH-totCellsEH
                    if diffNodesCells<=(totCellsCG-totNodesCG):	# Add some cores to the CG nodes
                        totNodesCG=totNodesCG+diffNodesCells # more weight in terms of cores in the LR region                    
                        totNodesEH=totNodesEH-diffNodesCells
                    else:
                        print "HDD message: You seem to have more Cores than Cells! Hint(H): reduce the Nr. of Cores or use cherrypickTotalProcs function!"
                elif totNodesCG>(totCellsCG):	# If there are more nodes than cells in LR=> redistribute nodes to EH and CG
                    diffNodesCells=totNodesCG-totCellsCG
                    if diffNodesCells<=(totCellsEH-totNodesEH):
                        totNodesCG=totNodesCG-diffNodesCells
                        totNodesEH=totNodesEH+diffNodesCells  # more weight in terms of cores in the HR region
			if totNodesEH>totCellsEH:
				print "HDD message: Reduce the number of Processors to be used or try with cherrypickTotalProcs function!"
                    else:
                        print "HDD message: You seem to have more Cores than Cells! Hint(H): reduce the Nr. of Cores"
