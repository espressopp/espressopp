...
class InteractionType:
    ...
    def createEspressoInteraction(self, system, fpl):
        print(("WARNING: could not set up interaction for",
               self.parameters, ": Espresso potential not implemented"))
        return None
...
class MorseBondedInteractionType(InteractionType):
    def createEspressoInteraction(self, system, fpl):
        ...
        pot = espressopp.interaction.Morse(
            self.parameters['D'], self.parameters['beta'], self.parameters['rmin'])
...
class HarmonicAngleInteractionType(InteractionType):
    def createEspressoInteraction(self, system, fpl):
        ...
        pot = espressopp.interaction.AngularHarmonic(
            self.parameters['k']/2.0, self.parameters['theta']*2*math.pi/360)
...
class HarmonicNCosDihedralInteractionType(InteractionType):
    def createEspressoInteraction(self, system, fpl):
        ...
        pot = espressopp.interaction.DihedralHarmonicNCos(
            self.parameters['K'], self.parameters['phi0']*2*math.pi/360, self.parameters['multiplicity'])
...
def ParseDihedralTypeParam(line):
    ...
    if type == 8:
        ...
        p = RyckaertBellemansDihedralInteractionType(
            {'K0': tmp[5], 'K1': tmp[6], 'K2': tmp[7],
                'K3': tmp[8], 'K4': tmp[9], 'K5': tmp[10]}
        )
...
def FindNNextNeighbours(startnode, numberNeighbours, neighbours, forbiddenNodes):
    ...
    for n in startnode.neighbours:
        if not n in forbiddenNodes:
            ...
            FindNNextNeighbours(n, numberNeighbours-1,
                                neighbours, forbiddenNodes)
