...
def FindType(proposedtype, typelist):
    list = [typeid for (typeid, atype) in typelist.items()
            if atype == proposedtype]
    if len(list) > 1:
...
class InteractionType:
    ...
    def createEspressoInteraction(self, system, fpl):
        print(
            "WARNING: could not set up interaction for",
            self.parameters,
            ": Espresso potential not implemented")
        return None

    def automaticExclusion(self):
        # overwrite in derrived class if the particular interaction is
...
class HarmonicBondedInteractionType(InteractionType):
    def createEspressoInteraction(self, system, fpl):
        ...
        pot = espressopp.interaction.Harmonic(
            self.parameters['kb'] / 2.0, self.parameters['b0'])
...
class MorseBondedInteractionType(InteractionType):
    def createEspressoInteraction(self, system, fpl):
        # interaction specific stuff here
        pot = espressopp.interaction.Morse(
            self.parameters['D'],
            self.parameters['beta'],
            self.parameters['rmin'])
...
class FENEBondedInteractionType(InteractionType):
    def createEspressoInteraction(self, system, fpl):
        # interaction specific stuff here
        # spring constant kb is half the gromacs spring constant
        pot = espressopp.interaction.Fene(
            self.parameters['kb'] / 2.0, self.parameters['b0'])
...
class HarmonicAngleInteractionType(InteractionType):
    def createEspressoInteraction(self, system, fpl):
        # interaction specific stuff here
        # spring constant kb is half the gromacs spring constant. Also convert
        # deg to rad
        pot = espressopp.interaction.AngularHarmonic(
            self.parameters['k'] / 2.0,
            self.parameters['theta'] * 2 * math.pi / 360)
        interb = espressopp.interaction.FixedTripleListAngularHarmonic(
            system, fpl, pot)
        return interb
...
class TabulatedBondInteractionType(InteractionType):
    def createEspressoInteraction(self, system, fpl):
        ...
        interb = espressopp.interaction.FixedPairListTabulated(
            system, fpl, potTab)
        return interb
...
class TabulatedAngleInteractionType(InteractionType):
    def createEspressoInteraction(self, system, fpl):
        ...
        fe = fg.split(".")[0] + ".tab"  # name of espressopp file
        gromacs.convertTable(fg, fe)
        potTab = espressopp.interaction.TabulatedAngular(
            itype=spline, filename=fe)
        interb = espressopp.interaction.FixedTripleListTabulatedAngular(
            system, fpl, potTab)
        return interb
...
class TabulatedDihedralInteractionType(InteractionType):
    def createEspressoInteraction(self, system, fpl):
        ...
        fe = fg.split(".")[0] + ".tab"  # name of espressopp file
        gromacs.convertTable(fg, fe)
        potTab = espressopp.interaction.TabulatedDihedral(
            itype=spline, filename=fe)
        interb = espressopp.interaction.FixedQuadrupleListTabulatedDihedral(
            system, fpl, potTab)
        return interb
...
class HarmonicNCosDihedralInteractionType(InteractionType):
    def createEspressoInteraction(self, system, fpl):
        # DihedralHarmonicNCos coded such that k = gromacs spring constant.
        # Convert degrees to rad
        pot = espressopp.interaction.DihedralHarmonicNCos(
            self.parameters['K'],
            self.parameters['phi0'] * 2 * math.pi / 360,
            self.parameters['multiplicity'])
        interb = espressopp.interaction.FixedQuadrupleListDihedralHarmonicNCos(
            system, fpl, pot)
        return interb
...
class HarmonicDihedralInteractionType(InteractionType):
    def createEspressoInteraction(self, system, fpl):
        #print('RyckaertBellemans: {}'.format(self.parameters))
        pot = espressopp.interaction.DihedralHarmonic(
            self.parameters['K'], self.parameters['phi0'] * 2 * math.pi / 360)
        return espressopp.interaction.FixedQuadrupleListDihedralHarmonic(
            system, fpl, pot)
...
def ParseBondTypeParam(line):
    ...
    if btype == "8":
        p = TabulatedBondInteractionType(
            {"tablenr": int(tmp[3]), "k": float(tmp[4]), 'excl': True})
    elif btype == "9":
        p = TabulatedBondInteractionType(
            {"tablenr": int(tmp[3]), "k": float(tmp[4]), 'excl': False})
    elif btype == "1":
        p = HarmonicBondedInteractionType(
            {"b0": float(tmp[3]), "kb": float(tmp[4])})
    elif btype == "3":
        p = MorseBondedInteractionType(
            {"b0": float(tmp[3]), "D": float(tmp[4]), "beta": float(tmp[5])})
    elif btype == "7":
        p = FENEBondedInteractionType(
            {"b0": float(tmp[3]), "kb": float(tmp[4])})
    elif btype == "9":
        p = TabulatedBondInteractionType(
            {"tablenr": int(tmp[3]), "k": float(tmp[4])})
    else:
...
def ParseAngleTypeParam(line):
    ...
    type = int(tmp[3])
    if type == 1:
        p = HarmonicAngleInteractionType(
            {"theta": float(tmp[4]), "k": float(tmp[5])})
    elif type == 8:
        p = TabulatedAngleInteractionType(
            {"tablenr": int(tmp[4]), "k": float(tmp[5])})
    else:
...
def ParseDihedralTypeParam(line):
    ...
    type = int(tmp[4])
    if type == 8:
        p = TabulatedDihedralInteractionType(
            {"tablenr": int(tmp[5]), "k": float(tmp[6])})
    elif type == 3:
        ...
        p = HarmonicNCosDihedralInteractionType(
            {"K": float(tmp[6]), "phi0": float(tmp[5]), "multiplicity": int(tmp[7])})
    else:
...
def ParseImproperTypeParam(line):
    ...
    if type == 4:
        ...
        p = HarmonicDihedralInteractionType(
            {"K": float(tmp[6]), "phi0": float(tmp[5])})
    else:
...
def GenerateRegularExclusions(bonds, nrexcl, exclusions):
    ...
    for b in bonds:
        bids = b[0:2]
        for i in bids:
            if FindNodeById(i, nodes) is None:
                n = Node(i)
                nodes.append(n)
    ...
    for n in nodes:
        ...
        for nb in neighbours:
            ...
            # TODO: find a clever algorithm which does avoid permuations from
            # the start
