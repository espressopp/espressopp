...
def createPathintegralSystem(allParticles,
    ...
    if not disableVVL:
        ...
        vvl = espressopp.VirtualVerletList(system, rcut, ftpl)
        # create a cell list which will store the virtual particles after
    ...
    for i in range(1, P):
        for p in allParticles:
            ...
            newparticle[propDict['type']
                        ] = newparticle[propDict['type']] + numtypes * i
            # set positions
            newpos = newparticle[propDict['pos']]
            newpos[0] = newpos[0] + polymerInitR * \
                math.cos(i * 2 * math.pi / P) - polymerInitR
            newpos[1] = newpos[1] + polymerInitR * \
                math.sin(i * 2 * math.pi / P)
    ...
    if not disableVVL:
        vParticles = []
        # this is the type assigned to virtual particles
        vptype = numtypes * (P + 1) + 1
    ...
    # now we analyze how many unique different masses are in the system as we
    # have to create an harmonic spring interaction for each of them
    unique_masses = []
    for p in allParticles:
        mass = p[propDict['mass']]
        if mass not in unique_masses:
    ...
    for m in unique_masses:
        ...
        k = m * P * P * langevin.temperature * \
            langevin.temperature / (hbar * hbar)
