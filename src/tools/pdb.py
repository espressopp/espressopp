...
def pdbwrite(filename, system, molsize=4, append=False, typenames=None):
    # typenames: a map of typeid to typename to be written, e.g. for water
    # typenames={0:'H', 1:'O'}
    if append:
    ...
    while pid <= maxParticleID:
        if system.storage.particleExists(pid):
            ...
            # the additional 'T' in the string is needed to be recognized as
            # string,%10000 to obey the fixed-width format
            st = "%-6s%5d %-4s%1s%3s %1s%4d%1s   %8.3f%8.3f%8.3f%6.2f%6.2f     T%04d%2s%2s\n" % (
                'ATOM  ', (pid + addToPid) % 100000, name, '', 'UNX', 'F', mol % 10000, '', xpos, ypos, zpos, 0, 0, mol % 10000, '', '')
            file.write(st)
            pid += 1
            molcnt += 1
            if molcnt == molsize:
...
def pqrwrite(filename, system, molsize=4, append=False):
    ...
    while pid <= maxParticleID:
        if system.storage.particleExists(pid):
            ...
            # ATOM      1  N   ALA     1      46.457  12.189  21.556  0.1414
            # 1.8240
