...
def read(gro_file, top_file="", doRegularExcl=True):
    ...
    # a dict: key atomtypeid , value : class storing actual parameters of each type e.g. c6, c12, etc..
    atomtypeparams = {}
    ...
    if top_file != "":
        ...
        bondtypes = {}
        # a dict: key atomindex(int), atomindex(int),atomindex(int) value: angletypeid(int)
        angletypes = {}
        # a dict: key atomtindex(int), atomindex(int), atomindex(int),atomindex(int) value: tuple of dihedraltypeid(int)
        dihedraltypes = {}
        # a dict: key atomtindex(int), atomindex(int), atomindex(int),atomindex(int) value: tuple of dihedraltypeid(int)
        impropertypes = {}
            ...
            if readdefaults:
                if line.strip() == "" or '[' in line:  # end of defaults section
                    ...
                    if len(fields) == 5:
                        ...
                        defaults = {
                            "nbtype": fields[0], "combinationrule": fields[1]}
        ...
        # Now we search for bonds, angles definitions and start from the beginning of the file buffer
        f.seek(0)

        for mol in molecules:  # this loop not modified for 1-4 pairs
            print(("Preparing %d %s molecules... " %
                   (mol['count'], mol['name'])))
            print("-----------------------------")
            ...
            types, masses, charges, num_atoms_molecule = \
                storeAtoms(f, types, atomtypes, atomtypeparams,
                           masses, charges, num_molecule_copies)
...
def storeBonds(f, types, bondtypes, bondtypeparams, bonds, num_atoms_molecule,
    ...
    while(len(line) > 1 and not '[' in line):
        if line[0] == ";":   # skip comment lines
            ...
            continue
        tmp = line.split()
        # if the bond has < 3 arguments, it is defined in the bondtypes section and we have to look it up
        lookup = (len(tmp) <= 3)
...
def storeDihedrals(f, types, dihedraltypes, dihedraltypeparams, dihedrals, num_atoms_molecule, num_molecule_copies, molstartindex, atomtype_wildcard):
    ...
    while(len(line) > 1 and not '[' in line):
        ...
        if lookup:
            ...
            t1, t2, t3, t4 = types[pid1-1], types[pid2 -
                                                  1], types[pid3-1], types[pid4-1]
            try:
    ...
    for i in range(num_molecule_copies):
        for j in range(dihedrals_per_mol):
            ...
            if dihtypeid in dihedrals:
                # what happens now that it's a tuple? this has only been briefly tested for the case of more than one molecule containing dihedrals
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            #                            print t1, t2, t3, t4, 'not yet found'
                            t1, t2, t3, t4 = types[pid4-1], types[pid2 -
                                                                  1], types[pid3-1], types[pid1-1]
                            try:
                                dihtypeid = impropertypes[t1][t2][t3][t4]
                            except KeyError:
                                #                                print t1, t2, t3, t4, 'not yet found'
                                t1, t2, t3, t4 = types[pid4-1], types[pid3 -
                                                                      1], types[pid2-1], types[pid1-1]
                                try:
                                    dihtypeid = impropertypes[t1][t2][t3][t4]
                                except KeyError:
                                    print(
        ...
        for j in list(atomtypeparams.keys()):
            ...
            if (sig > 0 and eps > 0):
                ljpot = espressopp.interaction.LennardJonesSoftcoreTI(
                    epsilonA=eps, sigmaA=sig, epsilonB=epsilonB, sigmaB=sigmaSC, alpha=alphaSC, power=powerSC, cutoff=cutoff, lambdaTI=lambdaTI, annihilate=annihilate)
...
def setCoulombInteractions(system, verletlist, rc, types, epsilon1, epsilon2, kappa, hadress=False, adress=False, ftpl=None):
    ...
    # we want gromacs units, so this is 1/(4 pi eps_0) ins units of kJ mol^-1 e^-2
    pref = 138.935485
...
def setCoulombInteractionsTI(system, verletlist, rc, types, epsilon1, epsilon2, kappa, lambdaTI, pidlist, annihilate=True, hadress=False, adress=False, ftpl=None):
    ...
    pot = espressopp.interaction.ReactionFieldGeneralizedTI(
        prefactor=pref, kappa=kappa, epsilon1=epsilon1, epsilon2=epsilon2, cutoff=rc, lambdaTI=lambdaTI, annihilate=annihilate)
...
def setCoulombInteractionsProtein(system, verletlist, rc, types, epsilon1, epsilonprot, epsilonwat, kappa, otype, htype, hadress=False, adress=False, ftpl=None):
    ...
    print("# Setting up Coulomb reaction field interactions")
    print(("# Using ", epsilonwat, " for water and wat-prot and ",
           epsilonprot, " for protein"))
...
def setCoulomb14Interactions(system, defaults, onefourlist, rc, types):
    ...
    # we want gromacs units, so this is 1/(4 pi eps_0) ins units of kJ mol^-1 e^-2, scaled by fudge factor
...
def setTabulatedInteractions(potentials, particleTypes, system, interaction):
    ...
    """
    allparticles = []
    for k, v in list(particleTypes.items()):
        for i in v:
