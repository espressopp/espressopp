...
def nodeGrid(
    ...
    if isinstance(n, numbers.Number) and box_size is None:
        ...
        if eh_size != 0:
            ...
            ratioEH2CG = [(2. * eh_size) / (abs(box_size[0] - (2. * eh_size))),
                          (2. * eh_size) / (abs(box_size[1] - (2. * eh_size))),
                          (2. * eh_size) / (abs(box_size[2] - (2. * eh_size)))]
            ratioEHCG = min(ratioEH2CG)
            if not idealGas:  # Non ideal Gas Simulation
                # This condition checks if the sys is AdResS, if afirmative it
                # resizes box accordingly
                if ratioMS > 1:  # Spatially heterogeneous simulation
                    # print ratioEHCG
                    # This condition checks if the sys is slab based or cubic
                    # and resizes the box accordingly
                    if sum(slabMSDims) > 0.1:
                        boxList = [slabMSDims[0] *
                                   (box_size[0] +
                                    (2. *
                                     eh_size) *
                                    (pow(ratioMS, 0.3333) -
            ...
            # Ideal Gas HeSpaDDA (idealGas==1)
                ...
                if ratioMS > 1:
                    ...
                    # and resizes the box accordingly
                    if sum(slabMSDims) > 0:
                        if n <= round((2. * eh_size) / (rc + skin) - 0.5 + \
                                      2.) and min(boxList) * ratioEHCG / (rc + skin) >= 1.:
                            boxList = [slabMSDims[0] * ((2. * eh_size) + 2. * (rc + skin)), slabMSDims[1] * (
                                (2. * eh_size) + 2. * (rc + skin)), slabMSDims[2] * ((2. * eh_size) + 2. * (rc + skin))]
                            boxList = [
                                (1 - slabMSDims[0]) * box_size[0] + boxList[0],
                                (1 - slabMSDims[1]) * box_size[1] + boxList[1],
                                (1 - slabMSDims[2]) * box_size[2] + boxList[2]]
                            print(
                            ...
                            print(
                                "HeSpaDDA message: No more space for distributing Cores...look if you could use some HalfCells, or reduce the nr. of processors used and try again")
                    else:
                        boxList = [(2. *
                                    eh_size) +
                                   2. *
                                   ...
                                   2. *
                                   (rc +
                                    skin)]  # IDEA: multiply here by the cells refina
                else:
                    print("HeSpaDDA message: Non AdResS DD!")

            # Here starts the new Dimensional aware HDD algorithm which.
            # Dependencies: loadbal
            ...
            if ((abs(box_size[1] -
                     box_size[2]) < (1.0 *
                                     (rc +
                                      skin))) or (abs(box_size[0] -
                                                      box_size[1]) < (1.0 *
                                                                      (rc +
                                                                       skin))) or (abs(box_size[0] -
                                                                                       box_size[2]) < (1 *
                                                                                                       (rc +
                                                                                                        skin)))) and not ((abs(box_size[1] -
                                                                                                                               box_size[2]) < (1.0 *
                                                                                                                                               (rc +
                                                                                                                                                skin))) and (abs(box_size[0] -
                                                                                                                                                                 box_size[1]) < (1.0 *
                                                                                                                                                                                 (rc +
                                                                                                                                                                                  skin)))):
                ...
                for i in range(1, n + 1):
                    for j in range(i, n + 1):
                        if (i * i * j == n) and (i *
                            ...
                            # NOTE: It could be further optimized on a system
                            # by system basis as f(deltaCellSize beteween
                            # cores) this corresponds to the general version.
                            if qbicity(box_size, rc, skin) == False:
                                ...
                                redDi = [
                                    ...
                                    sum(boxList)]
                                LoN_norm = [
                                    boxList[0] * redDi[0] / ndN[0],
                                    boxList[1] * redDi[1] / ndN[1],
                                    boxList[2] * redDi[2] / ndN[2]]
                                ...
                                if LoN_Avg <= LoN_Avgmin:
                                    ...
                                    ijkmax = fdN[0] * fdN[0] + \
                                        fdN[1] * fdN[1] + fdN[2] * fdN[2]
                                    print(
                                        'HeSpaDDA message: No update of dN req ...')
                            else:
                                print(
                    ...
                    for j in range(i, n + 1):
                        for k in range(j, n + 1):
                            if (i * j * k == n) and (i * \
                                ...
                                ijkmax = i * i + j * j + k * k
                                # This checks if the system's box is cubic if not add weighted averages
                                # NOTE: It could be further optimized on a
                                # system by system basis as f(deltaCellSize
                                # beteween cores) this corresponds to the
                                # general version.
                                if qbicity(box_size, rc, skin) == False:
                                    ndN = changeIndex(dN, ima, imi)[:]
                                    redDi = [
    ...
    if ix < 1:
        raise Error(
            "local box size in direction 0 (=%6f) is smaller than interaction range (cutoff + skin = %6f).\n \
                 hint: number of CPUs maybe too high or is prime, perhaps you could also try with Halfcells." %
            (ix, rc_skin))
    iy = (box_size[1] * halfCellInt) / (rc_skin * node_grid[1])
    if iy < 1:
        raise Error(
            "local box size in direction 1 (=%6f) is smaller than interaction range (cutoff + skin = %6f).\n \
                 hint: number of CPUs maybe too high or is prime, perhaps you could also try with Halfcells." %
            (iy, rc_skin))
    iz = (box_size[2] * halfCellInt) / (rc_skin * node_grid[2])
    if iz < 1:
        raise Error(
            "local box size in direction 2 (=%6f) is smaller than interaction range (cutoff + skin = %6f).\n \
                 hint: number of CPUs maybe too high or is prime, perhaps you could also try with Halfcells." %
            (iz, rc_skin))
        ...
        for j in range(i, n + 1):
            for k in range(j, n + 1):
                if (i * j * k == n) and (i * i + j * j + k * k < ijkmax):
                    ...
                    d3 = i
                    ijkmax = i * i + j * j + k * k
    print("nodeGrid reads as follows: ", d1, d2, d3,
          ".... BTW, You are not using HeSpaDDA")
    return Int3D(d1, d2, d3)


def cherrypickTotalProcs(
    box_size,
    rc,
    skin,
    MnN,
    ...
    print(
        "HeSpaDDA message: Your search array in terms of total number of processors is:",
        procArray)
    for i in range(1, MnN + 1):
        nX[i -
           1], nY[i -
                  1], nZ[i -
                         1] = nodeGrid(procArray[i -
                                                 1], box_size, rc, skin, eh_size, ratioMS, idealGas, slabMSDims)
    print("HeSpaDDA message: For your Information, we are tackling the following number of processors")
    print(
        "HeSpaDDA message: Processors in the x-axis are:",
        nX,
        "\nProcessors in the y-axis are:",
        nY,
        "\nProcessors in the z-axis are:",
        nZ)
    pickedP = []
    for i in range(0, MnN):
        if (indMax == 0 and indMin == 1) or (indMax == 0 and indMin == 2):
            if nX[i] > (SymFactor - percTol * SymFactor) * \
                    nY[i] and nX[i] < (SymFactor + percTol * SymFactor) * nY[i]:
                pickedP.append(i)
        elif (indMax == 1 and indMin == 0) or (indMax == 1 and indMin == 2):
            if nY[i] > (SymFactor - percTol * SymFactor) * \
                    nX[i] and nY[i] < (SymFactor + percTol * SymFactor) * nX[i]:
                pickedP.append(i)
        elif (indMax == 2 and indMin == 0) or (indMax == 2 and indMin == 1):
            if nZ[i] > (SymFactor - percTol * SymFactor) * \
                    nX[i] and nZ[i] < (SymFactor + percTol * SymFactor) * nX[i]:
                pickedP.append(i)
    print(
        "HeSpaDDA message: There are a couple of total number of processors that satisfy your requirements: ", [
            (v + 1) * CpN for v in pickedP])
    return [(v + 1) * CpN for v in pickedP]

# WARNING! This is a new function to find values for the new neighborList
# DataStruct for Hom-Sys
...
def neiListHom(node_grid, box, rc, skin, halfCellInt=1):
    ...
    # It makes use of reDistCellsHom and adaptNeiList form loadbal to find
    # homogenous DD
    ...
    return list(map(int, neiListx)), list(
        map(int, neiListy)), list(map(int, neiListz))

# WARNING! This is a new function to find values for the new neighborList
# DataStruct for inHom-Sys
...
def neiListAdress(
    ...
    # round(cg_sizeL/rc_skin-0.5)+round((cursor[0]-cg_sizeR)/
    # rc_skin-0.5)+round((cg_sizeR-cg_sizeL)/rc_skin-0.5) # old implementation
    # rounded # Cells too much!
    ...
    print(
        "HeSpaDDA message: Current heterogeneous CellGrid (X,Y,Z)",
        cellsX,
        cellsY,
        cellsZ)
    # This condition checks if the Sys is a Slab or a Sphere. It should work
    # for any middle based Sys
    if not sphereAdr:
        if slabMSDims[0] == 1:
            if ((cursor[0] / 2.0) - 5.0) < adrCenter[0]:
                ...
                print(
                    "HeSpaDDA message: My halfneilListx called a half decomposition of the first half of the sim box as (algorithm S.1)...,",
                    halfneilListx)
                # Next instruction is doubling(by unfolding halfDecomp) the
                # halfspace based DD
                neiListxin = addHsymmetry(
                    ...
                    idealGasFlag)
                print(
                    "HeSpaDDA message: My neiListxin if it make sense your heterogenous system can be splitted in 2 equal parts (algorithm S.2)...,",
                    neiListxin)
            else:
                neiListxin = fullDecomp(
                    adrCenter[0],
                    rc_skin,
                    eh_size,
                    node_grid[0],
            ...
            neiListxin = reDistCellsHom(
                node_grid[0], cursor[0], rc_skin, halfCellInt)
        ...
        if slabMSDims[1] == 1:
            ...
            neiListyin = reDistCellsHom(
                node_grid[1], cursor[1], rc_skin, halfCellInt)
        # Contains the homogeneously decomp cores Neighbor List in full format
        # for Y
        neiListy = adaptNeiList(neiListyin)
        if slabMSDims[2] == 1:
            ...
            neiListzin = reDistCellsHom(
                node_grid[2], cursor[2], rc_skin, halfCellInt)
        # Contains the homogeneously decomp cores Neighbor List in full format
        # for Z
        neiListz = adaptNeiList(neiListzin)
        ...
        print(
            "HeSpaDDA message: Is it worthy to use an advanced DD? Flags for it in each dir X,Y,Z (0 means OK for heterogenoeus DD)...,",
            flx,
            fly,
            flz)
        # on X-axis
    ...
    return Int3D(cellsX, cellsY, cellsZ), list(map(int, neiListx)), list(
        map(int, neiListy)), list(map(int, neiListz))
...
def tuneSkin(
    ...
    # this is an empirical formula in order to get the appropriate number of
    # steps
...
def printTimeVsSkin(
    ...
    print(
        '      Calculations is started. It will print out the dependece of time of \n\
      ...
      time depending on your system.' %
        (nsteps, minSkin, maxSkin, skinStep))
