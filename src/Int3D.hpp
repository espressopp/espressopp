...
inline Int3D::Int3D(int v) { data[0] = data[1] = data[2] = v; }
...
{
    for (int i = 0; i < 3; i++) data[i] = (int)v[i];
}
...
inline int& Int3D::operator[](int i) { return data[i]; }

inline const int& Int3D::operator[](int i) const { return data[i]; }

inline int& Int3D::at(int i)
{
    if (i < 0 || i > 2) throw std::out_of_range("Int3D::at");
    return (*this)[i];
}
...
inline void Int3D::setItem(int i, int v) { this->at(i) = v; }

inline int Int3D::getItem(int i) const { return this->at(i); }
...
inline bool Int3D::operator!=(const Int3D& v) const { return !(*this == v); }

// elementwise binary operators
inline Int3D Int3D::operator+(const Int3D& v) const { return Int3D(*this) += v; }
