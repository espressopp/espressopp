...
Extension::Extension(shared_ptr<SystemHPX> system, shared_ptr<StorageHPX> storage)
    : SystemAccess(system), systemHPX(system), storageHPX(storage)
{
    if (!system->storage)
    {
    ...
    if (!storage)
    {
        throw std::runtime_error("invalid StorageHPX");
    }
...
void Extension::setIntegrator(shared_ptr<MDIntegratorHPX> _integrator) { integrator = _integrator; }
...
void Extension::registerPython()
{
    ...
    class_<Extension, boost::noncopyable>("hpx4espp_integrator_Extension", no_init)
