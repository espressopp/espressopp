...
{
    static python::tuple getinitargs(const Quaternion v)
    {
        return python::make_tuple(v.getReal(), v.getImag());
    }
};

void Quaternion::registerPython()
{
    ...
    class_<Quaternion>("Quaternion", init<>())
        ...
        .def(init<double>())
        .def(init<Real3D>())
        .add_property("real_part", &Quaternion::getReal, &Quaternion::setReal)
        .add_property("unreal_part", &Quaternion::getImag, &Quaternion::setImag)
