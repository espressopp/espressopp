...
namespace espressopp { namespace vec {
  namespace interaction {
    ...
    class VerletListLennardJonesCapped
      : public VerletListInteractionTemplate<LennardJonesCapped>
    {
    protected:
      typedef LennardJonesCapped _Potential;
      typedef _Potential Potential;
      typedef VerletListInteractionTemplate<LennardJonesCapped> base;

      struct LJCoefficients
      {
        LJCoefficients(Potential const& p)
          : ff1(p.getff1()), ff2(p.getff2()),
            caprad(p.getCaprad()), capradSqr(p.getCaprad()*p.getCaprad()),
            sigma(p.getSigma()), epsilon(p.getEpsilon()),
    ...
    public:
      VerletListLennardJonesCapped(shared_ptr<VerletList> _verletList)
        : base(_verletList)
        , np_types(0)
        , p_types(0)
      {
            ...
            auto np_ii = nplist[in];
            {
              ...
              #endif
              {
                ...
                if(NO_CAP) {
                  ...
                  if(ONETYPE)
                    ffactor = 48.0 * epsilon_ * cfrac6_ * (cfrac6_-0.5) / (caprad_ * sqrt(distSqr));
                  else
                    ffactor = 48.0 * ffs[np_lookup].epsilon * ffs[np_lookup].cfrac6
                      * (ffs[np_lookup].cfrac6-0.5) / (ffs[np_lookup].caprad * sqrt(distSqr));
                }
