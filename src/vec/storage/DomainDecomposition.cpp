  ...
  namespace storage {
    ...
    void DomainDecomposition::resetVirtualStorage()
    {
      ...
      for(size_t i=0; i<3; i++) {
        VEC_ASSERT_EQUAL((size_t(mainCellGrid[i]) % subCellGrid[i]), 0);
      }
      ...
      if(!rank)
      {
        ...
        std::cout << rank << ": subCellGrid: " << subCellGrid[0] << ", " << subCellGrid[1] << ", " << subCellGrid[2] << std::endl;
        std::cout << rank << ": subNodeGrid: " << subNodeGrid[0] << ", " << subNodeGrid[1] << ", " << subNodeGrid[2] << std::endl;
      ...
      for(size_t i2=0; i2<subNodeGrid[2]; i2++){
        for(size_t i1=0; i1<subNodeGrid[1]; i1++){
          for(size_t i0=0; i0<subNodeGrid[0]; i0++){
            ...
            vs.particles.markRealCells(realCellsIdx);

            auto& vc = vs.localCells;
            vc.reserve(numSubLocalCells);
            VirtualStorage::MapType cellMap;
            for(size_t j2start=(i2*subCellGrid[2]), j2=j2start, j2end=(i2+1)*subCellGrid[2]+2*halfCellInt; j2<j2end; j2++){
              for(size_t j1start=(i1*subCellGrid[1]), j1=j1start, j1end=(i1+1)*subCellGrid[1]+2*halfCellInt; j1<j1end; j1++){
                for(size_t j0start=(i0*subCellGrid[0]), j0=j0start, j0end=(i0+1)*subCellGrid[0]+2*halfCellInt; j0<j0end; j0++){
                  const int gidx = j0 + cellGrid.getFrameGridSize(0) * (j1 + cellGrid.getFrameGridSize(1) * j2);
                  const int lidx = vc.size();
                  cellMap[gidx] = lidx;
            ...
            vs.cellNeighborList = CellNeighborList(localCells[0], vc, vs.particles.realCells(), vs.getCellMap());

            auto& vr = vs.realCellsIdx;
            vr.reserve(numSubCells);
            for(size_t j2start=(i2*subCellGrid[2]), j2=j2start, j2end=(i2+1)*subCellGrid[2]; j2<j2end; j2++){
              for(size_t j1start=(i1*subCellGrid[1]), j1=j1start, j1end=(i1+1)*subCellGrid[1]; j1<j1end; j1++){
                for(size_t j0start=(i0*subCellGrid[0]), j0=j0start, j0end=(i0+1)*subCellGrid[0]; j0<j0end; j0++){
                  const int gidx = j0 + cellGrid.getGridSize(0) * (j1 + cellGrid.getGridSize(1) * j2);
                  vr.push_back(gidx);
                }
              }
            }
            ...
            #if 0
            {
              // std::int64_t jst = (i0*subCellGrid[0]) + cellGrid.getFrameGridSize(0) * ((i1*subCellGrid[1]) + cellGrid.getFrameGridSize(1) * (i2*subCellGrid[2]));
              std::cout << "    >>> Block: (" << i0 << "," << i1 << "," << i2 << ") size: " << vcells.size() << " elements: ";
              for(auto & v: vcells)
              {
                std::cout << " " << std::int64_t(v-localCells[0]);
              }
              std::cout << std::endl;
            }
            #endif
      ...
      /// Fill in subnode parameters
      {
        ...
        std::vector<int> myVranks;
        {
          ...
          VEC_ASSERT_EQUAL(myVranks.size(), numSubNodes);
        }
          ...
          if(rank==irank)
          {
            ...
            for(size_t inode=0; inode<vdd.size(); inode++) {
              ...
              std::cout << rank << ": "
                ...
                << "\n    "
                << "vd.cellGrid: " << f_grid_info(vd.cellGrid)
                << std::endl;
            }
          }
      ...
      {
        auto vFillCells = [](std::vector<size_t>& cv, const int leftBoundary[3], const int rightBoundary[3], CellGrid const& cg)
        {
          longint total = 1;
          for (int i = 0; i < 3; ++i){
        ...
        /// For inter-node ghost updates
        {
          ...
          for(size_t i=0; i<3; i++) {
            VEC_ASSERT_GEQ((subCellGrid[i]), (2*halfCellInt));
          }
          ...
          if(!rank) {
            ...
            std::cout << "subNodeGrid:"<<" "<< subNodeGrid[0]<<" "<<subNodeGrid[1]<<" "<<subNodeGrid[2] << std::endl;
            std::cout << "subNodeGrid: ";
            std::copy(std::begin(subNodeGrid), std::end(subNodeGrid), std::ostream_iterator<size_t>(std::cout, " "));
            std::cout << std::endl;
          }
          #endif
          ...
          for(size_t thisCoord = 0; thisCoord < 3; ++thisCoord)
          {
              ...
              std::vector<size_t>& plane)
            {
              ...
              for(size_t i2=0; i2<subNodeGrid[otherCoord2]; i2++){
                ...
                for(size_t i1=0; i1<subNodeGrid[otherCoord1]; i1++){
                  ...
                  plane.push_back(nodeCoord[0]+subNodeGrid[0]*(nodeCoord[1]+subNodeGrid[1]*nodeCoord[2]));
                }
              }
            };
            ...
            if(!rank) {
              ...
              auto printList = [](auto const& list, auto const& name){
                std::cout << " " << name << ": ";
                std::copy(std::begin(list), std::end(list), std::ostream_iterator<size_t>(std::cout, " "));
                std::cout << std::endl;
              };

              printList(commNodesReal[2*thisCoord+0],std::string("commNodesReal[")+std::to_string(2*thisCoord+0)+"]");
              printList(commNodesReal[2*thisCoord+1],std::string("commNodesReal[")+std::to_string(2*thisCoord+1)+"]");
              printList(commNodesGhost[2*thisCoord+0],std::string("commNodesGhost[")+std::to_string(2*thisCoord+0)+"]");
              printList(commNodesGhost[2*thisCoord+1],std::string("commNodesGhost[")+std::to_string(2*thisCoord+1)+"]");
            }
            #endif
          }
          ...
          {
            auto verifyCommNodes = [this](size_t dir, std::vector<size_t> const& plane, bool commReal)
            {
              ...
              /// make sure that every commCell in this direction (ghost and real) belongs to lcSet
              if(commReal){
                for(Cell* c: commCells[dir].reals){
                  VEC_ASSERT(!(lcSet.find(c)==lcSet.end()));
                }
              } else {
          ...
          /// verify that commCells in virtual Node match the commCells of the entire subdomain
          {
            auto verifyCommCells = [this](size_t dir, std::vector<size_t> const& plane, bool commReal)
            {
              ...
              for(const auto& n: plane){
                ...
                if(commReal) {
                  for(const auto& ic: vcc[dir].reals) {
                    VEC_ASSERT(!(std::find(ccr.begin(),ccr.end(),vlc[ic])==ccr.end()));
                  }
                } else {
                  for(const auto& ic: vcc[dir].ghosts) {
                    VEC_ASSERT(!(std::find(ccg.begin(),ccg.end(),vlc[ic])==ccg.end()));
                  }
                }
              }
            };
          ...
          for (size_t coord = 0; coord < 3; ++coord) {
            ...
            for (size_t lr = 0; lr < 2; ++lr) {
              ...
              subNodePairsIntra[dir].clear();
              {
                ...
                for(size_t i=0; i<3; i++) numSubNodePairs *= (subNodeEnd[i]-subNodeStart[i]);
                for(size_t i2=subNodeStart[2]; i2<subNodeEnd[2]; i2++){
                  for(size_t i1=subNodeStart[1]; i1<subNodeEnd[1]; i1++){
                    for(size_t i0=subNodeStart[0]; i0<subNodeEnd[0]; i0++){
                      size_t sender = i0 + subNodeGrid[0] * (i1 + subNodeGrid[1] * i2);
                      size_t recver = vdd[sender].nodeGridLocal.getNodeNeighborIndex(dir); /// global index
                ...
                if(!rank)
                {
                  ...
                  std::cout << "      subNodeStart:   "; for(auto const& s: subNodeStart) std::cout << " " << s; std::cout << std::endl;
                  std::cout << "      subNodeEnd:     "; for(auto const& s: subNodeEnd) std::cout << " " << s; std::cout << std::endl;
                  std::cout << "      numSubNodePairs: " << numSubNodePairs << std::endl;
                  std::cout << "      subNodePairsIntra: ";
                    for(const auto& pair: subNodePairsIntra[dir]) std::cout << " (" << pair.first << "," << pair.second << ")";
                    std::cout << std::endl;
                }
                #endif
              }
              ...
              if(doPeriodic)
              {
                ...
                if(!rank)
                {
                  ...
                  std::cout << "      subNodeEnd:     "; for(auto const& s: subNodeEnd) std::cout << " " << s; std::cout << std::endl;
                  std::cout << "      numSubNodePairs: " << numSubNodePairs << std::endl;
                  std::cout << "      subNodePairsInter: ";
                    for(const auto& pair: subNodePairsInter[dir]) std::cout << " (" << pair.first << "," << pair.second << ")";
                    std::cout << std::endl;
                }
                #endif
              }
                ...
                for(std::int64_t a2=-1; a2<=+1; a2++){
                  ...
                  for(std::int64_t a1=-1; a1<=+1; a1++){
                    ...
                    for(std::int64_t a0=-1; a0<=+1; a0++){
                      ...
                      const std::int64_t nidx = n0 + subNodeGrid[0] * (n1 + subNodeGrid[1] * n2);
                      VEC_ASSERT_LT(nidx,numSubNodes);
                      const auto nwave = resortWavesAssigned[nidx];

                      VEC_ASSERT_NEQ(mywave, nwave);
      ...
      /// Prepare resort using hpx par-for
      {
        for(size_t inode=0; inode<vdd.size(); inode++)
        {
          ...
          VEC_ASSERT_EQUAL(vd.vLocalCells.size(), numLocals);
          VEC_ASSERT_EQUAL(vd.vRealCells.size(), numReals);
        ...
        for (int coord = 0; coord < 3; ++coord) {
          for (int lr = 0; lr < 2; ++lr) {
            ...
            VEC_ASSERT_EQUAL(commNodesReal[dir].size(),commNodesGhost[dir].size());
            ...
            for(int idxCommNode=0; idxCommNode<numCommNodes; idxCommNode++){
              ...
              VEC_ASSERT_EQUAL(
                vdd[inode].commCells[dir].reals.size(),
    ...
    void DomainDecomposition::initChannels()
    {
      ...
      VEC_ASSERT(commUseChannels);
      VEC_ASSERT(VecRuntime::isRunning());
    ...
    void DomainDecomposition::loadCells()
    {
      ...
      auto f = [this](size_t i){
        ...
        vs.particles.copyFrom(vs.localCells, this->vecMode);
      };
    ...
    void DomainDecomposition::prepareGhostBuffers()
    {
      ...
      for (size_t coord = 0; coord < 3; ++coord) {
        for (size_t lr = 0; lr < 2; ++lr) {
          ...
          auto f_count_particles = [this](bool commReals, size_t dir, std::vector<size_t>& nodeRange)
          {
            // bool commReals = true;
            std::vector< size_t > const& cn = commReals ? commNodesReal[dir] : commNodesGhost[dir];
          ...
          #if 0
          {
            ...
            std::cout << "  dir: "<<dir<<" dirTotalReal: " << dirTotalReal << " nodeRangeReal[dir]:";
            for(const auto& p: nodeRangeReal[dir]){
              std::cout << " " << p;
            }
            std::cout << std::endl;
            std::cout << "  oppDir: "<<oppDir<<" dirTotalGhost: " << dirTotalGhost << " nodeRangeGhost[oppDir]:";
      ...
      #if 0
      {
        ...
        std::cout << " buffReal.size(): " << buffReal.size() << " buffGhost.size(): " << buffGhost.size() << std::endl;
      }
      #endif
    ...
    void DomainDecomposition::decomposeVec()
    {
      ...
      }
      {
        ...
        if(decompUseParFor)
        {
          decomposeRealParticlesVecParFor_MultiNode_NonPeriodic();
        }
        else
        {
          decomposeRealParticlesVecCellTask_MultiNode_NonPeriodic();
        }
    ...
    void DomainDecomposition::exchangeGhosts_SingleNode()
    {
      ...
      for (int _coord = 0; _coord < 3; ++_coord) {
        ...
        if (nodeGrid.getGridSize(coord) != 1)
          VEC_THROW_EXCEPTION(hpx::not_implemented,"exchangeGhosts_SingleNode","not implemented for nodeGrid>1");
    ...
    void DomainDecomposition::decomposeRealParticlesVecCellTask_MultiNode_NonPeriodic()
    {
      // std::cout << getSystem()->comm->rank() << ": " << " decomposeRealParticlesVecCellTask_MultiNode_NonPeriodic useDecomposeVec="<< useDecomposeVec << std::endl;
...

      {
        ...
        auto mapPositionToLocalCell = [this, &cellSize, &invCellSize, &frame](const Real3D& pos)
        {
          ...
          for(int i = 0; i < 3; ++i) {
            ...
            #if 0
            {
              ...
              if(pos[i] < myLeft || pos[i] >= myRight) VEC_THROW_EXCEPTION(hpx::assertion_failure, "", "pos[i]="<<pos[i]<<
                " allowed: ["<<myLeft<<","<<myRight<<")" );
            ...
            #if 0
            {
              ...
              if((cpos[i] < 0) || (cpos[i] >= gridSize)) VEC_THROW_EXCEPTION(hpx::assertion_failure, "", "cpos[i]="<<cpos[i]<<
                " allowed: ["<< 0 << "," << gridSize  << ")");
            }
            #endif
          }
        ...
        auto resortRealCell = [this, &moveIndexedParticleNonGlobal, &mapPositionToLocalCell, &boundary](size_t ic)
        {
          ...
          for (size_t p = 0; p < cell.particles.size(); ++p)
          {
...

            {
              ...
              if (sortCell != &cell) {
                if (sortCell == 0) {
                  VEC_THROW_EXCEPTION(hpx::assertion_failure,"resortRealCell","sortCell == 0 not allowed");
                } else {
        ...
        #if 0
        {
          for(const auto cell: getGhostCells())
          {
            if(!(cell->particles.empty())) {
              VEC_THROW_EXCEPTION(hpx::assertion_failure, "decomposeRealParticlesVecCellTask_SingleNode_NonPeriodic",
          ...
          for(int ic=0; ic<numLocalCells; ic++)
          {
            ...
            for(Particle& part: particles)
            {
              ...
              if(sortCell!=currCell)
                VEC_THROW_EXCEPTION(hpx::assertion_failure,
                  "DomainDecomposition::decomposeRealParticlesVecCellTask_SingleNode_NonPeriodic",
                  "sortCell!=currCell");
            }
          }

          /// TODO: add test to check that particles remain in the left-frame to right+frame volume
            ...
            else
            {
              ...
              longint totalSend = 0, totalRecv = 0;
              {
                ...
                if (nodeGrid.getNodePosition(coord) % 2 == 0) {
                  ...
                  comm.send(nodeGrid.getNodeNeighborIndex(oppositeDir), DD_COMM_TAG, sendSizes.data(), sendSizes.size());
                  comm.recv(nodeGrid.getNodeNeighborIndex(dir), DD_COMM_TAG, recvSizes.data(), recvSizes.size());
                }
                else {
              ...
              recvBuf.resize(totalRecv);
              {
                ...
                if(sendBuf.size()!=totalSend) VEC_THROW_EXCEPTION(hpx::assertion_failure,
                  "decomposeRealParticlesVecCellTask_MultiNode_NonPeriodic",
...

              {
                ...
                // int ctr = 0;
                auto f_unpack = [dir,coord,&recvRanges,&recvSizes,&recvBuf,&boundary,this](int i)
                {
                  ParticleList & reals = commCells[dir].reals[i]->particles;
        ...
        #if 0
        {
          for(size_t ig=0; ig < numGhostCells; ig++)
          {
            ...
            if(ghosts.size()>0) VEC_THROW_EXCEPTION(hpx::assertion_failure,
              "DomainDecomposition::decomposeRealParticlesVecCellTask_SingleNode_NonPeriodic",
    ...
    // #define VEC_DDPF_DEBUG

    void DomainDecomposition::decomposeRealParticlesVecParFor_MultiNode_NonPeriodic()
    {
      ...
      auto resortRealCell = [&moveIndexedParticleNonGlobal, &mapPositionToLocalCell](VDD& vd, Cell& cell)
      {
        for (size_t p = 0; p < cell.particles.size(); ++p)
        {
          Particle &part = cell.particles[p];
          {
            ...
            if (sortCell != &cell) {
              if (sortCell == 0) {
                VEC_THROW_EXCEPTION(hpx::assertion_failure,"resortRealCell","sortCell == 0 not allowed");
                // VEC_THROW_EXCEPTION(hpx::assertion_failure,"resortRealCell","sortCell == 0 not allowed"
                //   << " particle position (" << part.position() << ") mapped to cell " << vd.cellGrid.mapPositionToIndex(part.position()));
              } else {
                moveIndexedParticleNonGlobal(sortCell->particles, cell.particles, p);
                --p;
              }
        ...
        for(size_t inode=0; inode<numSubNodes; inode++)
        {
          ...
          for(Cell* srcCell: vd.vLocalCells)
          {
            for(Particle& p: srcCell->particles)
            {
              ...
              // VEC_ASSERT_EQUAL(srcCell, dstCell);
              ...
              auto mapPositionToLocalCell = [this, &cellSize, &invCellSize, &frame](const Real3D& pos)
              {
                ...
                for(int i = 0; i < 3; ++i) {
                  ...
                  #if 1
                  {
                    const real myLeft  = cellGrid.getMyLeft(i) - cellSize[i];
                    const real myRight = cellGrid.getMyRight(i) + cellSize[i];
                    if(pos[i] < myLeft || pos[i] >= myRight) VEC_THROW_EXCEPTION(hpx::assertion_failure, "mapPositionToLocalCell", "pos[i]="<<pos[i]<<
                      " allowed: ["<<myLeft<<","<<myRight<<")" );
                  }
                  #endif
                  ...
                  {
                    const int gridSize = cellGrid.getFrameGridSize(i);
                    if((cpos[i] < 0) || (cpos[i] >= gridSize)) VEC_THROW_EXCEPTION(hpx::assertion_failure, "mapPositionToLocalCell", "cpos[i]="<<cpos[i]<<
                      " allowed: ["<< 0 << "," << gridSize  << ")");
                  }
                  #endif
                }
          ...
          for (int lr = 0; lr < 2; ++lr) {
            ...
            auto f_intraNode = [this,coord,dir,&boundary]()
            {
              auto f_pair = [this,coord,dir,&boundary](size_t i)
              {
                ...
                #ifdef VEC_DDPF_DEBUG
                  VEC_ASSERT_EQUAL(ccr.size(),ccg.size());
                #endif
                ...
                for(size_t ic=0; ic<numCells; ic++)
                {
                  ...
                  for(Particle &part: ghosts)
                  {
                    if(shift) boundary.foldCoordinate(part.position(), part.image(), coord);
                    reals.push_back(part);
                  }
                }
              };
            ...
            auto f_interNode = [this,coord,dir,oppositeDir,rank,&boundary,&comm]()
            {
              ...
              std::vector<longint> sendRanges, recvRanges; /// starting index for each cell
              longint totalSend = 0, totalRecv = 0; /// number of particles to send and recv
              {
                /// NOTE: assuming uniform subNode division
                const int numCommNodes = commNodesGhost[dir].size();
                const int numCommCells = vdd[commNodesGhost[dir][0]].commCells[dir].ghosts.size();
                ...
                {
                  {
                    ...
                    #ifdef VEC_DDPF_DEBUG
                      VEC_ASSERT_EQUAL(nodeStart, sendSizes.size());
                      VEC_ASSERT_EQUAL(nodeStart, idxCommNode * cc.size());
                    #endif
                    ...
                    #ifdef VEC_DDPF_DEBUG
                      VEC_ASSERT_EQUAL(nodeEnd, sendSizes.size());
                    #endif
                  }
                  {
                ...
                #ifdef VEC_DDPF_DEBUG
                  VEC_ASSERT_EQUAL(numRecvCells, numSendCells);
                  VEC_ASSERT_EQUAL(sendSizes.size(), numSendCells);
                  VEC_ASSERT_EQUAL(recvSizes.size(), numSendCells);
                  VEC_ASSERT_EQUAL(nodeCommCellRange[dir].back(), numSendCells);
                #endif
              ...
              {
                const int numCommNodes = commNodesGhost[dir].size();
                auto f_packNode = [dir,&sendRanges,&sendSizes,&sendBuf,totalSend,this]
                  (int idxCommNode)
                {
                  /// commCells to pack
                  const int inode = commNodesGhost[dir][idxCommNode];
                  ...
                  #ifdef VEC_DDPF_DEBUG
                    VEC_ASSERT_EQUAL(nodeEnd, nodeCommCellRange[dir][idxCommNode+1]);
                  #endif
                  ...
                  for(int ic=0; ic<gc.size(); ic++)
                  {
                    ...
                    #ifdef VEC_DDPF_DEBUG
                      VEC_ASSERT_EQUAL(ghosts.size(),size);
                    #endif

                    for(int ig = 0; ig < size; ig++)
                    {
                      #ifdef VEC_DDPF_DEBUG
                        VEC_ASSERT_LT(ig+start,totalSend);
                      #endif
              ...
              if(rank==1)
              {
                ...
                std::cout << rank << ": f_interNode dir " << dir << " sendSizes:  " << str_list(sendSizes) << "\n"
                          << rank << ": f_interNode dir " << dir << " sendRanges: " << str_list(sendRanges) << "\n"
                          << rank << ": f_interNode dir " << dir << " recvSizes:  " << str_list(recvSizes) << "\n"
                          << rank << ": f_interNode dir " << dir << " recvRanges: " << str_list(recvRanges) << std::endl;
              }
              #endif
              ...
              #if 0
              {
                std::cout << rank << ": f_interNode dir " << dir << " send " << totalSend << ", recv " << totalRecv << std::endl;
              ...
              #ifdef VEC_DDPF_DEBUG
                VEC_ASSERT_EQUAL(recvBuf.size(), totalRecv);
              #endif

              {
                const int numCommNodes = commNodesReal[dir].size();
                auto f_unpackNode = [coord,dir,totalRecv,&recvRanges,&recvSizes,&recvBuf,&boundary,this]
                  (int idxCommNode)
                {
                  /// commCells to unpack
                  const int inode = commNodesReal[dir][idxCommNode];
      ...
      )
    {
      ...
      sigBefCalcForces      = mdintegrator->aftInitF.connect(
                                boost::signals2::at_back,
                                boost::bind(&DomainDecomposition::befCalcForces,this));
      sigUpdateForces       = mdintegrator->aftCalcFLocal.connect(
                                boost::signals2::at_front,
                                boost::bind(&DomainDecomposition::updateForces,this));
      offload = true;
    }
    ...
    void DomainDecomposition::resetParticles()
    {
      ...
      #if 0
      {
        static const size_t chunk_size_ = ESPP_VECTOR_WIDTH;
        std::ostringstream _ss;
        for(auto const& vs: virtualStorage) {
          size_t total = 0;
          for(auto const& lc: vs.localCells) {
