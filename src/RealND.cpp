...
{
    static python::tuple getinitargs(const RealND v)
    {
        python::list ret;
        for (int i = 0; i < v.getDimension(); i++) ret.append(v[i]);
        return python::make_tuple(ret);
    }
};

void RealND::registerPython()
{
    ...
    class_<RealND>("RealND", init<>())
        ...
        .def(init<RealND>())
        .def(init<list>())
        .add_property("dimension", &RealND::getDimension, &RealND::setDimension)
...
{
    static python::tuple getinitargs(const RealNDs v)
    {
        boost::python::list ret;
        for (int i = 0; i < v.getDimension(); i++)
        {
            boost::python::list tmp;
            for (int j = 0; j < v[i].getDimension(); ++j) tmp.append(v[i][j]);
            ret.append(tmp);
        }
        return python::make_tuple(ret);
    }
...
void RealNDs::registerPython()
{
    ...
    class_<RealNDs>("RealNDs", init<>())
        ...
        .add_property("dimension", &RealNDs::getDimension, &RealNDs::setDimension)
