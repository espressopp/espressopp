...
{
/* Constructor */
OrthorhombicBC::OrthorhombicBC(std::shared_ptr<esutil::RNG> _rng, const Real3D& _boxL) : BC(_rng)
{
    setBoxL(_boxL);
}
...
void OrthorhombicBC::getMinimumImageVector(Real3D& dist,
                                           const Real3D& pos1,
...
/* Returns the minimum image vector between two positions */
void OrthorhombicBC::getMinimumImageVectorBox(Real3D& dist,
                                              const Real3D& pos1,
                                              const Real3D& pos2) const
{
    dist = pos1;
    ...
    if (dist[0] < -boxL2[0])
        dist[0] += boxL[0];
    else if (dist[0] > boxL2[0])
        dist[0] -= boxL[0];
    if (dist[1] < -boxL2[1])
        dist[1] += boxL[1];
    else if (dist[1] > boxL2[1])
        dist[1] -= boxL[1];
    if (dist[2] < -boxL2[2])
        dist[2] += boxL[2];
    else if (dist[2] > boxL2[2])
        dist[2] -= boxL[2];
}

/* Fold back a nearby position into box */

void OrthorhombicBC::getMinimumDistance(Real3D& dist) const
{
    if (dist[0] < -0.5 * boxL[0])
        dist[0] += boxL[0];
    else if (dist[0] > 0.5 * boxL[0])
        dist[0] -= boxL[0];
    if (dist[1] < -0.5 * boxL[1])
        dist[1] += boxL[1];
    else if (dist[1] > 0.5 * boxL[1])
        dist[1] -= boxL[1];
    if (dist[2] < -0.5 * boxL[2])
        dist[2] += boxL[2];
    else if (dist[2] > 0.5 * boxL[2])
        dist[2] -= boxL[2];
}

/* Returns the minimum image vector between two positions */
void OrthorhombicBC::getMinimumImageVectorX(real dist[3],
                                            const real pos1[3],
...
void OrthorhombicBC::getRandomPos(Real3D& res) const
{
