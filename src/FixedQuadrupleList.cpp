    ...
    : storage(_storage), globalQuadruples()
{
    ...
    sigBeforeSend = storage->beforeSendParticles.connect(
        std::bind(&FixedQuadrupleList::beforeSendParticles, this, std::placeholders::_1,
                  std::placeholders::_2));
    sigAfterRecv = storage->afterRecvParticles.connect(
        std::bind(&FixedQuadrupleList::afterRecvParticles, this, std::placeholders::_1,
                  std::placeholders::_2));
...
bool FixedQuadrupleList::add(longint pid1, longint pid2, longint pid3, longint pid4)
{
    ...
    if (returnVal)
    {
        ...
        else
        {
            ...
            for (GlobalQuadruples::const_iterator it = equalRange.first; it != equalRange.second;
                 ++it)
    ...
    for (ParticleList::Iterator pit(pl); pit.isValid(); ++pit)
    {
        ...
        if (n > 0)
        {
                 ...
                 ++it)
            {
                ...
                // printf ("send global quadruple: pid %d and partner %d\n", pid,
                // it->second.second); printf ("send global quadruple: pid %d and partner %d\n",
                // pid, it->second.third);
            }
