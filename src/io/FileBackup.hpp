...
public:
    FileBackup(std::string file_name)
    {
        ...
        if (boost::filesystem::exists(file_name))
        {
            ...
            ssnew_file_name << std::setfill('0') << std::setw(4) << now.date().year()
                            << std::setw(2) << now.date().month() << std::setw(2)
                            << now.date().day() << "_" << std::setw(2) << now.time_of_day().hours()
                            << std::setw(2) << now.time_of_day().minutes() << "_"
                            << file_name;  // this is a bit ugly since there are no leading zeros
            std::string new_file_name(ssnew_file_name.str());
            std::cout << "Note: file " << file_name << " exists already. Moving " << file_name
            ...
            if (boost::filesystem::exists(new_file_name))
            {
                ...
                msg << "Warning: can not backup file " << file_name << " to " << new_file_name
