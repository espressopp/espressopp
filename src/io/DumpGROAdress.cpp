...
void DumpGROAdress::dump()
{
    ...
    if (system->comm->rank() == 0)
    {
        ...
        if (myfile.is_open())
        {
            ...
            // myfile << "  currentStep " << integrator->getStep() << "  lengthUnit "<< length_unit
            // << endl;
            ...
            for (int i = 0; i < num_of_particles; i++)
            {
                ...
                // myfile << setw(5) << i+1;    //FIXME this should be the molecule number, not atom
                // number
                myfile << setw(5) << 10000;
                // myfile << setiosflags(ios::left) << setw(1) << "T" << setw(4) <<
                // particleIDToType.find(i+1)->second <<resetiosflags(ios::left);  // pid starts at
                // 1
                // // set(1)+set(4) makes in total 5, as required by the fixed format, should be
                // resname not atomtype stringstream ss; ss << particleIDToType.find(i+1)->second;
                // myfile << setiosflags(ios::right) << setw(5) << (string("T") + ss.str()) <<
                // resetiosflags(ios::right);
                myfile << setiosflags(ios::left) << setw(5) << "TTT"
                       << resetiosflags(ios::left);  // pid starts at 1
                myfile << setiosflags(ios::right) << setw(5) << (string("TTT"))
                       << resetiosflags(ios::right);
                if ((i + 1) < 100000)
                {
                    atomnumber = i + 1;
                ...
                myfile << setw(5)
                       << atomnumber;  // NOTE this is the actual atom number - wrapped at 99999
                // while get token
            ...
            myfile << setw(10) << setprecision(5) << Li[0] * length_factor << setw(10)
                   << setprecision(5) << Li[1] * length_factor << setw(10) << setprecision(5)
                   << Li[2] * length_factor << endl;
            myfile.close();
        }
        else
...
void DumpGROAdress::registerPython()
{
    ...
    class_<DumpGROAdress, bases<ParticleAccess>, boost::noncopyable>(
        "io_DumpGROAdress", init<std::shared_ptr<System>, std::shared_ptr<FixedTupleListAdress>,
                                 std::shared_ptr<integrator::MDIntegrator>, std::string, bool, real,
                                 std::string, bool>())
