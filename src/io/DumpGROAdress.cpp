...
void DumpGROAdress::dump() {
  ...
  if (system->comm->rank() == 0) {
    ...
    if (myfile.is_open()) {
      ...
      //       Li[1] * length_factor << "  0.0  0.0  0.0  "<< Li[2] *
      //       length_factor;
      ...
      for (size_t i = 0; i < num_of_particles; i++) {
        ...
        // myfile << setw(5) << i+1;    //FIXME this should be the molecule
        // number, not atom number
        myfile << setw(5) << 10000;
        // myfile << setiosflags(ios::left) << setw(1) << "T" << setw(4) <<
        // particleIDToType.find(i+1)->second <<resetiosflags(ios::left);  //
        // pid starts at 1 // set(1)+set(4) makes in total 5, as required by the
        // fixed format, should be resname not atomtype
        // stringstream ss;
        // ss << particleIDToType.find(i+1)->second;
        // myfile << setiosflags(ios::right) << setw(5) << (string("T") +
        // ss.str()) << resetiosflags(ios::right);
        ...
        myfile << setw(5) << atomnumber;  // NOTE this is the actual atom number
                                          // - wrapped at 99999
        // while get token
        ...
        ii = line.begin();
        ind = 0;  // FIXME ugly! how do I know the number of a loop when using
                  // iterators?
