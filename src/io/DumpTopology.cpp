...
namespace io
{
void DumpTopology::observeTuple(std::shared_ptr<FixedPairList> fpl) { fpls_.push_back(fpl); }

void DumpTopology::observeTriple(std::shared_ptr<FixedTripleList> ftl) { ftls_.push_back(ftl); }
...
void DumpTopology::saveDataToBuffer()
{
    ...
    int idx = 0;
    for (std::vector<std::shared_ptr<FixedPairList> >::iterator it = fpls_.begin();
         it != fpls_.end(); ++it)
    {
    ...
    idx = 0;
    for (std::vector<std::shared_ptr<FixedTripleList> >::iterator it = ftls_.begin();
         it != ftls_.end(); ++it)
    {
    ...
    idx = 0;
    for (std::vector<std::shared_ptr<FixedQuadrupleList> >::iterator it = fqls_.begin();
         it != fqls_.end(); ++it)
    {
...
void DumpTopology::clearBuffer() { fpl_buffer_.clear(); }

python::list DumpTopology::getData()
{
    // TODO(jakub): direct access to vector from Python instead of rewriting it to boost Python
    // list.
    python::list ret;
...
void DumpTopology::registerPython()
{
    ...
    class_<DumpTopology, bases<ParticleAccess>, boost::noncopyable>(
        "io_DumpTopology",
        init<std::shared_ptr<System>, std::shared_ptr<integrator::MDIntegrator> >())
