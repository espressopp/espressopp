    ...
    if (system->comm->rank() == 0)
    {
        ...
        if (myfile.is_open())
        {
            ...
            for (int i = 0; i < num_of_particles; i++)
            {
                ...
                if (store_velocities)
                {
                    myfile << " " << length_factor * cei.currentProperties()[3] << " "
                           << length_factor * cei.currentProperties()[4] << " "
...
void DumpXYZ::registerPython()
{
    ...
    class_<DumpXYZ, bases<ParticleAccess>, boost::noncopyable>(
        "io_DumpXYZ", init<std::shared_ptr<System>, std::shared_ptr<integrator::MDIntegrator>,
