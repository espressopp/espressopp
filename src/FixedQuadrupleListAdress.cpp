...
LOG4ESPP_LOGGER(FixedQuadrupleListAdress::theLogger,
...
FixedQuadrupleListAdress::FixedQuadrupleListAdress(
  ...
  sigBeforeSendAT = fixedtupleList->beforeSendATParticles.connect(boost::bind(
      &FixedQuadrupleListAdress::beforeSendATParticles, this, _1, _2));
...
bool FixedQuadrupleListAdress::add(longint pid1, longint pid2, longint pid3,
                                   longint pid4) {
  ...
  if (!p1) {
    ...
    if (!p2) {
      ...
      msg << "Quadruple particle p2 " << pid2
          << " does not exists here and cannot be added.";
      err.setException(msg.str());
    }
    if (!p3) {
      std::stringstream msg;
      msg << "Quadruple particle p3 " << pid3
          << " does not exists here and cannot be added.";
      err.setException(msg.str());
    }
    if (!p4) {
      std::stringstream msg;
      msg << "Quadruple particle p4 " << pid4
          << " does not exists here and cannot be added.";
      err.setException(msg.str());
    }
  }
  ...
  if (returnVal) {
    ...
    std::pair<GlobalQuadruples::const_iterator,
              GlobalQuadruples::const_iterator>
    ...
    if (equalRange.first == globalQuadruples.end()) {
      ...
      globalQuadruples.insert(std::make_pair(
          pid1, Triple<longint, longint, longint>(pid2, pid3, pid4)));
    } else {
...
void FixedQuadrupleListAdress::beforeSendATParticles(std::vector<longint> &atpl,
                                                     OutBuffer &buf) {
...
void FixedQuadrupleListAdress::beforeSendParticles(ParticleList &pl,
                                                   OutBuffer &buf) {}
...
void FixedQuadrupleListAdress::onParticlesChanged() {
  ...
  for (GlobalQuadruples::const_iterator it = globalQuadruples.begin();
    ...
    if (it->first != lastpid1) {
      ...
      if (p1 == NULL) {
        std::stringstream msg;
        msg << "Quadruple particle p1 " << it->first
    ...
    if (p2 == NULL) {
      std::stringstream msg;
      msg << "Quadruple particle p2 " << it->second.first
          << " does not exists here.";
      err.setException(msg.str());
    }
    ...
    if (p3 == NULL) {
      std::stringstream msg;
      msg << "Quadruple particle p3 " << it->second.second
          << " does not exists here.";
      err.setException(msg.str());
    }
    ...
    if (p4 == NULL) {
      std::stringstream msg;
      msg << "Quadruple particle p4 " << it->second.third
          << " does not exists here.";
      err.setException(msg.str());
    }
    this->add(p1, p2, p3, p4);
