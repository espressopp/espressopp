...
//////////////////////////////////////////////////
// Real3D
inline Real3D::Real3D() { data[0] = data[1] = data[2] = 0; }

inline Real3D::Real3D(real v) { data[0] = data[1] = data[2] = v; }
...
inline real& Real3D::operator[](int i) { return data[i]; }

inline const real& Real3D::operator[](int i) const { return data[i]; }

inline real& Real3D::at(int i)
{
    if (i < 0 || i > 2) throw std::out_of_range("Real3D::at");
    return (*this)[i];
}
...
inline bool Real3D::operator!=(const Real3D& v) const { return !(*this == v); }

// elementwise binary operators
inline Real3D Real3D::operator+(const Real3D& v) const { return Real3D(*this) += v; }

inline Real3D Real3D::operator-(const Real3D& v) const { return Real3D(*this) -= v; }

inline Real3D Real3D::operator*(real v) const { return Real3D(*this) *= v; }
...
{
    return Real3D(data[1] * v[2] - data[2] * v[1], data[2] * v[0] - data[0] * v[2],
...
inline real Real3D::abs() const { return sqrt(sqr()); }
...
//////////////////////////////////////////////////
// Global operators
inline Real3D operator*(real s, const Real3D& v) { return Real3D(v) * s; }
