...
// ESPP_CLASS
#ifndef _INTERACTION_DIHEDRALHARMONICUNIQUECOS_HPP
#define _INTERACTION_DIHEDRALHARMONICUNIQUECOS_HPP
...
namespace espressopp {
  namespace interaction {
     ...
     */
    class DihedralHarmonicUniqueCos : public DihedralUniquePotentialTemplate< DihedralHarmonicUniqueCos > {
    private:
      real K;
    public:
      static void registerPython();

      DihedralHarmonicUniqueCos() : K(0.0) { }
      DihedralHarmonicUniqueCos(real _K) : K(_K) { }
      ...
      real _computeEnergyRaw(real _phi, real _phi0) const {
        // _phi should be in radians
        real energy = K * pow(cos(_phi) - cos(_phi0), 2);
        return energy;
      }
      ...
      real _computeForceRaw(real phi, real _phi0) const {
        real cos_phi = cos(phi);
        if(cos_phi < -1.0) cos_phi = -1.0;
        else if(cos_phi >  1.0) cos_phi =  1.0;

        return 2.0 * K * (cos_phi - cos(_phi0));
      }
