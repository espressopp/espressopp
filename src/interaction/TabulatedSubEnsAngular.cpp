...
{
void TabulatedSubEnsAngular::setFilenames(int dim, int itype, boost::python::list _filenames)
{
...
void TabulatedSubEnsAngular::setColVarRef(const RealNDs& cvRefs)
{
...
void TabulatedSubEnsAngular::computeColVarWeights(const Real3D& dist12,
                                                  const Real3D& dist32,
                                                  const bc::BC& bc)
{
    // Sanity Check
    ...
    if (!stuck)
    {
        ...
        for (int i = 0; i < numInteractions - 1; ++i)
        {
            ...
            for (int j = 0; j < colVar.getDimension(); ++j)
            {
                ...
                if (j <= 0 + colVarAngleListSize)
                    k = 0;
                else if (j < 1 + colVarAngleListSize + colVarBondListSize)
                    k = 1;
                else
                    k = 2;
...
void TabulatedSubEnsAngular::setColVar(const Real3D& dist12, const Real3D& dist32, const bc::BC& bc)
{
...
void TabulatedSubEnsAngular::registerPython()
{
    ...
    class_<TabulatedSubEnsAngular, bases<AngularPotential> >("interaction_TabulatedSubEnsAngular",
                                                             init<>())
