...
public:
    ...
    real _computeEnergyRaw(real _phi) const
    {
        ...
        real energy = K0 + K1 * cos_phi + K2 * cos_phi * cos_phi +
    ...
    Real3D prod(Real3D a, Real3D b) const
    {
        ...
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++) res[i] += (1.0 - d(i, j)) * a[j] * b[j];
        return res;
    }
                          ...
                          const Real3D& r43) const
    {
        ...
        if (rijjk_abs < 0.00000001 || rjkkn_abs < 0.00000001) return;
        ...
        real coef1 = -K1 - 2.0 * K2 * cos_phi - 3.0 * K3 * cos_phi * cos_phi -
