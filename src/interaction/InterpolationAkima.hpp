...
private:
    ...
    void spline(const real* x, const real* y, int N, real* p0, real* p1, real* p2, real* p3);

    // Spline interpolation
    real splineInterpolation(
        real r, const real* p0, const real* p1, const real* p2, const real* p3) const;
...
inline real InterpolationAkima::splineInterpolation(
    real r, const real* p0, const real* p1, const real* p2, const real* p3) const
{
    int index;
    ...
    if (index < 0)
    {
        LOG4ESPP_ERROR(theLogger, "distance " << r << " out of range " << inner << " - "
                                              << inner + (N - 1) * delta);
        index = 0;
    }
    if (index >= N)
    ...
    int zz2 = z * z;
    return p0[index] + p1[index] * z + p2[index] * zz2 + p3[index] * zz2 * z;
}
