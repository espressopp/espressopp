...
{
void TabulatedSubEnsDihedral::setFilenames(int dim, int itype, boost::python::list _filenames)
{
...
void TabulatedSubEnsDihedral::setColVarRef(const RealNDs& cvRefs)
{
...
void TabulatedSubEnsDihedral::computeColVarWeights(const Real3D& dist21,
                                                   const Real3D& dist32,
                                                   const Real3D& dist43,
                                                   const bc::BC& bc)
{
    ...
    if (!stuck)
    {
        ...
        for (int i = 0; i < numInteractions - 1; ++i)
        {
            ...
            for (int j = 0; j < colVar.getDimension(); ++j)
            {
                ...
                if (j <= 0 + colVarDihedListSize)
                    k = 0;
                else if (j < 1 + colVarDihedListSize + colVarBondListSize)
                    k = 1;
                else
                    k = 2;
...
void TabulatedSubEnsDihedral::setColVar(const Real3D& r21,
                                        const Real3D& r32,
                                        const Real3D& r43,
                                        const bc::BC& bc)
{
...
void TabulatedSubEnsDihedral::registerPython()
{
    ...
    class_<FixedQuadrupleListTabulatedSubEnsDihedral, bases<Interaction> >(
        ...
        .def("getFixedQuadrupleList",
             &FixedQuadrupleListTabulatedSubEnsDihedral::getFixedQuadrupleList);
    ...
    class_<FixedQuadrupleListTypesTabulatedSubEnsDihedral, bases<Interaction> >(
        ...
        .def("setFixedQuadrupleList",
             &FixedQuadrupleListTypesTabulatedSubEnsDihedral::setFixedQuadrupleList)
        .def("getFixedQuadrupleList",
             &FixedQuadrupleListTypesTabulatedSubEnsDihedral::getFixedQuadrupleList);
}
