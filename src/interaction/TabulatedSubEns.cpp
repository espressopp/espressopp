...
{
void TabulatedSubEns::setFilenames(int dim, int itype, boost::python::list _filenames)
{
...
void TabulatedSubEns::addInteraction(int itype, boost::python::str fname, const RealND& _cvref)
{
    ...
    // Dimension 8: bond, angle, dihed(sin), dihed(cos), sd_bond, sd_angle, sd_dihed(sin),
    // sd_dihed(cos)
    colVarRef[i].setDimension(6);
    colVarRef[i] = _cvref;
...
void TabulatedSubEns::computeColVarWeights(const Real3D& dist, const bc::BC& bc)
{
    ...
    if (weightCounts > 0 && maxWeightI < numInteractions - 1 && maxWeight > 0. &&
    ...
    if (!stuck)
    {
        ...
        for (int i = 0; i < numInteractions - 1; ++i)
        {
            ...
            for (int j = 0; j < colVar.getDimension(); ++j)
            {
                ...
                if (j <= 0 + colVarBondListSize)
                    k = 0;
                else if (j < 1 + colVarBondListSize + colVarAngleListSize)
                    k = 1;
                else
                    k = 2;
            ...
            if (norm_d_i > norm_l_i) weights[i] = exp(-(sqrt(norm_d_i) - sqrt(norm_l_i)) / alpha);
            if (weights[i] > maxWeight)
            {
                maxWeight = weights[i];
                maxWeightI = i;
        ...
        for (int i = 0; i < numInteractions - 1; ++i)
        {
            ...
            {
                if (weightCounts > 0 && weights[i] > 0.01 &&
        ...
        if (maxWeightI == numInteractions - 1) maxWeight = 1.;
        weights[numInteractions - 1] = 1. - maxWeight;
    }

    // Update weightSum
    for (int i = 0; i < numInteractions; ++i) weightSum[i] += weights[i];
    weightCounts += 1;
}
...
typedef class VerletListAdressInteractionTemplate<TabulatedSubEns, TabulatedSubEns>
    VerletListAdressTabulatedSubEns;
typedef class VerletListHadressInteractionTemplate<TabulatedSubEns, TabulatedSubEns>
    VerletListHadressTabulatedSubEns;
...
typedef class FixedPairListTypesInteractionTemplate<TabulatedSubEns>
    FixedPairListTypesTabulatedSubEns;
...
void TabulatedSubEns::registerPython()
{
    ...
    class_<TabulatedSubEns, bases<Potential> >("interaction_TabulatedSubEns", init<>())
    ...
    class_<VerletListTabulatedSubEns, bases<Interaction> >("interaction_VerletListTabulatedSubEns",
    ...
    class_<CellListTabulatedSubEns, bases<Interaction> >("interaction_CellListTabulatedSubEns",
