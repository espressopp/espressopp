...
public:
    VerletListAdressInteractionTemplate(std::shared_ptr<VerletListAdress> _verletList,
...
//////////////////////////////////////////////////
template <typename _PotentialAT, typename _PotentialCG>
inline void VerletListAdressInteractionTemplate<_PotentialAT, _PotentialCG>::addForces()
{
    ...
    /*for (std::set<Particle*>::iterator it=cgZone.begin();
        ...
        if (it3 != fixedtupleList->end()) {
            ...
            for (std::vector<Particle*>::iterator it2 = atList.begin();
                ...
                //Real3D d1;
                //verletList->getSystem()->bc->getMinimumImageVectorBox(d1, at.position(),
    vp.position());
                //cmp += at.mass() * d1;
    ...
    intra-atomistic
    // rotations and vibrations in the CG zone. This leads to failures in the kinetic energy.
    However, in Force-AdResS there is no energy conservation anyway.
    // Here we calculate CG forces/velocities and distribute them to AT particles. In contrast, in
    H-AdResS, we calculate AT forces from intra-molecular
    // interactions and inter-molecular center-of-mass interactions and just update the positions of
    the center-of-mass CG particles. std::set<Particle*> cgZone = verletList->getCGZone(); for
    (std::set<Particle*>::iterator it=cgZone.begin(); it != cgZone.end(); ++it) {
    ...
    // Compute center of mass and weights for virtual particles in Adress and CG zone (HY and AT and
    // CG region).
    ...
    for (std::set<Particle *>::iterator it = cgZone.begin(); it != cgZone.end(); ++it)
    {
        ...
        if (it3 != fixedtupleList->end())
        {
                 ...
                 ++itv)
            {
                Particle &at = **itv;
                at.velocity() =
                    vp.velocity();  // Overwrite velocity - Note (Karsten): See comment above.
...
template <typename _PotentialAT, typename _PotentialCG>
inline real VerletListAdressInteractionTemplate<_PotentialAT, _PotentialCG>::computeEnergy()
{
    std::set<Particle *> cgZone = verletList->getCGZone();
    for (std::set<Particle *>::iterator it = cgZone.begin(); it != cgZone.end(); ++it)
    {
        ...
        if (it3 != fixedtupleList->end() && it4 != fixedtupleList->end())
        {
                 ...
                 ++itv)
            {
                     ...
                     itv2 != atList2.end(); ++itv2)
                {
                    ...
                    // counter += 1;
                    // std::cout << "AT Energy calculation AT/HY region done, w12 = " << w12 << "
                    // and w1 = " << w1 << " and w2 = " << w2 << " okay...\n";
                }
            }
        }
    }
...
template <typename _PotentialAT, typename _PotentialCG>
inline real VerletListAdressInteractionTemplate<_PotentialAT, _PotentialCG>::computeEnergyDeriv()
{
    // for use with TI versions of potentials
    // for the moment, it assumes all particles which differ between states A and B are in the
    // atomistic region, because _computeEnergyDeriv not defined for other potentials assumes lambda
    // for each particle is up-to-date, e.g. computeEnergy or a routine from Adress extension has
    // been called

    LOG4ESPP_INFO(theLogger,
                  "compute energy derivative of the Verlet list pairs, in the atomistic region");
...
template <typename _PotentialAT, typename _PotentialCG>
inline real VerletListAdressInteractionTemplate<_PotentialAT, _PotentialCG>::computeEnergyAA()
{
    std::cout << "Warning! At the moment computeEnergyAA() in VerletListAdressInteractionTemplate "
                 "does not work."
              << std::endl;
    return 0.0;
}
    ...
    int atomtype)
{
    std::cout << "Warning! At the moment computeEnergyAA(int atomtype) in "
                 "VerletListAdressInteractionTemplate does not work."
              << std::endl;
    return 0.0;
}

template <typename _PotentialAT, typename _PotentialCG>
inline real VerletListAdressInteractionTemplate<_PotentialAT, _PotentialCG>::computeEnergyCG()
{
    std::cout << "Warning! At the moment computeEnergyCG() in VerletListAdressInteractionTemplate "
                 "does not work."
              << std::endl;
    return 0.0;
}
...
template <typename _PotentialAT, typename _PotentialCG>
inline void VerletListAdressInteractionTemplate<_PotentialAT, _PotentialCG>::computeVirialX(
    std::vector<real> &p_xx_total, int bins)
{
    // std::cout << "Warning! At the moment computeVirialX in VerletListAdressInteractionTemplate
    // does not work." << std::endl << "Therefore, the corresponding interactions won't be included
    // in calculation." << std::endl;
...
template <typename _PotentialAT, typename _PotentialCG>
inline real VerletListAdressInteractionTemplate<_PotentialAT, _PotentialCG>::computeVirial()
{
...
template <typename _PotentialAT, typename _PotentialCG>
inline void VerletListAdressInteractionTemplate<_PotentialAT, _PotentialCG>::computeVirialTensor(
    Tensor &w)
{
    ...
    Tensor &w, real z)
{
    ...
    std::cout << "Warning! At the moment IK computeVirialTensor in VerletListAdress does'n work"
              << std::endl;
...
template <typename _PotentialAT, typename _PotentialCG>
inline real VerletListAdressInteractionTemplate<_PotentialAT, _PotentialCG>::getMaxCutoff()
{
    real cutoff = 0.0;
