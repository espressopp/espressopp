...
public:
    VerletListPIadressInteractionTemplate(std::shared_ptr<VerletListAdress> _verletList,
    ...
    {  // Iterate over all particles
        ...
        if (w < 1.0 && w > 0.0)
        {
            ...
            if (verletList->getAdrRegionType())
            {
                ...
                mindriftforce *=
                    (0.5 * (1.0 / ntrotter) * energydiff.find(&vp)->second) * vp.lambdaDeriv();
...
template <typename _PotentialQM, typename _PotentialCL>
inline real VerletListPIadressInteractionTemplate<_PotentialQM, _PotentialCL>::computeEnergy()
{
    // Pairs not inside the QM/Hybrid Zone (i.e. CL region)
    real e = 0.0;
    for (PairList::Iterator it(verletList->getPairs()); it.isValid(); ++it)
...
template <typename _PotentialQM, typename _PotentialCL>
inline real VerletListPIadressInteractionTemplate<_PotentialQM, _PotentialCL>::computeEnergyDeriv()
{
    std::cout << "Warning! At the moment computeEnergyDeriv() in "
                 "VerletListPIadressInteractionTemplate does not work."
              << std::endl;
    return 0.0;
}

template <typename _PotentialQM, typename _PotentialCL>
inline real VerletListPIadressInteractionTemplate<_PotentialQM, _PotentialCL>::computeEnergyAA()
{
    real e = 0.0;
...
template <typename _PotentialQM, typename _PotentialCL>
inline real VerletListPIadressInteractionTemplate<_PotentialQM, _PotentialCL>::computeEnergyCG()
{
    real e = 0.0;
...
template <typename _PotentialQM, typename _PotentialCL>
inline real VerletListPIadressInteractionTemplate<_PotentialQM, _PotentialCL>::computeVirial()
{
    real w = 0.0;
...
template <typename _PotentialQM, typename _PotentialCL>
inline real VerletListPIadressInteractionTemplate<_PotentialQM, _PotentialCL>::getMaxCutoff()
{
    real cutoff = 0.0;
