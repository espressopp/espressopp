...
public:
    FixedLocalTupleRgListInteractionTemplate(std::shared_ptr<System> _system,
    ...
    void setRG(longint id, const real rg) { rg_origin[id] = rg; }

    void setFixedLocalTupleList(std::shared_ptr<FixedLocalTupleList> _fixedtupleList)
    {
        fixedtupleList = _fixedtupleList;
    }

    std::shared_ptr<FixedLocalTupleList> getFixedLocalTupleList() { return fixedtupleList; }
...
protected:
    ...
    boost::unordered_map<long unsigned int, real> computeRG(
        boost::unordered_map<long unsigned int, Real3D> center)
    {
...
//////////////////////////////////////////////////
template <typename _Potential>
inline void FixedLocalTupleRgListInteractionTemplate<_Potential>::addForces()
{
    ...
    for (FixedLocalTupleList::TupleList::Iterator it(*fixedtupleList); it.isValid(); ++it)
    {
        ...
        bc.getMinimumImageVectorBox(diff, p->position(), center[id]);
        ...
        {
            Particle* pt = pList[i];
            bc.getMinimumImageVectorBox(diff, pt->position(), center[id]);
            pt->force() += p->mass() * potential->_computeForce(diff, diff_rg, N_Constrain);
        }
    }
}

template <typename _Potential>
inline real FixedLocalTupleRgListInteractionTemplate<_Potential>::computeEnergy()
{
...
template <typename _Potential>
inline real FixedLocalTupleRgListInteractionTemplate<_Potential>::computeEnergyDeriv()
{
    std::cout << "Warning! At the moment computeEnergyDeriv() in "
                 "FixedLocalTupleRgListInteractionTemplate does not work."
              << std::endl;
    return 0.0;
}

template <typename _Potential>
inline real FixedLocalTupleRgListInteractionTemplate<_Potential>::computeEnergyAA()
{
...
template <typename _Potential>
inline real FixedLocalTupleRgListInteractionTemplate<_Potential>::computeEnergyAA(int atomtype)
{
...
template <typename _Potential>
inline real FixedLocalTupleRgListInteractionTemplate<_Potential>::computeEnergyCG()
{
...
template <typename _Potential>
inline real FixedLocalTupleRgListInteractionTemplate<_Potential>::computeEnergyCG(int atomtype)
{
...
template <typename _Potential>
inline void FixedLocalTupleRgListInteractionTemplate<_Potential>::computeVirialX(
...
template <typename _Potential>
inline real FixedLocalTupleRgListInteractionTemplate<_Potential>::computeVirial()
{
...
template <typename _Potential>
inline void FixedLocalTupleRgListInteractionTemplate<_Potential>::computeVirialTensor(Tensor& w)
{
    ...
    for (FixedLocalTupleList::TupleList::Iterator it(*fixedtupleList); it.isValid(); ++it)
    {
        ...
        for (int i = 0; i < N_Constrain - 1; i++)
        {
            ...
            bc.getMinimumImageVectorBox(diff, pt->position(), center[id]);
            wlocal +=
                Tensor(diff, p->mass() * potential->_computeForce(diff, diff_rg, N_Constrain));
        }
    }
                                                                                      ...
                                                                                      real z)
{
    std::cout << "Warning! Calculating virial layerwise is not supported for "
                 "in FixedLocalTupleRgListInteractionTemplate."
              << std::endl;
}
