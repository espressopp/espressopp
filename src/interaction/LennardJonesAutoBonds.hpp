...
public:
    ...
    LennardJonesAutoBonds() : epsilon(0.0), sigma(0.0)
    {
        setShift(0.0);
        setCutoff(0.0);
        max_crosslinks = 0;
    ...
    LennardJonesAutoBonds(real _epsilon,
                          real _sigma,
                          real _cutoff,
                          real _shift,
    ...
    void setMaxCrosslinks(int _max_crosslinks) { max_crosslinks = _max_crosslinks; }
    ...
    inline real computeEnergy(const Real3D& dist) const { return computeEnergySqr(dist.sqr()); }

    inline real computeEnergy(real dist) const { return computeEnergySqr(dist * dist); }

    inline real computeEnergySqr(real distsqr) const { return _computeEnergySqr(distsqr); }
    ...
    inline real _computeEnergy(const Real3D& dist) const { return _computeEnergySqr(dist.sqr()); }

    inline real _computeEnergy(real dist) const { return _computeEnergySqr(dist * dist); }
    ...
    inline bool _computeForce(Real3D& force, const Particle& p1, const Particle& p2) const
    {
        ...
        {
            printf(
                "WARNING (this should never happen !): didn't add crosslink, because particle1 is "
                "ghosts");
