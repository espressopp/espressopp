...
public:
    FixedQuadrupleListTypesInteractionTemplate(
        std::shared_ptr<System> _system, std::shared_ptr<FixedQuadrupleList> _fixedquadrupleList)
    ...
    void setPotential(int type1, int type2, int type3, int type4, const Potential &potential)
    {
        ...
        ntypes = std::max(ntypes,
                          std::max(std::max(std::max(type1 + 1, type2 + 1), type3 + 1), type4 + 1));
        potentialArray.at(type1, type2, type3, type4) = potential;
        if (type1 != type4 || type2 != type3)
...
//////////////////////////////////////////////////
template <typename _DihedralPotential>
inline void FixedQuadrupleListTypesInteractionTemplate<_DihedralPotential>::addForces()
{
    ...
    for (FixedQuadrupleList::QuadrupleList::Iterator it(*fixedquadrupleList); it.isValid(); ++it)
    {
        ...
        potential.computeForce(force1, force2, force3, force4, dist21, dist32, dist43);
        p1.force() += force1;
        p2.force() += force2;
        p3.force() += force3;
        p4.force() += force4;
...
template <typename _DihedralPotential>
inline real FixedQuadrupleListTypesInteractionTemplate<_DihedralPotential>::computeEnergy()
{
...
template <typename _DihedralPotential>
inline real FixedQuadrupleListTypesInteractionTemplate<_DihedralPotential>::computeEnergyDeriv()
{
    std::cout << "Warning! At the moment computeEnergyDeriv() in "
                 "FixedQuadrupleListTypesInteractionTemplate does not work."
              << std::endl;
    return 0.0;
}

template <typename _DihedralPotential>
inline real FixedQuadrupleListTypesInteractionTemplate<_DihedralPotential>::computeEnergyAA()
{
...
template <typename _DihedralPotential>
inline real FixedQuadrupleListTypesInteractionTemplate<_DihedralPotential>::computeEnergyCG()
{
...
template <typename _DihedralPotential>
inline void FixedQuadrupleListTypesInteractionTemplate<_DihedralPotential>::computeVirialX(
    std::vector<real> &p_xx_total, int bins)
{
    std::cout << "Warning! At the moment computeVirialX(std::vector<real> &p_xx_total, int bins) "
                 "in FixedQuadrupleListTypesInteractionTemplate does not work."
              << std::endl;
}

template <typename _DihedralPotential>
inline real FixedQuadrupleListTypesInteractionTemplate<_DihedralPotential>::computeVirial()
{
...
template <typename _DihedralPotential>
inline void FixedQuadrupleListTypesInteractionTemplate<_DihedralPotential>::computeVirialTensor(
    Tensor &w)
{
    ...
    Tensor &w, real z)
{
    ...
    std::cout << "Warning!!! computeVirialTensor in specified volume doesn't work for "
                 "FixedQuadrupleListTypesInteractionTemplate at the moment"
              << std::endl;
...
template <typename _DihedralPotential>
inline real FixedQuadrupleListTypesInteractionTemplate<_DihedralPotential>::getMaxCutoff()
{
    real cutoff = 0.0;
