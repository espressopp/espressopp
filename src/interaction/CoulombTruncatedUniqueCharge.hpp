...
public:
    ...
    CoulombTruncatedUniqueCharge() : qq(0.0)
    {
        setShift(0.0);
        setCutoff(infinity);
    }

    CoulombTruncatedUniqueCharge(real _qq, real _cutoff, real _shift) : qq(_qq)
    {
        setShift(_shift);
        setCutoff(_cutoff);
    }

    CoulombTruncatedUniqueCharge(real _qq, real _cutoff) : qq(_qq)
    {
        autoShift = false;
        setCutoff(_cutoff);
        setAutoShift();
...
{
    static boost::python::tuple getinitargs(CoulombTruncatedUniqueCharge const& pot)
    {
        real q2;
        real rc;
        real sh;
