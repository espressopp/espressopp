...
void LJcos::registerPython()
{
    ...
    class_<LJcos, bases<Potential> >("interaction_LJcos", init<real>())
        .add_property("phi", &LJcos::getPhi, &LJcos::setPhi)
        .add_property("sigma", &LJcos::getSigma, &LJcos::setSigma)
        .def_pickle(LJcos_pickle());

    class_<VerletListLJcos, bases<Interaction> >("interaction_VerletListLJcos",
                                                 init<std::shared_ptr<VerletList> >())
        .def("getVerletList", &VerletListLJcos::getVerletList)
        .def("setPotential", &VerletListLJcos::setPotential,
             return_value_policy<reference_existing_object>())
        .def("getPotential", &VerletListLJcos::getPotential,
             return_value_policy<reference_existing_object>());
