...
namespace espressopp {
  ...
  namespace interaction {
    ...
    */
    class SquareWell: public PotentialTemplate < SquareWell > {
    private:
      real lambda;
      real epsilon;
      real halfepsilon;
    ...
    public:
      ...
      SquareWell(): epsilon(0.0), sigma(0.0), a(0.0), lambda(0.0), halfepsilon(0.0) {
        setShift(0.0);
        setCutoff(infinity);
      }

      SquareWell(real _epsilon, real _sigma, real _cutoff, real _shift): epsilon(_epsilon), sigma(_sigma) {
        halfepsilon = _epsilon/2;
        setShift(_shift);
        setCutoff(_cutoff);
      }

      SquareWell(real _epsilon, real _sigma, real _cutoff): epsilon(_epsilon), sigma(_sigma) {
        halfepsilon = _epsilon/2;
        autoShift = false;
        setCutoff(_cutoff);
        setAutoShift();
    ...
    struct SquareWell_pickle : boost::python::pickle_suite
    {
      static
      boost::python::tuple
      getinitargs(SquareWell const& pot)
      {
        real eps;
        real sig;
        real rc;
