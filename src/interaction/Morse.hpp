...
public:
    ...
    Morse() : epsilon(0.0), alpha(0.0), rMin(0.0)
    {
        setShift(0.0);
        setCutoff(infinity);
    }
    ...
    {
        real r = sqrt(distSqr);
        real energy = epsilon * (exp(-2.0 * alpha * (r - rMin)) - 2.0 * exp(-alpha * (r - rMin)));
        return energy;
    }
    ...
    {
        real r = sqrt(distSqr);
        real ffactor = epsilon *
                       (2.0 * alpha * exp(-2.0 * alpha * (r - rMin)) -
                        2.0 * alpha * exp(-alpha * (r - rMin))) /
                       r;
        force = dist * ffactor;
        return true;
    }
...
{
    static boost::python::tuple getinitargs(Morse const& pot)
    {
        real eps;
        real al;
        eps = pot.getEpsilon();
