...
public:
    ...
    void setLambda2(real _lambda2) { lambda2 = _lambda2; }
    ...
    void setN(real _n) { n = _n; }
    real getN() const { return n; }

    void setBeta(real _beta) { beta = _beta; }
    real getBeta() const { return beta; }

    void setM(real _m) { m = _m; }
    real getM() const { return m; }

    void setLambda3(real _lambda3) { lambda3 = _lambda3; }
    ...
    real getCutoff() const { return std::max(rc1, rc2); }
    void setCutoff1(real _cutoff)
    {
        rc1 = _cutoff;
        preset();
    }
    real getCutoff1() const { return rc1; }
    void setCutoff2(real _cutoff)
    {
        rc2 = _cutoff;
        preset();
    }
    real getCutoff2() const { return rc2; }
                          ...
                          const Real3D& r32) const
    {
        ...
        else
        {
            ...
            Real3D D32_zeta = fC_k * (expTerm * D32_g + g * D32_expTerm) + g * expTerm * D32_fC_k;
            Real3D D12_b =
                -0.5 * pow(b_base, -0.5 / n - 1) * beta * pow(beta * zeta, n - 1) * D12_zeta;
            Real3D D32_b =
                -0.5 * pow(b_base, -0.5 / n - 1) * beta * pow(beta * zeta, n - 1) * D32_zeta;
    ...
    {
        std::cout << "Function _computeForceRaw(teta) doesn't work in TersoffTripleTerm"
                  << std::endl;
        return 0.0;
    }
};
