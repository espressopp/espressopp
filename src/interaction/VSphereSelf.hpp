...
public:
    ...
    VSphereSelf() : e1(0.0), a1(0.0), a2(0.0), Nb(0)
    {
        setShift(0.0);
        setCutoff(infinity);
        preset();
    ...
    VSphereSelf(real _e1, real _a1, real _a2, int _Nb, real _cutoff, real _shift)
    ...
    VSphereSelf(real _e1, real _a1, real _a2, int _Nb, real _cutoff)
        : e1(_e1), a1(_a1), a2(_a2), Nb(_Nb)
    {
        autoShift = false;
        setCutoff(_cutoff);
    ...
    real _computeEnergySqrRaw(real distSqr) const
    {
        ...
        energy = e1 * pow(1.0L * M_4PI3 * sigma2, 1.0L * mth) +
                 a1 * NbNbNb / (sigma2 * sigma2 * sigma2) + a2dNb * sigma2;
        return energy;
    }
    ...
    bool _computeForceRaw(Real3D& force, const Real3D& dist, real distSqr) const
    {
        ...
        force[0] = M_4PI * e1 * pow(1.0L * M_4PI3 * sigma2, 1.0L * mfh) * sigma +
                   a16NbNbNb / (sigma2 * sigma2 * sigma2 * sigma) - a22dNb * sigma;
        return true;
    }
};
}  // namespace interaction
