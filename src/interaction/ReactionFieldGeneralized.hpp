 ...
 */
class ReactionFieldGeneralized : public PotentialTemplate<ReactionFieldGeneralized>
{
private:
    // real qq; // p1.q() * p2.q()
    real prefactor;
    ...
    {
        real krc = kappa * rc;
        real tmp1 = (epsilon1 - 4.0 * epsilon2) * (1.0 + krc) - 2.0 * epsilon2 * krc * krc;
        real tmp2 = (epsilon1 + 2.0 * epsilon2) * (1.0 + krc) + epsilon2 * krc * krc;
        rc3 = pow(rc, 3);
        rc2 = pow(rc, 2);
        B1 = tmp1 / tmp2;
        B1 = (1.0 + B1) / rc3;
...
public:
    ...
    ReactionFieldGeneralized() : prefactor(0.0), kappa(0.0), epsilon1(1.0), epsilon2(80.0), rc(1.0)
    {
        setShift(0.0);
        setCutoff(infinity);
        initialize();
    ...
    ReactionFieldGeneralized(
        real _prefactor, real _kappa, real _eps1, real _eps2, real _cutoff, real _shift)
        : prefactor(_prefactor), kappa(_kappa), epsilon1(_eps1), epsilon2(_eps2), rc(_cutoff)
    {
    ...
    ReactionFieldGeneralized(real _prefactor, real _kappa, real _eps1, real _eps2, real _cutoff)
        : prefactor(_prefactor), kappa(_kappa), epsilon1(_eps1), epsilon2(_eps2), rc(_cutoff)
    {
        autoShift = false;
        setCutoff(_cutoff);
    ...
    bool _computeForce(Real3D& force, const Particle& p1, const Particle& p2) const
    {
        ...
        if (r2 > capradSqr) {
            ...
            //std::cout << "ReactionField, capped Force: " << sqrt(dist.sqr()) * ffactor *
        (caprad/r) << "\n"; 0.1 LEADS TO 2492.93 return true;
        }*/
    }

    real _computeEnergySqrRaw(real distSqr) const
    {
        cout << "_computeEnergySqrRaw not possible for reaction field, no particle information"
             << endl;
        exit(0);
        return 0;
    }
...
{
    static boost::python::tuple getinitargs(ReactionFieldGeneralized const& pot)
    {
