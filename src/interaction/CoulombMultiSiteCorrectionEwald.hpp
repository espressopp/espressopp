...
private:
    ...
    // predefined auxiliary factors for the force calculation, which should be calculated in preset
    // function
    real factor, alpha2;
...
public:
    ...
    bool _computeForce(Real3D& force, const Particle& p1, const Particle& p2) const
    {
        ...
        real forceFactor = prefactor * p1.q() * p2.q() *
                           (factor * exp(-alpha2 * sqr_dist) - erf(alpha * abs_dist) / abs_dist) /
                           sqr_dist;
    ...
    {
        cout << "This function currently doesn't work (_computeEnergySqrRaw(real distSqr) in "
                "CoulombMultiSiteCorrectionEwald.hpp)"
             << endl;
        return 0.0;
    }
    bool _computeForceRaw(Real3D& force, const Real3D& dist, real distSqr) const
    {
        cout << "This function currently doesn't work (_computeForceRaw(Real3D& force, const "
                "Real3D& dist, real distSqr) in CoulombMultiSiteCorrectionEwald.hpp)"
             << endl;
        return false;
    }
};
