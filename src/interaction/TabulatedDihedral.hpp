...
public:
                          ...
                          const Real3D& r43) const
    {
        if (table)
        {
            ...
            // get sign of phi
            // positive if (rij x rjk) x (rjk x rkn) is in the same direction as rjk, negative
            // otherwise (see DLPOLY manual)
            ...
            for (int l = 0; l < 4; l++)
            {
                ...
                {
                    B1[i] =
                        r21[i] * (p3232[i] * (d(l, 2) - d(l, 3)) + p3243[i] * (d(l, 2) - d(l, 1))) +
...
{
    static boost::python::tuple getinitargs(TabulatedDihedral const& pot)
    {
