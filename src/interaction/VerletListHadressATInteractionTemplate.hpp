...
public:
    VerletListHadressATInteractionTemplate(std::shared_ptr<VerletListAdress> _verletList,
...
//////////////////////////////////////////////////
template <typename _Potential>
inline void VerletListHadressATInteractionTemplate<_Potential>::addForces()
{
            ...
            if (it3 != fixedtupleList->end() && it4 != fixedtupleList->end())
            {
                     ...
                     ++itv)
                {
                         ...
                         itv2 != atList2.end(); ++itv2)
                    {
                        ...
                        // Compute AT energies of particles in the hybrid and store and subtract in
                        // map energydiff
                        if (w12 != 1.0)
                        {  // at least one particle in hybrid region => need to do the energy
                           // calculation
                            real energyat = potential._computeEnergy(p3, p4);
                            if (w1 != 1.0)
                            {  // if particle one is in hybrid region
                                energydiff[&p1] -=
                                    energyat;  // subtract AT energy for virtual particle 1
                            }
                            if (w2 != 1.0)
                            {  // if particle two is in hybrid region
                                energydiff[&p2] -=
    ...
    {  // Iterate over all particles
        ...
        {  //   only chose those in the hybrid region
            ...
            if (verletList->getAdrRegionType())
            {
                ...
                // mindriftforce *= (0.5 * energydiff.find(&vp)->second); // get the energy
                // differences which were calculated previously and put in drift force
            ...
            {
                real mindriftforceX =
                    (1.0 / min1sq) * mindriftforce[0];  // normalized driftforce vector
                // mindriftforceX *= (0.5 * energydiff.find(&vp)->second); // get the energy
                // differences which were calculated previously and put in drift force
...
// Energy calculation does currently only work if integrator.run( ) (also with 0) and decompose have
// been executed before. This is due to the initialization of the tuples.
template <typename _Potential>
inline real VerletListHadressATInteractionTemplate<_Potential>::computeEnergy()
{
...
template <typename _Potential>
inline real VerletListHadressATInteractionTemplate<_Potential>::computeEnergyDeriv()
{
    std::cout << "Warning! At the moment computeEnergyDeriv in "
                 "VerletListHadressATInteractionTemplate does not work."
              << std::endl;
    return 0.0;
}

template <typename _Potential>
inline real VerletListHadressATInteractionTemplate<_Potential>::computeEnergyAA()
{
    std::cout << "Warning! At the moment computeEnergyAA in VerletListHadressATInteractionTemplate "
                 "does not work."
              << std::endl;
    return 0.0;
}
...
template <typename _Potential>
inline real VerletListHadressATInteractionTemplate<_Potential>::computeEnergyAA(int atomtype)
{
...
template <typename _Potential>
inline real VerletListHadressATInteractionTemplate<_Potential>::computeEnergyCG()
{
    std::cout << "Warning! At the moment computeEnergyCG in VerletListHadressATInteractionTemplate "
                 "does not work."
              << std::endl;
    return 0.0;
}
...
template <typename _Potential>
inline real VerletListHadressATInteractionTemplate<_Potential>::computeEnergyCG(int atomtype)
{
...
template <typename _Potential>
inline void VerletListHadressATInteractionTemplate<_Potential>::computeVirialX(
    std::vector<real>& p_xx_total, int bins)
{
    std::cout << "Warning! At the moment computeVirialX in VerletListHadressATInteractionTemplate "
                 "does'n work"
              << std::endl;
}

template <typename _Potential>
inline real VerletListHadressATInteractionTemplate<_Potential>::computeVirial()
{
    LOG4ESPP_INFO(theLogger, "compute the virial for the Verlet List");
    std::cout << "Warning! At the moment computeVirial in VerletListHadressATInteractionTemplate "
                 "does not work."
              << std::endl;
    return 0.0;
}
...
template <typename _Potential>
inline void VerletListHadressATInteractionTemplate<_Potential>::computeVirialTensor(Tensor& w)
{
    ...
    std::cout << "Warning! At the moment computeVirialTensor in "
                 "VerletListHadressATInteractionTemplate does'n work"
              << std::endl;
}
...
template <typename _Potential>
inline real VerletListHadressATInteractionTemplate<_Potential>::getMaxCutoff()
{
    real cutoff = 0.0;
