...
public:
    ...
    std::shared_ptr<FixedTupleListAdress> getFixedTupleList() { return fixedtupleList; }

    void setNTrotter(int _ntrotter) { ntrotter = _ntrotter; }

    int getNTrotter() { return ntrotter; }

    void setSpeedup(bool _speedup) { speedup = _speedup; }
...
//////////////////////////////////////////////////
template <typename _Potential>
inline void VerletListPIadressNoDriftInteractionTemplate<_Potential>::addForces()
{
        ...
        else
        {
            ...
            if (it3 != fixedtupleList->end() && it4 != fixedtupleList->end())
            {
                     ...
                     ++itv)
                {
                    ...
                    {
                        std::stringstream ss;
                        ss << "Tuplelists seem to have different lengths or are not started "
                              "properly. Corresponding CG particles "
                           << p1.id() << " and " << p2.id();
                    ...
                    {
                        std::stringstream ss;
                        ss << "Path Integral bead numbers do not match in "
                              "VerletListPIadressInteractionTemplate for particles "
                           << p3.id() << " and " << p4.id();
                        throw std::runtime_error(ss.str());
                    }
            ...
            {
                std::stringstream ss;
                ss << "One of the VP particles not found in tuples. Particle 1 - ID: " << p1.id()
                   << " Ghost: " << p1.ghost() << " Position: " << p1.position()
                   << " Particle 2 - ID: " << p2.id() << " Ghost: " << p2.ghost()
    ...
    for (PairList::Iterator it(verletList->getAdrPairs()); it.isValid(); ++it)
    {
        ...
        if (it3 != fixedtupleList->end() && it4 != fixedtupleList->end())
        {
                 ...
                 ++itv)
            {
                ...
                {
                    std::stringstream ss;
                    ss << "Tuplelists seem to have different lengths or are not started properly. "
                          "Corresponding CG particles "
                       << p1.id() << " and " << p2.id();
                    throw std::runtime_error(ss.str());
                }
...
template <typename _Potential>
inline real VerletListPIadressNoDriftInteractionTemplate<_Potential>::computeEnergy()
{
    // Pairs not inside the QM/Hybrid Zone (i.e. CL region)
    real e = 0.0;
    for (PairList::Iterator it(verletList->getPairs()); it.isValid(); ++it)
...
template <typename _Potential>
inline real VerletListPIadressNoDriftInteractionTemplate<_Potential>::computeEnergyDeriv()
{
    std::cout << "Warning! At the moment computeEnergyDeriv() in "
                 "VerletListPIadressNoDriftInteractionTemplate does not work."
              << std::endl;
    return 0.0;
}

template <typename _Potential>
inline real VerletListPIadressNoDriftInteractionTemplate<_Potential>::computeEnergyAA()
{
    real e = 0.0;
...
template <typename _Potential>
inline real VerletListPIadressNoDriftInteractionTemplate<_Potential>::computeEnergyAA(int atomtype)
{
    real e = 0.0;
...
template <typename _Potential>
inline real VerletListPIadressNoDriftInteractionTemplate<_Potential>::computeEnergyCG()
{
    real e = 0.0;
...
template <typename _Potential>
inline real VerletListPIadressNoDriftInteractionTemplate<_Potential>::computeEnergyCG(int atomtype)
{
    real e = 0.0;
...
template <typename _Potential>
inline void VerletListPIadressNoDriftInteractionTemplate<_Potential>::computeVirialX(
    std::vector<real>& p_xx_total, int bins)
{
    std::cout << "Warning! At the moment computeVirialX in VerletListPIadressNoDrift does not work"
              << std::endl;
    return;
}

template <typename _Potential>
inline real VerletListPIadressNoDriftInteractionTemplate<_Potential>::computeVirial()
{
    real w = 0.0;
    ...
    for (PairList::Iterator it(verletList->getPairs()); it.isValid(); ++it)
    {
        ...
        if (speedup == true)
        {
            ...
            if (potential._computeForce(forcecl, p1, p2))
            {
                ...
                verletList->getSystem()->bc->getMinimumImageVector(dist, p1.position(),
                                                                   p2.position());
                w += dist * forcecl;
            }
        }
            ...
            if (it3 != fixedtupleList->end() && it4 != fixedtupleList->end())
            {
                     ...
                     ++itv)
                {
                    ...
                    if (potential._computeForce(forcecl, p3, p4))
                    {
                        forcecl *= 1.0 / ntrotter;
                        Real3D dist(0.0, 0.0, 0.0);
                        verletList->getSystem()->bc->getMinimumImageVector(dist, p3.position(),
                                                                           p4.position());
                        w += dist * forcecl;
                    }
...
template <typename _Potential>
inline void VerletListPIadressNoDriftInteractionTemplate<_Potential>::computeVirialTensor(Tensor& w)
{
    std::cout << "Warning! At the moment IK computeVirialTensor in VerletListPIadressNoDrift does "
                 "not work"
              << std::endl;
    return;
}
...
template <typename _Potential>
inline real VerletListPIadressNoDriftInteractionTemplate<_Potential>::getMaxCutoff()
{
    real cutoff = 0.0;
