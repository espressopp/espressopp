...
public:
    ...
    void preset()
    {
        ...
        map_indx =
            vector<vector<vector<int> > >(M[0], vector<vector<int> >(M[1], vector<int>(M[2], 0)));
        for (int i = 0; i < M[0]; i++)
        {
            int mi = M[1] * i;
            for (int j = 0; j < M[1]; j++)
            ...
            for (int i = 0; i < P; i++)
            {
                ...
                for (int j = 0; j < P; j++)
                {
                    ...
                    for (int k = 0; k < P; k++)
                    {
                        ...
                        Real3D f_add(phi[0][indx].real(), phi[1][indx].real(), phi[2][indx].real());
    ...
    dcomplex swap_complex(dcomplex C) { return dcomplex(C.imag(), C.real()); }
    ...
    real asignment_f1(real x, int k, int P)
    {
        ...
        if (P < 1 || P > 7)
        {
            msg << "Charge assignment order P=" << P << "  For some reason order " << k
                << " is used";
            err.setException(msg.str());
        }
        err.checkException();
        ...
        switch (P)
        {
            ...
            case 2:
            {
                switch (i)
                {
                    ...
                    default:
                        fprintf(stderr,
                                "%d: Tried to access charge assignment "
            ...
            case 3:
            {
                switch (i)
                {
                    ...
                    case 1:
                        return 0.75 - pow(x, 2);
                    case 2:
                        return 0.5 * pow(0.5 + x, 2);
                    default:
                        fprintf(stderr,
                                "%d: Tried to access charge assignment function"
