...
StochasticVelocityRescaling::StochasticVelocityRescaling(std::shared_ptr<System> system)
    : Extension(system)
{
    temperature = 0.0;
    ...
    // TODO benchmark those distributions! a first dirty benchmark did not show any significant
...
void StochasticVelocityRescaling::connect()
{
    ...
    _runInit =
        integrator->runInit.connect(std::bind(&StochasticVelocityRescaling::initialize, this));
    // connection to the signal at the end of the run
    _aftIntV = integrator->aftIntV.connect(
        std::bind(&StochasticVelocityRescaling::rescaleVelocities, this));
}
...
void StochasticVelocityRescaling::initialize()
{
    ...
    DegreesOfFreedom =
        3.0 * NPart;  // TODO this is _only_ true for simple system without any constraints
...
void StochasticVelocityRescaling::rescaleVelocities()
{
    ...
    for (CellListIterator cit(realCells); !cit.isDone(); ++cit)
    {
        ...
        EKin_local += 0.5 * cit->mass() *
                      (vel * vel);  // FIXME do not forget that his is only correct for
                                    // velocity-verlet; leap-frog would require adjustments
    }
    ...
    if (getSystem()->comm->rank() == 0)
    {
        ...
        if (EKin_new <= 0)
            throw std::runtime_error(
                "EKin_new in StochasticVelocityRescaling::rescaleVelocities() is equal or smaller "
                "than 0");
    ...
    real Ekin, real Ekin_ref, int dof, real taut, std::shared_ptr<esutil::RNG> rng)
{
    ...
    if (taut < 0.1)
        throw std::runtime_error(
            "taut in stochasticVR_pullEkin is very low."
    ...
    return Ekin +
           (1.0 - factor) *
               (Ekin_ref * (stochasticVR_sumGaussians(dof - 1) + rr * rr) / dof - Ekin) +
...
real GammaDistributionNR2nd::drawNumber(const unsigned int ia)
{
    ...
    if (ia < 1)
        throw std::runtime_error(
            "Error in routine stochasticVR_gammaDeviate2nd");  // TODO: how do I get __FUNCTION__ in
                                                               // here?
    if (ia < 6)
    {
        x = 1.0;
        for (int j = 0; j <= int_c(ia); j++) x = x * (*rng)();
        x = -log(x);
    }
    else
    {
...
void StochasticVelocityRescaling::registerPython()
{
    ...
    class_<StochasticVelocityRescaling, std::shared_ptr<StochasticVelocityRescaling>,
           bases<Extension> >
