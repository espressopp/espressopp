...
StochasticVelocityRescaling::StochasticVelocityRescaling(shared_ptr<System> system)
  ...
  // chose one of currently three implementations of a gamma-distributed random
  // number
  gammaDist = new GammaDistributionBoost(rng);
  // gammaDist = new GammaDistributionNR2nd(rng);
  // gammaDist = new GammaDistributionNR3rd(rng);
  // TODO benchmark those distributions! a first dirty benchmark did not show
  // any significant difference in computational time
  ...
  if (getSystem()->comm->rank() == 0) {
    ...
    if (EKin_new <= 0)
      throw std::runtime_error(
          "EKin_new in StochasticVelocityRescaling::rescaleVelocities() is "
...
real GammaDistributionNR2nd::drawNumber(const unsigned int ia) {
  ...
  if (ia < 1)
    throw std::runtime_error(
        "Error in routine stochasticVR_gammaDeviate2nd");  // TODO: how do I get
                                                           // __FUNCTION__ in
                                                           // here?
  if (ia < 6) {
    x = 1.0;
    for (int j = 0; j <= ia; j++) x = x * (*rng)();
    x = -log(x);
  } else {
    do {
      do {
