...
real LangevinBarostat::getGammaP() { return gammaP; }

void LangevinBarostat::setPressure(real _press) { externalPressure = _press; }
real LangevinBarostat::getPressure() { return externalPressure; }

void LangevinBarostat::setMass(real _mass) { mass = _mass; }
real LangevinBarostat::getMass() { return mass; }
...
void LangevinBarostat::updVolume() {
  ...
  // The volume is scaled according to the equations V(t+1/2*dt) = V(t) +
  // 1/2*dt*V';
  // V' = d*V*pe/W
...
/*
 *  TODO now it is valid for nonbonded systems because the value of degreees of
 * freedom
 *  Nf = 3*N, N - number of particles, 3 - d-dimensional system (d=3). Thus d/Nf
 * is
 *  replaced by 1/N.
 */
void LangevinBarostat::updVolumeMomentum() {
  ...
  // it's not efficient to use the analysis.Pressure because of double
  // calculation of m*v*v
  real v2sum;
  real v2 = 0.0;
  ...
  // TODO optimization is needed, some terms are the same at the begin and at
  // the end of integration
  real X = p_kinetic + p_nonbonded;
  /*
   * in order not to do double calculations X is not divided by 3V and term in
   * the next line
...
void LangevinBarostat::initialize() {
  ...
  /* Nf - degrees of freedom. For N particles in d-dimentional system without
   * constrains
   * Nf = d * N
   */
  ...
  // uniform distribution prefactor. (it can be used instead of normal
  // distribution)
