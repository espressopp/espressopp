...
LangevinBarostat::LangevinBarostat(std::shared_ptr<System> _system,
                                   ...
                                   real _temperature)
    : Extension(_system), desiredTemperature(_temperature), rng(_rng)
{
    // external parameters
    gammaP = 0.0;
    mass = 0.0;
...
void LangevinBarostat::connect()
{
    ...
    _inIntP = integrator->inIntP.connect(
        std::bind(&LangevinBarostat::updDisplacement, this, std::placeholders::_1));
...
void LangevinBarostat::updVolumeMomentum()
{
    ...
    // TODO optimization is needed, some terms are the same at the begin and at the end of
    // integration
     ...
     */
    real pe_deriv =
        (X - m3V * externalPressure) + pref6 * v2sum + pref4 * momentum + pref5 * rannum;
...
void LangevinBarostat::registerPython()
{
    ...
    class_<LangevinBarostat, std::shared_ptr<LangevinBarostat>, bases<Extension> >
        ...
        ("integrator_LangevinBarostat",
         init<std::shared_ptr<System>, std::shared_ptr<esutil::RNG>, real>())
