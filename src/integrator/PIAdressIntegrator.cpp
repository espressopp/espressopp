...
PIAdressIntegrator::PIAdressIntegrator(std::shared_ptr<System> system,
...
void PIAdressIntegrator::run(int nsteps)
{
    ...
    if (PILE == true && realkinmass == false)
    {
        throw std::invalid_argument(
            "Using the path integral Langevin scheme (PILE) only makes sense when using real "
...
void PIAdressIntegrator::initializeSetup()
{
    ...
    for (CellListIterator cit(localCells); !cit.isDone(); ++cit)
    {
        ...
        if (it3 != fixedtupleList->end())
        {
            ...
            for (std::vector<Particle *>::iterator it2 = it3->second.begin();
                 it2 != it3->second.end(); ++it2)
            {
                Particle &at = **it2;
                cmp += at.position();
                cmv += at.velocity();
            ...
            {  // kinetic mass also changes - hence, second decomposition level required
                ...
                {  // do not propagate the higher modes in the CL region when using speedup
                         ...
                         it2 != it3->second.end(); ++it2)
                    {
                        ...
                        {
                            throw std::runtime_error(
                                "at.pib() outside of trotter range in integrateV2 function "
                                "(PIAdressIntegrator)!");
                        }
                    }
                }
                     ...
                     it2 != it3->second.end(); ++it2)
                {
                    ...
                    {  // only centroid
                        ...
                        if (vp.lambda() < 1.0 && vp.lambda() > 0.0)
                        {
                            ...
                            real xi = 0.0;
                            for (std::vector<Particle *>::iterator it5 = it3->second.begin();
                                 it5 != it3->second.end(); ++it5)
                            {
                                Particle &at2 = **it5;
                                if (at2.pib() != 1)
                                {
                            ...
                            verletList->getSystem()->bc->getMinimumImageVector(
                                mindriftforce, (1.0 / sqrt(ntrotter)) * at.modepos(), pa);
                            real min1sq = 0.0;
                            if (verletList->getAdrRegionType())
                            {
                            ...
                            for (; it2 != verletList->getAdrPositions().end(); ++it2)
                            {
                                ...
                                verletList->getSystem()->bc->getMinimumImageVector(
                                    driftforce, (1.0 / sqrt(ntrotter)) * at.modepos(), pa);
                                real distsq1 = 0.0;
                                if (verletList->getAdrRegionType())
                                {
                            ...
                            if (verletList->getAdrRegionType())
                            {
                                ...
                                mindriftforce *= 0.5 * (clmassmultiplier - 1.0) * vp.mass() *
                                                 sqrt(ntrotter) * xi * vp.lambdaDeriv() /
                                                 (vp.varmass() * vp.varmass() * CMDparameter);
                                at.modemom() += half_dt * (at.forcem() - mindriftforce);
                            }
                            else
                            {
                                real mindriftforceX = (1.0 / min1sq) * mindriftforce[0];
                                mindriftforceX *= 0.5 * (clmassmultiplier - 1.0) * vp.mass() *
                                                  sqrt(ntrotter) * xi * vp.lambdaDeriv() /
                                                  (vp.varmass() * vp.varmass() * CMDparameter);
                     ...
                     it2 != it3->second.end(); ++it2)
                {
                    ...
                    {  // only centroid
                        ...
                        if (KTI == false)
                        {
                            ...
                            real min1sq;
                            verletList->getSystem()->bc->getMinimumImageVector(
                                d1, (1.0 / sqrt(ntrotter)) * at.modepos(), pa);
                            if (verletList->getAdrRegionType())
                            {  // spherical adress region
                                min1sq = d1.sqr();
                                ++it2;
                            ...
                            vp.lambdaDeriv() = wDeriv;
                            vp.varmass() =
                                vp.mass() * (w * (1.0 - clmassmultiplier) + clmassmultiplier);
                        }
                     ...
                     it2 != it3->second.end(); ++it2)
                {
                    ...
                    else if (at.pib() > 1 && int_c(at.pib()) <= ntrotter)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                at.modepos() += half_dt * at.modemom() /
                                                (CMDparameter * Eigenvalues[at.pib() - 1]);
                            }
                            else
                            {
                    ...
                    {
                        throw std::runtime_error(
                            "at.pib() outside of trotter range in integrateModePos function "
                            "(PIAdressIntegrator)!");
                    }
                }
            }
...
void PIAdressIntegrator::OUintegrate()
{  // this is the Ornstein-Uhlenbeck process, this is, the Langevin thermostat
    ...
    for (CellListIterator cit(realCells); !cit.isDone(); ++cit)
    {
        ...
        if (it3 != fixedtupleList->end())
        {
                 ...
                 it2 != it3->second.end(); ++it2)
            {
                    ...
                    if ((speedup == false) || (vp.lambda() > 0.000000001))
                    {
                        ...
                        if (PILE == false)
                        {
                            if (constkinmass == false)
                            {
                                if (realkinmass == false)
                                {
                                    at.modemom() = prefac1 * at.modemom() +
                                                   (prefac2 * sqrt(CMDparameter * vp.varmass() *
                                                                   Eigenvalues[at.pib() - 1])) *
                                                       ranval;
                                }
                                else
                                {
                                    at.modemom() =
                                        prefac1 * at.modemom() +
                                        (prefac2 * sqrt(CMDparameter * vp.varmass())) * ranval;
                                }
                            }
                            else
                            {
                                ...
                                {
                                    at.modemom() = prefac1 * at.modemom() +
                                                   (prefac2 * sqrt(CMDparameter * vp.mass() *
                                                                   Eigenvalues[at.pib() - 1])) *
                                                       ranval;
                                }
                                else
                                {
                                    at.modemom() =
                                        prefac1 * at.modemom() +
                                        (prefac2 * sqrt(CMDparameter * vp.mass())) * ranval;
                                }
                            }
                        }
                        else
                        {
                            real modegamma = 2.0 * PILElambda * sqrt(omega2 * ntrotter) *
                                             sqrt(Eigenvalues[at.pib() - 1]);
                            real prefac1_PILE = exp(-modegamma * dt);
                            real prefac2_PILE = sqrt(12.0 * temperature *
                                                     (1.0 - exp(-2.0 * modegamma * dt)) / ntrotter);
                            if (constkinmass == false)
                            {
                                at.modemom() =
                                    prefac1_PILE * at.modemom() +
                                    (prefac2_PILE * sqrt(CMDparameter * vp.varmass())) * ranval;
                            }
                            else
                            {
                                at.modemom() =
                                    prefac1_PILE * at.modemom() +
                                    (prefac2_PILE * sqrt(CMDparameter * vp.mass())) * ranval;
                            }
                        }
                    }
                }
                ...
                {
                    throw std::runtime_error(
                        "at.pib() outside of trotter range in OUintegrate function "
                        "(PIAdressIntegrator)!");
                }
            }
        }
...
{  // Update the real positions from mode positions
    ...
    for (CellListIterator cit(localCells); !cit.isDone(); ++cit)
    {
        ...
        if (it2 != fixedtupleList->end())
        {
            ...
            for (std::vector<Particle *>::iterator it3 = it2->second.begin();
                 it3 != it2->second.end(); ++it3)
            {
                ...
                for (std::vector<Particle *>::iterator it5 = it2->second.begin();
                     it5 != it2->second.end(); ++it5)
                {
                    Particle &at2 = **it5;
                    ...
                    {
                        throw std::runtime_error(
                            "at.pib() outside of trotter range in transPos1 function "
                            "(PIAdressIntegrator)!");
                    }
                }
            }
        ...
        if (it2 != fixedtupleList->end())
        {
                 ...
                 it3 != it2->second.end(); ++it3)
            {
                     ...
                     it5 != it2->second.end(); ++it5)
                {
                    ...
                    {
                        throw std::runtime_error(
                            "at.pib() outside of trotter range in transPos2 function "
                            "(PIAdressIntegrator)!");
                    }
                }
            }
...
void PIAdressIntegrator::transMom1()
                    ...
                    if (int_c(at.pib()) <= ntrotter)
                    {
                        if (int_c(at2.pib()) > 1 && int_c(at2.pib()) <= ntrotter)
                        {
                            if (constkinmass == false)
                            {
                                if (realkinmass == false)
                                {
                                    at.velocity() += at2.modemom() *
                                                     Tvectors[at.pib() - 1][at2.pib() - 1] *
                                                     ntrotter /
                                                     ((Eigenvalues[at2.pib() - 1]) * CMDparameter *
                                                      vp.varmass());
                                }
                                else
                                {
                                    at.velocity() += at2.modemom() *
                                                     Tvectors[at.pib() - 1][at2.pib() - 1] *
                                                     ntrotter / (CMDparameter * vp.varmass());
                                }
                            }
                            else
                            {
                                ...
                                {
                                    at.velocity() +=
                                        at2.modemom() * Tvectors[at.pib() - 1][at2.pib() - 1] *
                                        ntrotter /
                                        ((Eigenvalues[at2.pib() - 1]) * CMDparameter * vp.mass());
                                }
                                else
                                {
                                    at.velocity() += at2.modemom() *
                                                     Tvectors[at.pib() - 1][at2.pib() - 1] *
                                                     ntrotter / (CMDparameter * vp.mass());
                                }
                            }
                        }
                        else if (at2.pib() == 1)
                        {
                            at.velocity() += at2.modemom() * Tvectors[at.pib() - 1][at2.pib() - 1] *
                                             ntrotter / (vp.mass());
                        }
                        else
                        {
                            throw std::runtime_error(
                                "at.pib() outside of trotter range in transMom1 function "
                                "(PIAdressIntegrator)!");
                        }
                        if ((at2.pib() == 1) && (at.pib() == 1))
                        {
        ...
        if (it2 != fixedtupleList->end())
        {
                 ...
                 it3 != it2->second.end(); ++it3)
            {
                     ...
                     it5 != it2->second.end(); ++it5)
                {
                    ...
                    {
                        throw std::runtime_error(
                            "at.pib() outside of trotter range in transMom2 function "
                            "(PIAdressIntegrator)!");
                    }
                }
                ...
                if (at.pib() > 1 && int_c(at.pib()) <= ntrotter)
                {
                    ...
                    else
                    {
                        ...
                        {
                            at.modemom() *=
                                (vp.mass() * CMDparameter * (Eigenvalues[at.pib() - 1]) / ntrotter);
                        }
                        else
                        {
        ...
        if (it2 != fixedtupleList->end())
        {
                 ...
                 it3 != it2->second.end(); ++it3)
            {
                     ...
                     it5 != it2->second.end(); ++it5)
                {
                    ...
                    {
                        throw std::runtime_error(
                            "at.pib() outside of trotter range in transForces function "
                            "(PIAdressIntegrator)!");
                    }
                }
            }
...
{  // Calculate fast forces, this is, the intra-ring forces between the path integral beads (purely
    ...
    for (CellListIterator cit(localCells); !cit.isDone(); ++cit)
    {
        ...
        if (speedup == true)
        {
            ...
            {  // In speedup mode we do not integrate the higher modes in the CL region
                continue;
            }
            else
            {
                if (it2 != fixedtupleList->end())
                {
                         ...
                         it3 != it2->second.end(); ++it3)
                    {
                        ...
                        {
                            at.forcem() -=
                                at.modepos() * omega2 * vp.varmass() * Eigenvalues[at.pib() - 1];
                        }
                        else if (at.pib() == 1)
                        {
                            // If necessary, calculate drift force
                            if (vp.lambda() < 1.0 && vp.lambda() > 0.0)
                            {
                                ...
                                {
                                    mindriftforce = (1.0 / min1sq) * mindriftforce;
                                    mindriftforce *= 0.5 * xi * (clmassmultiplier - 1.0) *
                                                     (1.0 / sqrt(ntrotter)) * vp.lambdaDeriv() *
                                                     vp.mass() * omega2;
                                ...
                                {
                                    real mindriftforceX = (1.0 / min1sq) * mindriftforce[0];
                                    mindriftforceX *= 0.5 * xi * (clmassmultiplier - 1.0) *
                                                      (1.0 / sqrt(ntrotter)) * vp.lambdaDeriv() *
                                                      vp.mass() * omega2;
                                    Real3D driftforceadd(mindriftforceX, 0.0, 0.0);
                        ...
                        {
                            throw std::runtime_error(
                                "at.pib() outside of trotter range in calcForcesF function "
                                "(PIAdressIntegrator)!");
                        }
                    }
                }
...
void PIAdressIntegrator::updateForces(int f)
{
    ...
    {
        throw std::runtime_error(
            "updateForces routine in PIAdressIntegrator received wrong integer.");
    }

    if (f == 2 || f == 3)
    {  // Not necessary for internal ring forces
...
void PIAdressIntegrator::initForces(int f)
{
    ...
    {
        throw std::runtime_error(
            "initForces routine in PIAdressIntegrator received wrong integer.");
    }
}
    ...
    for (CellListIterator cit(realCells); !cit.isDone(); ++cit)
    {
        ...
        if (it3 != fixedtupleList->end())
        {
                 ...
                 it2 != it3->second.end(); ++it2)
            {
                ...
                else if (at.pib() > 1 && int_c(at.pib()) <= ntrotter)
                {
                    ...
                    else
                    {
                        if (constkinmass == false)
                        {
                            if (realkinmass == false)
                            {
                                esum += at.modemom().sqr() * ntrotter /
                                        (vp.varmass() * CMDparameter * Eigenvalues[at.pib() - 1]);
                            }
                            else
                            {
                                esum +=
                                    at.modemom().sqr() * ntrotter / (vp.varmass() * CMDparameter);
                            }
                        }
                        else
                        {
                            ...
                            {
                                esum += at.modemom().sqr() * ntrotter /
                                        (vp.mass() * CMDparameter * Eigenvalues[at.pib() - 1]);
                            }
                            else
                            {
                ...
                {
                    throw std::runtime_error(
                        "at.pib() outside of trotter range in computeKineticEnergy routine "
                        "(PIAdressIntegrator).");
                }
            }
        }
    ...
    for (CellListIterator cit(realCells); !cit.isDone(); ++cit)
    {
        ...
        if (it3 != fixedtupleList->end())
        {
                 ...
                 it2 != it3->second.end(); ++it2)
            {
                ...
                {
                    esum +=
                        at.modepos().sqr() * omega2 * (vp.varmass() * Eigenvalues[at.pib() - 1]);
                }
                else
                {
                    throw std::runtime_error(
                        "at.pib() outside of trotter range in computeRingEnergy routine "
                        "(PIAdressIntegrator).");
                }
            }
        }
    ...
    for (CellListIterator cit(realCells); !cit.isDone(); ++cit)
    {
        ...
        if (it3 != fixedtupleList->end())
        {
                 ...
                 it2 != it3->second.end(); ++it2)
            {
                ...
                {
                    throw std::runtime_error(
                        "at.pib() outside of trotter range in computeRingEnergyRaw routine "
                        "(PIAdressIntegrator).");
                }
            }
        }
            ...
            if (it3 != fixedtupleList->end())
            {
                     ...
                     it2 != it3->second.end(); ++it2)
                {
                    ...
                    else if (at.pib() > 1 && int_c(at.pib()) <= ntrotter)
                    {
                        if (realkinmass == false)
                        {
                            esum += (clmassmultiplier - 1.0) * 0.5 * vp.mass() * ntrotter *
                                    at.modemom().sqr() /
                                    (vp.varmass() * vp.varmass() * CMDparameter *
                                     Eigenvalues[at.pib() - 1]);
                        }
                    ...
                    {
                        throw std::runtime_error(
                            "at.pib() outside of trotter range in computeMomentumDrift routine "
                            "(PIAdressIntegrator).");
                    }
                }
            }
        ...
        if (int_c(vp.type()) == parttype)
        {
            ...
            if (it3 != fixedtupleList->end())
            {
                     ...
                     it2 != it3->second.end(); ++it2)
                {
                    ...
                    {
                        esum -= (clmassmultiplier - 1.0) * 0.5 * vp.mass() * omega2 *
                                at.modepos().sqr() * Eigenvalues[at.pib() - 1];
                    }
                    else
                    {
                        throw std::runtime_error(
                            "at.pib() outside of trotter range in computePositionDrift routine "
                            "(PIAdressIntegrator).");
                    }
                }
            }
...
void PIAdressIntegrator::setNtrotter(int _ntrotter) { ntrotter = _ntrotter; }

void PIAdressIntegrator::setSpeedup(bool _speedup) { speedup = _speedup; }

void PIAdressIntegrator::setKTI(bool _KTI) { KTI = _KTI; }

void PIAdressIntegrator::setPILE(bool _PILE) { PILE = _PILE; }

void PIAdressIntegrator::setRealKinMass(bool _realkinmass) { realkinmass = _realkinmass; }
...
void PIAdressIntegrator::setConstKinMass(bool _constkinmass) { constkinmass = _constkinmass; }
...
void PIAdressIntegrator::setOmegaSquared()
{
    ...
    real Nav = 6.02214129;
    real kb = 1.3806488;
    omega2 = ntrotter * temperature * temperature * kb * kb * Nav * 1.66053892 /
             (hbar * hbar * 0.00831451 * 0.00831451 * 1000.0);
}

void PIAdressIntegrator::setGamma(real _gamma) { gamma = _gamma; }

void PIAdressIntegrator::setCMDparameter(real _CMDparameter) { CMDparameter = _CMDparameter; }

void PIAdressIntegrator::setPILElambda(real _PILElambda) { PILElambda = _PILElambda; }
...
void PIAdressIntegrator::transp()
{  // We need both the regular and the transposed normal mode transformation matrix
...
void PIAdressIntegrator::registerPython()
{
    ...
    class_<PIAdressIntegrator, bases<MDIntegrator>, boost::noncopyable>(
        ...
        init<std::shared_ptr<System>, std::shared_ptr<VerletListAdress> >())
        ...
        .add_property("timestep", &PIAdressIntegrator::getTimeStep,
                      &PIAdressIntegrator::setTimeStep)
        ...
        .add_property("nTrotter", &PIAdressIntegrator::getNtrotter,
                      &PIAdressIntegrator::setNtrotter)
        .add_property("temperature", &PIAdressIntegrator::getTemperature,
                      &PIAdressIntegrator::setTemperature)
        .add_property("gamma", &PIAdressIntegrator::getGamma, &PIAdressIntegrator::setGamma)
        .add_property("CMDparameter", &PIAdressIntegrator::getCMDparameter,
                      &PIAdressIntegrator::setCMDparameter)
        .add_property("CLmassmultiplier", &PIAdressIntegrator::getClmassmultiplier,
                      &PIAdressIntegrator::setClmassmultiplier)
        .add_property("PILElambda", &PIAdressIntegrator::getPILElambda,
                      &PIAdressIntegrator::setPILElambda)
        ...
        .add_property("centroidThermostat", &PIAdressIntegrator::getCentroidThermostat,
                      &PIAdressIntegrator::setCentroidThermostat)
        .add_property("PILE", &PIAdressIntegrator::getPILE, &PIAdressIntegrator::setPILE)
        .add_property("realKinMass", &PIAdressIntegrator::getRealKinMass,
                      &PIAdressIntegrator::setRealKinMass)
        .add_property("constKinMass", &PIAdressIntegrator::getConstKinMass,
                      &PIAdressIntegrator::setConstKinMass)
        .add_property("verletList", &PIAdressIntegrator::getVerletList,
                      &PIAdressIntegrator::setVerletList)
