...
BerendsenBarostatAnisotropic::BerendsenBarostatAnisotropic(std::shared_ptr<System> system)
    : Extension(system)
{
    tau = 1.0;
    P0 = Real3D(1.0);
...
void BerendsenBarostatAnisotropic::connect()
{
    ...
    _runInit =
        integrator->runInit.connect(std::bind(&BerendsenBarostatAnisotropic::initialize, this));

    // connection to the signal at the end of the run
    _aftIntV =
        integrator->aftIntV.connect(std::bind(&BerendsenBarostatAnisotropic::barostat, this));
}

// set and get time constant for Berendsen barostat
void BerendsenBarostatAnisotropic::setTau(real _tau) { tau = _tau; }
real BerendsenBarostatAnisotropic::getTau() { return tau; }
// set and get external pressure
void BerendsenBarostatAnisotropic::setPressure(Real3D _P0) { P0 = _P0; }
Real3D BerendsenBarostatAnisotropic::getPressure() { return P0; }
...
void BerendsenBarostatAnisotropic::barostat()
{
    ...
    Real3D P = Real3D(
        Ptensor[0], Ptensor[1],
        Ptensor[2]);  // take the diagonal elements, xx, yy, zz from the ordered pressure tensor,
                      // ignore xy etc since only orthorhombic cells are considered so far //FIXME

    Real3D mu3 =
        Real3D(1) + pref * (P - P0) / 3.0;  // calculating the current scaling parameter // this is
                                            // the tensorial form in Berendsen's paper from 1984
...
void BerendsenBarostatAnisotropic::registerPython()
{
    ...
    class_<BerendsenBarostatAnisotropic, std::shared_ptr<BerendsenBarostatAnisotropic>,
           bases<Extension> >

        ("integrator_BerendsenBarostatAnisotropic", init<std::shared_ptr<System> >())

            .add_property("tau", &BerendsenBarostatAnisotropic::getTau,
                          &BerendsenBarostatAnisotropic::setTau)
            .add_property("pressure", &BerendsenBarostatAnisotropic::getPressure,
