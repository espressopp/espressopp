...
LangevinThermostat1D::LangevinThermostat1D(shared_ptr<System> system) : Extension(system) {
...
void LangevinThermostat1D::setGamma(real _gamma) { gamma = _gamma; }

real LangevinThermostat1D::getGamma() { return gamma; }

void LangevinThermostat1D::setAdress(bool _adress) { adress = _adress; }

bool LangevinThermostat1D::getAdress() { return adress; }

void LangevinThermostat1D::setTemperature(real _temperature) { temperature = _temperature; }

real LangevinThermostat1D::getTemperature() { return temperature; }

void LangevinThermostat1D::setDirection(int _direction) { direction = _direction; }

int LangevinThermostat1D::getDirection() { return direction; }

LangevinThermostat1D::~LangevinThermostat1D() { disconnect(); }
...
void LangevinThermostat1D::connect() {
  ...
  _initialize = integrator->runInit.connect(boost::bind(&LangevinThermostat1D::initialize, this));

  _heatUp = integrator->recalc1.connect(boost::bind(&LangevinThermostat1D::heatUp, this));

  _coolDown = integrator->recalc2.connect(boost::bind(&LangevinThermostat1D::coolDown, this));

  if (adress) {
    _thermalizeAdr =
        integrator->aftCalcF.connect(boost::bind(&LangevinThermostat1D::thermalizeAdr, this));
  } else {
    _thermalize =
        integrator->aftCalcF.connect(boost::bind(&LangevinThermostat1D::thermalize, this));
  }
}
...
void LangevinThermostat1D::thermalizeAdr() {
  ...
  // TODO: It doesn't make that much sense to thermalize both CG and AT
  // particles, since CG particles get velocities of AT particles anyway.
...
void LangevinThermostat1D::frictionThermo(Particle& p) {
  ...
  p.force()[direction] += pref1 * p.velocity()[direction] * p.mass() + pref2 * ranval * massf;

  LOG4ESPP_TRACE(theLogger, "new force of p = " << p.force());
}

void LangevinThermostat1D::initialize() {  // calculate the prefactors
...
void LangevinThermostat1D::registerPython() {
  ...
  class_<LangevinThermostat1D, shared_ptr<LangevinThermostat1D>, bases<Extension> >(
      ...
      .add_property("direction", &LangevinThermostat1D::getDirection,
                    &LangevinThermostat1D::setDirection)
      .add_property("temperature", &LangevinThermostat1D::getTemperature,
                    &LangevinThermostat1D::setTemperature);
}
}
}
