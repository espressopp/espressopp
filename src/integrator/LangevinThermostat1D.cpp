...
LangevinThermostat1D::LangevinThermostat1D(std::shared_ptr<System> system) : Extension(system)
{
...
void LangevinThermostat1D::connect()
{
    ...
    _initialize = integrator->runInit.connect(std::bind(&LangevinThermostat1D::initialize, this));

    _heatUp = integrator->recalc1.connect(std::bind(&LangevinThermostat1D::heatUp, this));

    _coolDown = integrator->recalc2.connect(std::bind(&LangevinThermostat1D::coolDown, this));

    if (adress)
    {
        _thermalizeAdr =
            integrator->aftCalcF.connect(std::bind(&LangevinThermostat1D::thermalizeAdr, this));
    }
    else
    {
        _thermalize =
            integrator->aftCalcF.connect(std::bind(&LangevinThermostat1D::thermalize, this));
    }
}
...
void LangevinThermostat1D::thermalizeAdr()
{
    ...
    // TODO: It doesn't make that much sense to thermalize both CG and AT particles, since CG
