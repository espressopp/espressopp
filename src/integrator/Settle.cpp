...
Settle::Settle(std::shared_ptr<System> _system,
...
void Settle::connect()
{
    ...
    _aftIntV =
        integrator->aftIntV.connect(std::bind(&Settle::correctVelocities, this));  // OUT AGAIN?
    ...
    for (iterator::CellListIterator cit(realCells); !cit.isDone(); ++cit)
    {
        ...
        if (molIDs.count(cit->id()) > 0)
        {
            ...
            oldPos.insert(std::make_pair(
                cit->id(), Triple<Real3D, Real3D, Real3D>((it->second.at(0))->getPos(),
...
void Settle::applyConstraints()
{
    ...
    {
        // std::cout << "in applyConstraints " << system.comm->rank() << " molID " << it->first <<
        // std::endl;
        settlep(it->first);
    }
}
...
void Settle::settlev(longint molID)
{
    ...
    real cosA = (rca2 + rab2 - rbc2) /
                (2 * rca_abs * rab_abs);  // angles depend on constrained geom, no need to recalc,
                                          // rewrite code to just do it once
...
void Settle::registerPython()
{
    ...
    class_<Settle, std::shared_ptr<Settle>, bases<Extension> >(
        "integrator_Settle", init<std::shared_ptr<System>, std::shared_ptr<FixedTupleListAdress>,
                                  real, real, real, real>())
        .def("add", pyAdd);
}
}  // namespace integrator
