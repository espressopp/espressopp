...
void LBSite::applyForces(real *m, Real3D _f, std::vector<real> &_gamma) {
  ...
  /* update stress modes */
  // See def. of _sigma (Eq.198) in B.Dünweg & A.J.C.Ladd in Adv.Poly.Sci. 221,
  // 89-166 (2009)
  real _sigma[6];
...
void LatticePar::initInvBLoc() {
  ...
  setInvBLoc(14, 9. / 2.);
  setInvBLoc(15, 9. / 2.);
  // In PRE 76, 036704 (2007) the 17th and 18th Bi are swapped in comp. to Ulf's
  // thesis. Here we use the latter one
  setInvBLoc(16, 1. / 2.);
  setInvBLoc(17, 3. / 4.);
  setInvBLoc(18, 9. / 4.);
}
