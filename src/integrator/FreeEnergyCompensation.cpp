...
FreeEnergyCompensation::FreeEnergyCompensation(std::shared_ptr<System> system,
                                               bool _sphereAdr,
                                               int _ntrotter,
                                               bool _slow)
    : Extension(system),
...
void FreeEnergyCompensation::connect()
{
    ...
    {
        _applyForce =
            integrator->aftCalcSlow.connect(std::bind(&FreeEnergyCompensation::applyForce, this));
    }
    else
    {
        _applyForce =
            integrator->aftCalcF.connect(std::bind(&FreeEnergyCompensation::applyForce, this));
    }
}
...
void FreeEnergyCompensation::applyForce()
{
    ...
    for (CellListIterator cit(cells); !cit.isDone(); ++cit)
    {
        ...
        {  // because there may be CG particles to which Free Energy Compensation is not applied
            ...
            if (weight != 1.0 && weight != 0.0)
            {
                ...
                {  // Are there atomistic particles for given CG particle? If yes, use those for
                   // calculation.
                         ...
                         ++it3)
                    {
                        ...
                        {
                            at.force() +=
                                vp.lambdaDeriv() * at.mass() * dist3D / (vp.mass() * ntrotter);
                        }
                        else
                        {
                            at.force()[0] +=
                                vp.lambdaDeriv() * at.mass() * fforce / (vp.mass() * ntrotter);
                        }
                    }
                }
                else
                {  // If not, use CG particle itself for calculation.
                    std::cout << "Particle " << vp.id() << " not found in tuples!" << std::endl
                              << "It's unclear how FEC work when combining particles, which do "
                                 "change resolution with particles that don't."
                              << std::endl;
                    exit(1);
                    return;
                }
