...
void BerendsenBarostat::setTau(real _tau) { tau = _tau; }
real BerendsenBarostat::getTau() { return tau; }
// set and get external pressure
void BerendsenBarostat::setPressure(real _P0) { P0 = _P0; }
real BerendsenBarostat::getPressure() { return P0; }
void BerendsenBarostat::setFixed(Int3D _fix) {
  fixed = _fix;
  int e = 0;
  for (int i = 0; i < 3; i++) e += fixed[i];
  exponent = 1.0 / (real)e;
}
Int3D BerendsenBarostat::getFixed() { return fixed; }
