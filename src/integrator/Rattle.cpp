...
// Refs:
// Andersen, H. C. ``Rattle: A ``velocity'' version of the Shake algorithm for
// molecular dynamics calculations'', J. Comp. Physics, 52, 24-34 (1983)
// Allen & Tildesley, ``Computer Simulation of Liquids'', OUP, 1987
// Tip: to see how to add the calculation of kinetic energy and constraint
...
Rattle::Rattle(shared_ptr<System> _system, real _maxit, real _tol, real _rptol)
    ...
    if (hp) {  // heavy atom is on this node
      ...
      if (!lp) {
        ...
        msg << "In Rattle, cannot find light particle " << lightID
            << ", all light and heavy particles in a group of rigid bonds must "
               "be on the same node"
            << std::endl;
        throw std::runtime_error(msg.str());
      }
...
void Rattle::applyPositionConstraints() {
  ...
  for (it = oldPos.begin(); it != oldPos.end(); it++) {
    ...
    // In the maps below, the values for each key pidHeavy may be initialised
    // more than, if pidHeavy is involved in more than one constrained bond.
  ...
  while (!done && iteration < maxit) {
    ...
    for (it = oldPos.begin(); it != oldPos.end(); it++) {
      ...
      if (movedLastTime[a] || movedLastTime[b]) {
        ...
        Real3D pab;
        bc.getMinimumImageVectorBox(pab, currPosition[a],
                                    currPosition[b]);  // a-b, current positions
                                                       // which change during
                                                       // iterations
        real pabsq = pab.sqr();
...
void Rattle::applyVelocityConstraints() {
  ...
  // can't use contents of oldPos to identify which constrained bonds are on
  // this CPU because particles may have changed CPU since
