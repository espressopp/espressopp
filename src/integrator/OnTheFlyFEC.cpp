...
OnTheFlyFEC::OnTheFlyFEC(std::shared_ptr<System> system) : Extension(system), center(0.0, 0.0, 0.0)
{
...
OnTheFlyFEC::~OnTheFlyFEC() { disconnect(); }

// Connect & Disconnect
void OnTheFlyFEC::connect()
{
    _gatherStats = integrator->aftIntV.connect(std::bind(&OnTheFlyFEC::gatherStats, this));
}
...
// Setter & Getter
void OnTheFlyFEC::setBins(int _bins) { bins = _bins; }
int OnTheFlyFEC::getBins() { return bins; }

void OnTheFlyFEC::setGap(int _gap) { gap = _gap; }
int OnTheFlyFEC::getGap() { return gap; }

void OnTheFlyFEC::setSteps(int _steps) { steps = _steps; }
int OnTheFlyFEC::getSteps() { return steps; }
...
// Reset the counter
void OnTheFlyFEC::resetCounter() { counter = 0; }
...
python::list OnTheFlyFEC::writeFEC()
{
    ...
    // boost::mpi::reduce(*mpiWorld, (real*)&EnergyDiff, bins, (real*)&EnergyDiffTotal,
    // std::plus<real>(),0); boost::mpi::reduce(*mpiWorld, (int*)&NumbersAtoms, bins,
