...
AssociationReaction::AssociationReaction(std::shared_ptr<System> system,
...
void AssociationReaction::setRate(real _rate) { rate = _rate; }

real AssociationReaction::getRate() { return rate; }
...
real AssociationReaction::getCutoff() { return cutoff; }

void AssociationReaction::setTypeA(size_t _typeA) { typeA = _typeA; }

size_t AssociationReaction::getTypeA() { return typeA; }

void AssociationReaction::setTypeB(size_t _typeB) { typeB = _typeB; }

size_t AssociationReaction::getTypeB() { return typeB; }

void AssociationReaction::setDeltaA(int _deltaA) { deltaA = _deltaA; }

int AssociationReaction::getDeltaA() { return deltaA; }

void AssociationReaction::setDeltaB(int _deltaB) { deltaB = _deltaB; }
...
void AssociationReaction::setInterval(int _interval) { interval = _interval; }

int AssociationReaction::getInterval() { return interval; }

AssociationReaction::~AssociationReaction() { disconnect(); }
...
void AssociationReaction::connect()
{
    ...
    _initialize = integrator->runInit.connect(std::bind(&AssociationReaction::initialize, this));

    _react = integrator->aftIntV.connect(std::bind(&AssociationReaction::react, this));
}
...
void AssociationReaction::uniqueA(boost::unordered_multimap<longint, longint>& mm)
{
    ...
    if (idxSet.size() > 0)
    {
        for (boost::unordered_set<longint>::iterator it = idxSet.begin(); it != idxSet.end(); it++)
        {
            ...
            {
                int pick = (*rng)(size);
                std::pair<boost::unordered_multimap<longint, longint>::iterator,
...
void AssociationReaction::registerPython()
{
    ...
    class_<AssociationReaction, std::shared_ptr<AssociationReaction>, bases<Extension> >(
        ...
        init<std::shared_ptr<System>, std::shared_ptr<VerletList>, std::shared_ptr<FixedPairList>,
        ...
        .add_property("interval", &AssociationReaction::getInterval,
                      &AssociationReaction::setInterval);
}
}  // namespace integrator
}  // namespace espressopp
