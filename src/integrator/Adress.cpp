...
Adress::Adress(std::shared_ptr<System> _system,
...
void Adress::connect()
{
    ...
    _SetPosVel =
        integrator->runInit.connect(std::bind(&Adress::SetPosVel, this), boost::signals2::at_front);

    // connection to after initForces()
    _initForces = integrator->aftInitF.connect(std::bind(&Adress::initForces, this),
    ...
    _integrate2 = integrator->aftIntV.connect(std::bind(&Adress::integrate2, this),
                                              boost::signals2::at_front);

    // connection to after integrate2()
    _integrateSlow = integrator->aftIntSlow.connect(std::bind(&Adress::integrateSlow, this),
                                                    boost::signals2::at_front);

    // Note: Both this extension as well as Langevin Thermostat access singal aftCalcF. This might
    // lead to undefined behavior. Therefore, we use other signals here, to make sure the Thermostat
    // would be always called first, before force distributions take place. connection to after
    // _aftCalcF()
    //_aftCalcF = integrator->aftCalcF.connect(
    //        std::bind(&Adress::aftCalcF, this));

    // connection to after aftCalcSlow
    _aftCalcSlow = integrator->aftCalcSlow.connect(std::bind(&Adress::aftCalcF, this),
                                                   boost::signals2::at_back);

    // connection to after _recalc2()
    _recalc2 =
        integrator->recalc2.connect(std::bind(&Adress::aftCalcF, this), boost::signals2::at_front);

    // connection to after _befIntV()
    _befIntV =
        integrator->befIntV.connect(std::bind(&Adress::aftCalcF, this), boost::signals2::at_front);
}
...
void Adress::communicateAdrPositions()
{
    ...
    // As this is a bit complicated: When upating only every other step, it's important to use and
    // communicate not the pointer to the actual region defining particle's position, but to a copy
    // of it. Otherwise, while the other CPUs would create new copies that don't move in between the
    // communication, the pointer on the original CPU would still point to the original particle,
    // which keeps moving. Hence, we create a totally new position vector to hold copies of the
    // positions. It has to keep existing outside of this function, hence static. Furthermore, we
    // want to reserve the memory once at the beginning, as soon as we know the what to reserve.
    // Hence, the firstcall construct. The reservation is necessary, as the vector elements are not
    // supposed to move in memory during push_back.
    ...
    if (!(verletList->getAdrCenterSet()))
    {
        ...
        if (updatecount == 0)
        {
            ...
            if (verletList->getAdrList().size() == 1)
            {
                ...
                for (CellListIterator it(realcells); it.isValid(); ++it)
                {
                    if (verletList->getAdrList().count(it->id()) == 1)
                    {
                        ...
                        // verletList->adrPositions.push_back(&(it->position())); // without the
                        // additional copy, old version
            ...
            else if (verletList->getAdrList().size() > 1)
            {
                ...
                for (CellListIterator it(realcells); it.isValid(); ++it)
                {
                    if (verletList->getAdrList().count(it->id()) == 1)
                    {
                        ...
                        // procAdrPositions.push_back(&(it->position())); // without the additional
                        // copy, old version
                    }
                }
                ...
                {
                    for (std::vector<Real3D*>::iterator itr2 = (*itr).begin(); itr2 != (*itr).end();
                         ++itr2)
                    {
            ...
            {
                std::cout << "When using moving AdResS regions there can be only either one or "
                             "several of them. It seems like the list of PIDs of region defining "
                             "particles is empty for some reason.\n";
                exit(1);
                return;
            }
        }
...
real Adress::weight(real distanceSqr)
{
    ...
    else
    {
        ...
        // return 1.0-(30.0/(pow(dhy, 5.0)))*(1.0/5.0*pow(argument, 5.0)-dhy/2.0*pow(argument, 4.0)+1.0/3.0*pow(argument,
        // 3.0)*dhy*dhy);
...
real Adress::weightderivative(real distanceSqr)
{
    ...
    else
    {
        real argument = sqrt(distanceSqr) - dex;
        // return
        // -(30.0/(pow(dhy, 5.0)))*(pow(argument, 4.0)-2.0*dhy*pow(argument, 3.0)+argument*argument*dhy*dhy);
        return -pidhy2 * 2.0 * cos(pidhy2 * argument) *
               sin(pidhy2 * argument);  // for cosine squared weighting function
    }
}
...
void Adress::registerPython()
{
    ...
    class_<Adress, std::shared_ptr<Adress>, bases<Extension> >(
        "integrator_Adress", init<std::shared_ptr<System>, std::shared_ptr<VerletListAdress>,
                                  std::shared_ptr<FixedTupleListAdress>, bool, int, int>())
