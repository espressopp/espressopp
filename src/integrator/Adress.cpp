...
void Adress::connect() {
  ...
  //         boost::bind(&Adress::communicateAdrPositions, this),
  //         boost::signals2::at_front);
  ...
  // Therefore, we use other signals here, to make sure the Thermostat would be
  // always called first, before force distributions take place.
  ...
  if (KTI == false) {
    ...
    for (CellListIterator cit(localCells); !cit.isDone(); ++cit) {
      ...
      if (it3 != fixedtupleList->end()) {
        ...
        // This loop is required when applying routines which use atomistic
        // lambdas.
...
void Adress::communicateAdrPositions() {
  // if adrCenter is not set, the center of adress zone moves along with some
  // particles
  // the coordinates of the center(s) (adrPositions) must be communicated to all
  // nodes

  // As this is a bit complicated: When upating only every other step, it's
  // important to use and communicate not the pointer to the actual region
  // defining particle's position, but to a copy of it. Otherwise, while the
  // other CPUs would create new copies that don't move in between the
  // communication, the pointer on the original CPU would still point to the
  // original particle, which keeps moving. Hence, we create a totally new
  // position vector to hold copies of the positions. It has to keep existing
  // outside of this function, hence static. Furthermore, we want to reserve the
  // memory once at the beginning, as soon as we know the what to reserve.
  // Hence, the firstcall construct. The reservation is necessary, as the vector
  // elements are not supposed to move in memory during push_back.
      ...
      if (verletList->getAdrList().size() == 1) {
        ...
        for (CellListIterator it(realcells); it.isValid(); ++it) {
          if (verletList->getAdrList().count(it->id()) == 1) {
            ...
            // verletList->adrPositions.push_back(&(it->position())); // without
            // the additional copy, old version
        ...
        for (CellListIterator it(realcells); it.isValid(); ++it) {
          if (verletList->getAdrList().count(it->id()) == 1) {
            ...
            // procAdrPositions.push_back(&(it->position())); // without the
        ...
        std::cout << "When using moving AdResS regions there can be only "
                     "either one or several of them. It seems like the list of "
                     "PIDs of region defining particles is empty for some "
                     "reason.\n";
        exit(1);
        return;
      }
...
real Adress::weightderivative(real distanceSqr) {
  if (dex2 > distanceSqr)
    ...
    real argument = sqrt(distanceSqr) - dex;
    // return -(30.0/(pow(dhy, 5.0)))*(pow(argument, 4.0)-2.0*dhy*pow(argument,
    // 3.0)+argument*argument*dhy*dhy);
