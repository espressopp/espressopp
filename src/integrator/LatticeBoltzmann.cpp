...
LatticeBoltzmann::LatticeBoltzmann(std::shared_ptr<System> _system,
                                   Int3D _nodeGrid,
                                   real _a,
                                   real _tau,
                                   int _numDims,
...
void LatticeBoltzmann::setViscB(real _visc_b)
{
    ...
    real _sec =
        getCs2() * getTau() * convTimeMDtoLB() / getNSteps() * convLenMDtoLB() / convMassMDtoLB();
...
///* it is needed as we couple LB to MD at half-timestep and, most importantly, because real
/// particles can still
...
void LatticeBoltzmann::makeDecompose()
{
    ...
    boost::mpi::all_reduce(*system.comm, _myDecompose, decompose, std::plus<bool>());
...
void LatticeBoltzmann::collideStream()
{
    ...
    for (int i = _offset; i < _myNi[0] - _offset; i++)
    {
        for (int j = _offset; j < _myNi[1] - _offset; j++)
        {
            ...
            {
                Real3D _f =
                    (*lbfor)[i][j][k].getExtForceLoc() + (*lbfor)[i][j][k].getCouplForceLoc();
...
void LatticeBoltzmann::readLBConf(int _mode)
{
    ...
    else
    {
        ...
        if (popsFile.is_open())
        {
            ...
            while (popsFile >> _i >> _j >> _k >> _fi[0] >> _fi[1] >> _fi[2] >> _fi[3] >> _fi[4] >>
                   _fi[5] >> _fi[6] >> _fi[7] >> _fi[8] >> _fi[9] >> _fi[10] >> _fi[11] >>
                   _fi[12] >> _fi[13] >> _fi[14] >> _fi[15] >> _fi[16] >> _fi[17] >> _fi[18])
            {
...
void LatticeBoltzmann::saveLBConf()
{
    ...
    {
        long int _id = cit->id();
        fprintf(couplForcesFile, "%ld %15.10f %15.10f %15.10f \n", _id, getFOnPart(_id).getItem(0),
                getFOnPart(_id).getItem(1), getFOnPart(_id).getItem(2));
    }
            ...
            for (int _k = 0; _k < _myNi[2]; _k++)
            {
                ...
                else
                {
                    fprintf(couplForcesFile, "%d %d %d %15.10f %15.10f %15.10f \n", _i, _j, _k,
                            _couplForceLoc[0], _couplForceLoc[1], _couplForceLoc[2]);
                }
            }
        }
    }
        ...
        for (int _j = _offset; _j < _myNi[1] - _offset; _j++)
        {
            for (int _k = _offset; _k < _myNi[2] - _offset; _k++)
            {
                ...
                fprintf(fluidFile, "%d %d %d %15.10f %15.10f %15.10f %15.10f\n",
                        ...
                        _myPos[2] * (_myNi[2] - 2 * _offset) + _k - 1, _rho, _jx / _rho, _jy / _rho,
                        _jz / _rho);
            }
        }
    }
...
void LatticeBoltzmann::assignMyLattice()
{
    ...
    for (int _dim = 0; _dim < getNumDims(); ++_dim)
    {
        ...
        _numSites[_dim] = (int)(ceil((_myPos[_dim] + 1) * _L[_dim] / (_nodeGrid[_dim] * getA()))) -
...
void LatticeBoltzmann::registerPython()
{
    ...
    class_<LatticeBoltzmann, std::shared_ptr<LatticeBoltzmann>, bases<Extension> >
        ...
        ("integrator_LatticeBoltzmann",
         init<std::shared_ptr<System>, Int3D, real, real, int, int>())
