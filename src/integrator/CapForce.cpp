...
CapForce::CapForce(std::shared_ptr<System> system,
                   const Real3D& _capForce,
...
void CapForce::disconnect() { _aftCalcF.disconnect(); }
...
void CapForce::connect()
{
    ...
    if (!allParticles)
    {
        _aftCalcF = integrator->aftCalcF.connect(
            std::bind(&CapForce::applyForceCappingToGroup, this), boost::signals2::at_back);
    }
    else
    {
        _aftCalcF = integrator->aftCalcF.connect(std::bind(&CapForce::applyForceCappingToAll, this),
                                                 boost::signals2::at_back);
    }
}

void CapForce::setCapForce(Real3D& _capForce) { capForce = _capForce; }

void CapForce::setAbsCapForce(real _absCapForce) { absCapForce = _absCapForce; }

Real3D& CapForce::getCapForce() { return capForce; }

real CapForce::getAbsCapForce() { return absCapForce; }

void CapForce::setAdress(bool _adress) { adress = _adress; }

bool CapForce::getAdress() { return adress; }
