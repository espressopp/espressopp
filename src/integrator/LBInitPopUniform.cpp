...
void LBInitPopUniform::createDenVel(real _rho0, Real3D _u0)
{
    ...
    {
        printf(
            "Creating an initial configuration with uniform density %f and \nvelocity %f, %f, %f\n",
        ...
        for (int j = 0; j < _Ni.getItem(1); j++)
        {
            for (int k = 0; k < _Ni.getItem(2); k++)
            {
                for (int l = 0; l < _numVels; l++)
                {
                    ...
                    value = 0.5 * latticeboltzmann->getEqWeight(l) * _rho0 *
                            (2. + 2. * scalp * invCs2 + scalp * scalp * invCs4 - trace);
...
void LBInitPopUniform::registerPython()
{
    ...
    class_<LBInitPopUniform, bases<LBInit> >(
        ...
        init<std::shared_ptr<System>, std::shared_ptr<LatticeBoltzmann> >())
