...
TDforce::TDforce(std::shared_ptr<System> system,
                 std::shared_ptr<VerletListAdress> _verletList,
                 real _startdist,
                 real _enddist,
                 int _edgeweightmultiplier,
                 bool _slow)
    : Extension(system),
      ...
      slow(_slow)
{
    ...
    // This is usually more or less the thickness of the hybrid region. However, the TD force
    // sometimes is applied in a slighty wider area.

    type = Extension::FreeEnergyCompensation;

    if (verletList->getAdrCenterSet())
    {  // adress region centre is fixed in space
...
void TDforce::connect()
{
    if (slow)
    {
        _applyForce = integrator->aftCalcSlow.connect(std::bind(&TDforce::applyForce, this),
                                                      boost::signals2::at_front);
    }
    else
    {
        _applyForce = integrator->aftCalcF.connect(std::bind(&TDforce::applyForce, this));
    }
}
...
real TDforce::getForce(longint type_id, real dist)
{
    ...
    Table table;
    if (tableIt != forces.end()) table = tableIt->second;
    if (table)
...
void TDforce::applyForce()
{
    ...
    for (CellListIterator cit(cells); !cit.isDone(); ++cit)
    {
        ...
        {  // because there may be CG particles to which TD force is not applied
        ...
        {
            if (!(verletList->getAdrCenterSet()))
            {  // adress regions defined based on particles
                ...
                if (sphereAdr)
                {
                    if (verletList->getAdrList().size() > 1)
                    {
                        ...
                        {
                            std::cout
                                << "In TDforce: Trying to apply TD force with moving, "
                                   "particle-based spherical AdResS region. However, both "
                                   "startdist and enddist set to 0. This does not make sense.\n";
                            exit(1);
                            return;
                        }

                        real buffer = 0.000001;
                        real width = fabs(enddist - startdist +
                        ...
                        if ((dist3Dabs < enddist + buffer) && (dist3Dabs > startdist - buffer))
                        {
                            weight =
                                ...
                                (dist3Dabs - startdist - buffer) /
                                    width;  // 0 at edge to CG region, 1 at edge to atomistic region
                        ...
                        Real3D direction = dist3D * weight /
                        ...
                        real mindist3Dabs =
                            sqrt(mindist3D.sqr());  // calculate overall smallest absolute distance
                ...
                {
                    std::cout << "In TDforce: Trying to use moving AdResS region with adaptive "
                                 "region slab geometry. This is not implemented and doesn't make "
                                 "much sense anyhow.\n";
                    exit(1);
                    return;
                }
            ...
            if (!(verletList->getAdrCenterSet()))
            {
                if (sphereAdr)
                {
                    if (verletList->getAdrList().size() > 1)
                    {
                        ...
                        {
                            pa = **it2;
                            verletList->getSystem()->bc->getMinimumImageVector(dist3D,
                                                                               cit->getPos(), pa);
...
void TDforce::registerPython()
{
    ...
    void (TDforce::*pyAddForce)(int itype, const char* filename, int type) = &TDforce::addForce;

    class_<TDforce, std::shared_ptr<TDforce>, bases<Extension> >(
        "integrator_TDforce",
        init<std::shared_ptr<System>, std::shared_ptr<VerletListAdress>, real, real, int, bool>())
