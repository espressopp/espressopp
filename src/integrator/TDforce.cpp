...
TDforce::TDforce(shared_ptr<System> system,
                 shared_ptr<VerletListAdress> _verletList, real _startdist,
                 real _enddist, int _edgeweightmultiplier)
    : Extension(system),
      verletList(_verletList),
      startdist(_startdist),
      enddist(_enddist),
      edgeweightmultiplier(_edgeweightmultiplier) {
  // startdist & enddist are the distances between which the TD force actually
  // acts.
  // This is usually more or less the thickness of the hybrid region. However,
  // the TD force sometimes is applied in a slighty wider area.

  type = Extension::FreeEnergyCompensation;

  if (verletList->getAdrCenterSet()) {  // adress region centre is fixed in
                                        // space
...
void TDforce::connect() {
  _applyForce =
      integrator->aftCalcF.connect(boost::bind(&TDforce::applyForce, this));
}

void TDforce::disconnect() { _applyForce.disconnect(); }
...
void TDforce::applyForce() {
  ...
  for (CellListIterator cit(cells); !cit.isDone(); ++cit) {
    Table table;
    if (forces.find(cit->getType()) !=
        forces.end()) {  // because there may be CG particles to which TD force
                         // is not applied
      table =
          forces.find(cit->getType())->second;  // TODO shouldn't do find twice
    }

    if (table) {
      if (!(verletList->getAdrCenterSet())) {  // adress regions defined based
                                               // on particles

        if (sphereAdr) {
          if (verletList->getAdrList().size() > 1) {
            if ((startdist == 0.0) && (enddist == 0.0)) {
              std::cout << "In TDforce: Trying to apply TD force with moving, "
                           "particle-based spherical AdResS region. However, "
                           "both startdist and enddist set to 0. This does not "
                           "make sense.\n";
              exit(1);
              return;
            }
            ...
            real width =
                fabs(enddist - startdist +
                     2.0 * buffer);  // width of region where TD force acts
            ...
            // weighting scheme: only particles that are in a hybrid region
            // contribute
            if ((dist3Dabs < enddist + buffer) &&
                (dist3Dabs > startdist - buffer)) {
              weight = 1.0 -
                       (dist3Dabs - startdist - buffer) /
                           width;  // 0 at edge to CG region, 1 at edge to
                                   // atomistic region
            ...
            Real3D direction =
                dist3D * weight /
                dist3Dabs;  // normalized but weighted direction vector
            ...
            for (; it2 != verletList->getAdrPositions().end(); ++it2) {
              ...
              if (dist3D.sqr() < mindist3D.sqr())
                mindist3D =
                    dist3D;  // make it the minimum if shortest length so far
              ...
              direction +=
                  dist3D * weight / dist3Dabs;  // add to direction vector
            }

            real mindist3Dabs = sqrt(
                mindist3D
                    .sqr());  // calculate overall smallest absolute distance

            // apply td force
            if ((mindist3Dabs > startdist) && (mindist3Dabs < enddist) &&
                (direction.sqr() > 0.0)) {
            ...
            Real3D dist3D;
            bc.getMinimumImageVectorBox(dist3D, cit->getPos(),
                                        center);  // pos - center
          ...
          std::cout << "In TDforce: Trying to use moving AdResS region with "
                       "adaptive region slab geometry. This is not implemented "
                       "and doesn't make much sense anyhow.\n";
          exit(1);
          return;
        }
...
void TDforce::registerPython() {
  ...
  class_<TDforce, shared_ptr<TDforce>, bases<Extension> >(
      "integrator_TDforce",
      init<shared_ptr<System>, shared_ptr<VerletListAdress>, real, real, int>())
