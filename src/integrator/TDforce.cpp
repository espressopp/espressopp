...
TDforce::TDforce(shared_ptr<System> system, shared_ptr<VerletListAdress> _verletList,
                 real _startdist, real _enddist, int _edgeweightmultiplier)
    : Extension(system),
...
void TDforce::connect() {
  _applyForce = integrator->aftCalcF.connect(boost::bind(&TDforce::applyForce, this));
}
...
void TDforce::applyForce() {
  ...
  for (CellListIterator cit(cells); !cit.isDone(); ++cit) {
    ...
    if (tableIt !=
        forces.end()) {  // because there may be CG particles to which TD force is not applied
        ...
        if (sphereAdr) {
          if (verletList->getAdrList().size() > 1) {
            if ((startdist == 0.0) && (enddist == 0.0)) {
              std::cout << "In TDforce: Trying to apply TD force with moving, particle-based "
                           "spherical AdResS region. However, both startdist and enddist set to 0. "
            ...
            real width =
                fabs(enddist - startdist + 2.0 * buffer);  // width of region where TD force acts
            ...
            Real3D direction =
                dist3D * weight / dist3Dabs;  // normalized but weighted direction vector
            ...
            for (; it2 != verletList->getAdrPositions().end(); ++it2) {
              ...
              if (dist3D.sqr() < mindist3D.sqr())
                mindist3D = dist3D;            // make it the minimum if shortest length so far
          ...
          std::cout << "In TDforce: Trying to use moving AdResS region with adaptive region slab "
                       "geometry. This is not implemented and doesn't make much sense anyhow.\n";
          exit(1);
          return;
        }
