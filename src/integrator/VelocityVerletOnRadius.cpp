...
VelocityVerletOnRadius::VelocityVerletOnRadius(shared_ptr<System> system,
                                               real _radialDampingMass)
...
void VelocityVerletOnRadius::connect() {
  ...
  _aftIntP = integrator->aftIntP.connect(
      boost::bind(&VelocityVerletOnRadius::integrate1, this));
  _aftIntV = integrator->aftIntV.connect(
      boost::bind(&VelocityVerletOnRadius::integrate2, this));
  _aftInitF = integrator->aftInitF.connect(
      boost::bind(&VelocityVerletOnRadius::initForces, this));
}
...
void VelocityVerletOnRadius::integrate1() {
  ...
  for (CellListIterator cit(realCells); !cit.isDone(); ++cit) {
    LOG4ESPP_DEBUG(
        theLogger,
        "Particle " << cit->id() << ", forceRadius = " << cit->fradius());
  }
  for (CellListIterator cit(realCells); !cit.isDone(); ++cit) {
    // LOG4ESPP_DEBUG(theLogger, "Particle " << cit->id() <<  ", radius = " <<
    // cit->radius());
...
void VelocityVerletOnRadius::registerPython() {
  ...
  class_<VelocityVerletOnRadius, shared_ptr<VelocityVerletOnRadius>,
         bases<Extension> >("integrator_VelocityVerletOnRadius",
                            init<shared_ptr<System>, real>())
      .def("connect", &VelocityVerletOnRadius::connect)
      .def("disconnect", &VelocityVerletOnRadius::disconnect)
      .add_property("radialDampingMass",
                    &VelocityVerletOnRadius::getRadialDampingMass,
                    &VelocityVerletOnRadius::setRadialDampingMass);
}
}
}
