...
LangevinThermostatOnRadius::LangevinThermostatOnRadius(std::shared_ptr<System> system,
                                                       real _dampingmass)
    : Extension(system)
{
...
void LangevinThermostatOnRadius::setGamma(real _gamma) { gamma = _gamma; }
...
LangevinThermostatOnRadius::~LangevinThermostatOnRadius() { disconnect(); }
...
void LangevinThermostatOnRadius::connect()
{
    ...
    _initialize =
        integrator->runInit.connect(std::bind(&LangevinThermostatOnRadius::initialize, this));

    _heatUp = integrator->recalc1.connect(std::bind(&LangevinThermostatOnRadius::heatUp, this));

    _coolDown = integrator->recalc2.connect(std::bind(&LangevinThermostatOnRadius::coolDown, this));

    _thermalize =
        integrator->aftCalcF.connect(std::bind(&LangevinThermostatOnRadius::thermalize, this));
}
...
void LangevinThermostatOnRadius::frictionThermo(Particle& p)
{
    ...
    p.fradius() += pref1 * p.vradius() * dampingmass + pref2 * ranval * massf;
...
void LangevinThermostatOnRadius::registerPython()
{
    ...
    class_<LangevinThermostatOnRadius, std::shared_ptr<LangevinThermostatOnRadius>,
           bases<Extension> >("integrator_LangevinThermostatOnRadius",
                              init<std::shared_ptr<System>, real>())
        ...
        .add_property("gamma", &LangevinThermostatOnRadius::getGamma,
                      &LangevinThermostatOnRadius::setGamma)
        .add_property("temperature", &LangevinThermostatOnRadius::getTemperature,
                      &LangevinThermostatOnRadius::setTemperature);
}
