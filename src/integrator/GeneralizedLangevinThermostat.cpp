...
{
    _integrate =
        integrator->aftIntP.connect(std::bind(&GeneralizedLangevinThermostat::integrate, this));

    _friction =
        integrator->aftCalcF.connect(std::bind(&GeneralizedLangevinThermostat::friction, this));
}
    ...
    for (CellListIterator cit(cells); !cit.isDone(); ++cit)
    {
        ...
        if (table)
        {
            ...
            else  // if(weight < 1.0 && weight > 0.0)
            {
                ...
                t = table->getEnergy(weight);
                vp.extVar() =
                    vp.extVar() * (1.0 - timestep / t) -
                    (timestep / t) * c * vp.lambdaDeriv() * vp.lambdaDeriv() * vp.velocity()[0];
            } /*else {
                  std::cout << "ERROR: In Generalized Langevin Thermostat Extension weight is out of
            [0.0, 1.0] interval: " << weight <<  std::endl; exit(1);
            }*/
        ...
        {
            std::cout
                << "ERROR: Using Generalized Langevin Thermostat Extension without providing table."
                << std::endl;
            exit(1);
        }
    }
...
void GeneralizedLangevinThermostat::registerPython()
{
    ...
    class_<GeneralizedLangevinThermostat, std::shared_ptr<GeneralizedLangevinThermostat>,
           bases<Extension> >("integrator_GeneralizedLangevinThermostat",
