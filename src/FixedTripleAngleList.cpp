    ...
    : storage(_storage), triplesAngles()
{
    ...
    con1 = storage->beforeSendParticles.connect(
        std::bind(&FixedTripleAngleList::beforeSendParticles, this, std::placeholders::_1,
                  std::placeholders::_2));
    con2 = storage->afterRecvParticles.connect(std::bind(&FixedTripleAngleList::afterRecvParticles,
