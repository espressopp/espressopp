...
python::list MeanSquareInternalDist::compute() const {
  ...
  sort(localIDs.begin(), localIDs.end());  // sorts entries from low to high -
                                           // should not be necessary as long as
                                           // the above iterator iterates in
  ...
  int N_chains = localIDs.size() / chainlength;  // N_chains is number of chains
                                                 // in this cpu and chainlength
                                                 // is a global variable from
                                                 // ConfigsParticleDecomp.hpp
