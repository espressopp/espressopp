...
python::list MeanSquareInternalDist::compute() const
{
    ...
    vector<longint> localIDs;  // localIDs is a vector with PID of particles in each CPU and it
                               // contains whole chains
    ...
    sort(localIDs.begin(),
         localIDs.end());  // sorts entries from low to high - should not be necessary as long as
                           // the above iterator iterates in ascending order according to the keys
    ...
    int N_chains =
        localIDs.size() / chainlength;  // N_chains is number of chains in this cpu and chainlength
                                        // is a global variable from ConfigsParticleDecomp.hpp

    for (int n = 1; n < chainlength; n++)
    {  // loop over chainlength
        ...
        for (int j = 0; j < N_chains; j++)
        {
            ...
            intdist +=
                sumdistsq /
                real(chainlength -
                     n);  // sum of the internal distances (chainlength-n) of the chains in this cpu
        }
