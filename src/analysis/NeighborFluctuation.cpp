...
python::list NeighborFluctuation::computeValues() const {
  ...
  // For the moment we don't need the pairs itself. We just need to know what is
  // the
  // number of neighbors each particle has.
  // boost::unordered_multimap <longint, longint> pairs;
  // boost::unordered_multimap <longint, longint>::iterator pairs_it =
  // pairs.begin();
  ...
  for (CellListAllPairsIterator it(cl); it.isValid(); ++it) {
    ...
    // cout << "pair (" << it->first->id() << ", " << it->second->id() << ")" <<
    // endl;
    if (distsq <= radsq) {
      neighbNum.insert(it->first->id());
      neighbNum.insert(it->second->id());
      // pairs_it = pairs.insert(pairs_it, std::make_pair(it->first->id(),
      // it->second->id()));
      // pairs_it = pairs.insert(pairs_it, std::make_pair(it->second->id(),
      // it->first->id()));
      // std::cout << "added pair (" << it->first->id() << ", " <<
  ...
  // std::cout << "nave_sq="<<nave_sq<<" nsq_ave="<<nsq_ave<< " myE="<< myE <<
  // std::endl;
...
real NeighborFluctuation::compute() const { return -1.0; }
