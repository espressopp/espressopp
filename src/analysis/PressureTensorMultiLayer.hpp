...
public:
    ...
    PressureTensorMultiLayer(std::shared_ptr<System> system, int _n, real _dz)
        : AnalysisBaseTemplate<vector<Tensor> >(system), n(_n), dz(_dz)
    {
    }
    ...
    python::list getAverageValue()
    {
        ...
        {
            Tensor aux(0.0);
            for (int j = 0; j < 6; j++) aux[j] = sqrt(res[i][j] / (nMeasurements - 1));
        ...
        if (nMeasurements > 0)
        {
            ...
            else
            {
                for (int i = 0; i < n; i++)
                {
                    ...
                    for (int j = 0; j < 6; j++)
                    {
                        newVariance[i][j] = lastVariance[i][j] + (res[i][j] - lastAverage[i][j]) *
                                                                     (res[i][j] - newAverage[i][j]);
                    }
