...
public:
    ...
    ConfigsParticleDecomp(std::shared_ptr<System> system) : SystemAccess(system)
    {
        ...
        {
            if (this_node == 0) cout << "Exception: " << e.what() << "\n";
        }
    }
     ...
     */
    ConfigsParticleDecomp(std::shared_ptr<System> system, int _chainlength, int _start_pid)
        : SystemAccess(system)
    {
        // by default key = "position", it will store the particle positions
        // (option: "velocity" or "unfolded")
        ...
        // CPU0 will use particles 0+start_pid, 1+start_pid, 2+start_pid, ...
        // local_num_particles-1+start_pid. CPU1 will use particles local_num_particles+start_pid,
        // local_num_particles+1+start_pid,...
        int nodeNum = -1;
        for (int id = 0; id < num_of_part; id++)
        {
            if (id % local_num_part == 0)
            {
                ++nodeNum;
            }
        ...
        if (nodeNum >= n_nodes)
        {
            if (this_node == 0)
            {
                cout << "assignment went wrong. Particles were assigned to proc " << nodeNum
                     << "\n";
        ...
        // chain 0 consists of particles 0+start_pid, 1+start_pid, 2+start_pid, ... chainlength
        // -1+start_pid chain1 consists of particles chainlength+start_pid,
        // chainlength+1+start_pid,...
        int cid = -1;
        for (int id = 0; id < num_of_part; id++)
        {
            if (id % chainlength == 0)
        ...
        if (nodeNum >= n_nodes)
        {
            if (this_node == 0)
            {
                cout << "assignment went wrong. Chains were assigned to proc " << nodeNum << "\n";
                cout << "highest process number should be " << n_nodes - 1 << "\n";
                cout << "check if total number of particles matches with chainlength\n";
            }
        }
