...
// It will calculate the molecular local virial pressure in 'n' layers along X
// axis, works only for AdResS simulations.
python::list XTemperature::computeArray(int n) const {
  ...
  for (CellListIterator cit(realCells); !cit.isDone(); ++cit) {
    ...
    if (it2 != fixedtupleList->end()) {  // Are there atomistic particles for
                                         // given CG particle? If yes, use those
                                         // for calculation.
...
real XTemperature::compute() const { return -1.0; }
