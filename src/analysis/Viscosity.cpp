...
python::list Viscosity::compute(real t0, real dt, real T)
{
    ...
    for (int i = 0; i < n_nodes; i++)
    {
        ...
        if (num_m[i] >= M) num_m[i] = M - 1;
    }

    int min_m = 1;
    if (this_node != 0) min_m = num_m[this_node - 1];

    int max_m = num_m[this_node];

    python::list integr;

    if (this_node == 0) cout << "integration..." << endl;

    real SUM = (auto_pxy_pxy[0] + auto_pxy_pxy[min_m]) * 0.5;
    for (int j = 1; j < min_m; j++) SUM += auto_pxy_pxy[j];
