...
/*
 * calculates the mean square displacement of the particles/monomers in the COM
 * of the whole system
 *
...
python::list MeanSquareDispl::computeG2() const {
  ...
  //
  //      //creating vector which stores particleIDs for each CPU
  //      vector<longint> localIDs; //for each CPU this will store particle IDs
  //      of particles calculated by CPU
  //      for (map<size_t,int>::const_iterator itr=idToCpu.begin();
  //      itr!=idToCpu.end(); ++itr) {
  ...
  //        }
  //      }
  //      sort(localIDs.begin(), localIDs.end()); //sorts entries from low to
  //      high
  ...
  //      vector< vector<Real3D> > local_chainCOMlist; //will store COM of conf
  //      n and chain cid as chainCOMlist[n][cid]
  //      for(int m=0; m<M; m++){
  //        vector<Real3D> innerList; //will store the local chains' COM of one
  //        conf/snapshot
  //
  ...
  //            mass += 1;
  //            count += 1;
  //            // this is the right if request. remember that particle 0 also
  //            has a mass of 1
  ...
  //      }
  //      //now local_chainCOMlist contains the local chains' COMs of each
  //      snapshot
  //
  //      // MSD calculation
  //      int perc=0, perc1=0;
  ...
  //        for(int n=0; n<M-m; n++){
  //          int part_count = 0;
  //          int local_cid = 0; //local chainID. each CPU starts with chain
...
python::list MeanSquareDispl::computeG3() const {
  ...
  for (int m = 0; m < M; m++) {
    ...
    for (int entry = 0; entry < localIDs.size(); entry++) {
      ...
      mass += 1;
      part_count += 1;
      // this is the right if request. remember that particle 0 also has a mass
      // of 1
      if (part_count == chainlength) {
