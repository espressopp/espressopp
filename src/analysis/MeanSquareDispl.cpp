...
python::list MeanSquareDispl::compute() const
{
    ...
    for (int m = 0; m < M; m++)
    {
        ...
        if (print_progress && system.comm->rank() == 0)
        {
            ...
            if (perc % 5 == 0)
            {
                cout << "calculation progress (mean square displacement): " << perc << " %\r"
                     << flush;
            }
        }
    }
...
python::list MeanSquareDispl::computeG2() const
{
    ...
    //
    //      //creating vector which stores particleIDs for each CPU
    //      vector<longint> localIDs; //for each CPU this will store particle IDs of particles
    //      calculated by CPU for (map<size_t,int>::const_iterator itr=idToCpu.begin();
    //      itr!=idToCpu.end(); ++itr) {
    ...
    //      vector< vector<Real3D> > local_chainCOMlist; //will store COM of conf n and chain cid as
    //      chainCOMlist[n][cid] for(int m=0; m<M; m++){
    ...
    //        for(int n=0; n<M-m; n++){
    //          int part_count = 0;
    //          int local_cid = 0; //local chainID. each CPU starts with chain local_cid = 0, so it
    //          is not a global id
    //          //loop over local particles
    //          for(int entry = 0; entry < localIDs.size(); entry++){
    //            longint i = localIDs[entry]; //pid
    ...
    //            local_chainCOMlist[n+m][local_cid]; Real3D pos2 = getConf(n)->getCoordinates(i) -
    //            local_chainCOMlist[n][local_cid]; Real3D delta = pos2 - pos1; Z[m] += delta.sqr();
    //            part_count++;
    //          }
    //        }
    //        if(print_progress && system.comm->rank()==0){
...
python::list MeanSquareDispl::computeG3() const
{
    ...
    for (int m = 0; m < M; m++)
    {
        ...
        for (int i = 0; i < int_c(local_chainCOMlist[m].size()); i++)
        {
            ...
            printReal3D(local_chainCOMlist[m][i]);
            printf("\n");
        }
    }
    ...
    for (int m = 0; m < M; m++)
    {
        ...
        for (int n = 0; n < M - m; n++)
        {
            ...
            int local_chains = int_c(local_chainCOMlist[0].size());  // looked up only for snapshot
                                                                     // 0
            // int local_chains = local_chainCOMlist[n].size(); //same value as above, but looked up
            // new for every snapshot
...
void MeanSquareDispl::registerPython()
{
    ...
    class_<MeanSquareDispl, bases<ConfigsParticleDecomp> >("analysis_MeanSquareDispl",
        ...
        .add_property("print_progress", &MeanSquareDispl::getPrint_progress,
