...
{
public:
    NeighborFluctuation(std::shared_ptr<System> system, real _radius)
        : Observable(system), radius(_radius)
    {
        ...
        real minL = std::min(Li[0] / (real)cellG[0],
                             std::min(Li[1] / (real)cellG[1], Li[2] / (real)cellG[2]));
        if (radius > minL)
        {
