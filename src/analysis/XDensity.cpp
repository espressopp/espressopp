...
// splitN is a level of discretisation of density profile (how many elements it
// contains)
python::list XDensity::computeArray(int splitN) const {
  ...
  Iterate over all (CG) particles.
                ...
                if (it2 != fixedtupleList->end()) {  // Are there atomistic
  particles for given CG particle? If yes, use those for calculation.
                      std::vector<Particle*> atList;
                      atList = it2->second;
                      for (std::vector<Particle*>::iterator it3 =
  atList.begin();
                                           it3 != atList.end(); ++it3) {
                          Particle &at = **it3;
                          int id = at.id();
  ...
  if (system.storage->getFixedTuples()) {
    ...
    for (CellListIterator cit(realCells); !cit.isDone();
         ++cit) {  // Iterate over all (CG) particles.
      Particle &vp = *cit;
      FixedTupleListAdress::iterator it2;
      it2 = fixedtupleList->find(&vp);

      if (it2 != fixedtupleList->end()) {  // Are there atomistic particles for
                                           // given CG particle? If yes, use
  ...
  /*if(system.comm->rank()==0){
    ...
    if(perc>perc1){
      cout<<"calculation progress (density profile along x axis): "<< perc << "
  %\r"<<flush;
      perc1 = perc;
    }
  }*/
  //}
  // if(system.comm->rank()==0)
  // cout<<"calculation progress (density profile along x axis): "<< 100 << " %"
  // <<endl;
