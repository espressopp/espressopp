    ...
    /*for (int rank_i=0; rank_i<nprocs; rank_i++) {
      ...
      if (rank_i == myrank) {
          if(system.storage->getFixedTuples()){
              ...
              for (CellListIterator cit(realCells); !cit.isDone(); ++cit) {  // Iterate over all
    (CG) particles. Particle &vp = *cit; FixedTupleListAdress::iterator it2; it2 =
    fixedtupleList->find(&vp);

                  if (it2 != fixedtupleList->end()) {  // Are there atomistic particles for given CG
    particle? If yes, use those for calculation. std::vector<Particle*> atList; atList =
    it2->second; for (std::vector<Particle*>::iterator it3 = atList.begin(); it3 != atList.end();
    ++it3) { Particle &at = **it3; int id = at.id(); conf[id] = at.position();
                        }
                  }
...
real XDensity::compute() const { return -1.0; }
...
void XDensity::registerPython()
{
    ...
    class_<XDensity, bases<Observable> >("analysis_XDensity", init<std::shared_ptr<System> >())
