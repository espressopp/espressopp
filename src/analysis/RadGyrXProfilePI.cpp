    ...
    if (system.storage->getFixedTuples())
    {
        ...
        for (CellListIterator cit(realCells); !cit.isDone(); ++cit)
        {
            ...
            if (int_c(vp.type()) == ptype)
            {
                ...
                {
                    std::cout << "No atomistic particles found in RadGyrXProfilePI::computeArray "
                                 "function.";
                    exit(1);
                }
...
real RadGyrXProfilePI::compute() const { return -1.0; }
...
void RadGyrXProfilePI::registerPython()
{
    ...
    class_<RadGyrXProfilePI, bases<Observable> >("analysis_RadGyrXProfilePI",
