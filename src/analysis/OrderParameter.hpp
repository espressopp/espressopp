...
public:
    OrderParticleProps() : d(0), qlmSumSqrt(0), nnns(0), ang_m(0), particle_id(-1), label(-1)
    {
        is_solid = false;
        is_surface = false;
    }

    OrderParticleProps(int am) : d(0), qlmSumSqrt(0), nnns(0), ang_m(am), particle_id(-1), label(-1)
    {
        qlm = vector<dcomplex>(2 * am + 1, dcomplex(0.0, 0.0));
        is_solid = false;
        is_surface = false;
    }
    OrderParticleProps(int am, int pid)
        : d(0), qlmSumSqrt(0), nnns(0), ang_m(am), particle_id(pid), label(-1)
    {
        qlm = vector<dcomplex>(2 * am + 1, dcomplex(0.0, 0.0));
        is_solid = false;
        is_surface = false;
    ...
    {
        if (v.size() != qlm.size())
            cout << "Vectors have not the same size. Local: " << qlm.size() << "  added  "
                 << v.size() << endl;
    ...
    {
        if (v.size() != qlm.size())
            cout << "Vectors have not the same size. Local: " << qlm.size() << "  new  " << v.size()
                 << endl;
        for (int i = 0; i < int_c(qlm.size()); i++)
        {
            qlm[i] = v[i];
...
public:
    ...
    RealND computeRaw()
    {
        ...
        // ------------------------------------------------------------------------------
        // loop over pairs
        for (boost::unordered_multimap<int, int>::iterator pit = pairs.begin(); pit != pairs.end();
             ++pit)
        {
            int first = (*pit).first;
            ...
            {
                cerr << " Bead2: " << second << " in pair has 0 sum: " << opp_i2->getSumQlm()
                     << endl;
            }
            if (opp_i1->getSumQlm() == 0)
            {
                cerr << " Bead1: " << first << " in pair has 0 sum: " << opp_i1->getSumQlm()
                     << endl;
            }
            ...
            {
                real d1 =
                    (opp_i1->getQlm(m) * conj(opp_i2->getQlm(m))).real() / opp_i2->getSumQlm();
                opp_i1->setD(opp_i1->getD() + d1);
                real d2 =
                    (opp_i2->getQlm(m) * conj(opp_i1->getQlm(m))).real() / opp_i1->getSumQlm();
    ...
    void define_solid()
    {
        ...
        if (incl_surface)
        {
            ...
            int maxSize, vecSize = sendGhostInfo.size();
            mpi::all_reduce(*cmm, vecSize, maxSize, mpi::maximum<int>());
            while (int_c(sendGhostInfo.size()) < maxSize)
                sendGhostInfo.push_back(OrderParticleProps());
            vector<OrderParticleProps> totID;
    ...
    void cluster_analysis()
    {
        ...
        for(CellListIterator cit(cells); !cit.isDone(); ++cit) {
          ...
          OrderParticleProps &opp_i = (opp_map.find( pid ))->second;
          if ( opp_i.getLabel() > -1){
            cout<< "cpu:  " << getSystem()->comm->rank() << "  pid: "<< pid<< "  label:"<<
        opp_i.getLabel() << endl;
          }
        }
        */
    ...
    void global_cluster_analysis()
    {
        ...
        vector<communicate_label> tot_label_info =
            vector<communicate_label>(numProc * maxSize, cccc);
             ...
             it != tot_label_info.end(); ++it)
        {
            ...
            if (cur_lab.cpu != -1 && getSystem()->storage->lookupRealParticle(cur_lab.pid))
            {
                ...
                if ((opp.getSolid() || opp.getSurface()))
                {
                    ...
                    << " particle: "<< opp.getPID()<< " is "<<  opp.getSolid() << "  " <<
                    opp.getSurface() << " and has label:  "<< opp.getLabel() << endl;
                    }
                    */
                }
            }
    ...
    void biggest_cluster()
    {
        ...
        int numProc = getSystem()->comm->size();
        vector<OrderParticleProps> totID =
            vector<OrderParticleProps>(numProc * maxSize, OrderParticleProps());
        ...
        if (getSystem()->comm->rank() == 0)
        {
            for (vector<OrderParticleProps>::iterator it = totID.begin(); it != totID.end(); ++it)
            {
                ...
                if (gop.getPID() >= 0)
                {
                    if (gop.getLabel() >= 0)
                    {
                        while ((int)clusters.size() <= (int)gop.getLabel())
            ...
            /*
            for(boost::unordered_multimap<int, OrderParticleProps>::iterator opm = opp_map.begin();
            opm!=opp_map.end(); ++opm){ OrderParticleProps &opp = (*opm).second;
            ...
            */
            ...
            for (vector<vector<int> >::iterator it = clusters.begin(); it != clusters.end(); ++it)
            {
                ...
                {
                    empty++;
                    // cout<<"  cpu: "<< getSystem()->comm->rank() <<   " label: "<< (it -
                    // clusters.begin()) << "  is empty" << endl;
                }
            }
    ...
    python::list compute()
    {
        ...
        int numProc = getSystem()->comm->size();
        vector<OrderParticleProps> totID = vector<OrderParticleProps>( numProc * maxSize,
        OrderParticleProps() ); mpi::gather( *getSystem()->comm, &sendGhostInfo[0], maxSize, totID,
        0);
        ...
        */
        ...
        if (getSystem()->comm->rank() == 0)
        {
            ...
            cout << "\n   inside OP:  max_cl: "<< getMax_Cl() << "  num_cl: "<< getNum_of_Cl()<<
            endl;
            */
