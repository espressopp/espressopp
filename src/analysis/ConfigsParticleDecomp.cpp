...
int ConfigsParticleDecomp::getListSize() const { return configurations.size(); }

ConfigurationList ConfigsParticleDecomp::all() const { return configurations; }
...
void ConfigsParticleDecomp::gather() {
  ...
  if (myrank == 0) {
    ...
    if (curNumP != num_of_part) {
      stringstream msg;
      msg << "   ConfigsParticleDecomp gathers the configurations of the same "
             "system\n"
             " with the same number of particles. If you need to store the "
             "systems\n"
             " with different number of particles you should use something "
             "else."
             " E.g `Configurations`";
      err.setException(msg.str());
    }
  ...
  if (myrank == 0) {
    ...
    if (file.is_open()) {
      ...
      if (count != num_of_part) {
        stringstream msg;
        msg << "Number of read particles does not match the number of "
               "particles of the system (which is "
            << num_of_part << ")";
        err.setException(msg.str());
      }
    } else {
