...
void LBOutputScreen::findLBMom(int _mode) {
  ...
  if (_mode == 0) {
    ...
    boost::mpi::reduce(*system.comm, _myMom, result, std::plus<Real3D>(), 0);
  } else {
    // collect momenta and share the result between CPUs
    boost::mpi::all_reduce(*system.comm, _myMom, result, std::plus<Real3D>());
    setLBMom(result);
  }
...
void LBOutputScreen::registerPython() {
  ...
  class_<LBOutputScreen, bases<LBOutput> >(
      "analysis_LBOutput_Screen",
      init<shared_ptr<System>, shared_ptr<integrator::LatticeBoltzmann> >())
