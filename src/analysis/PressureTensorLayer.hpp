...
public:
    ...
    PressureTensorLayer(std::shared_ptr<System> system, real _z0, real _dz)
        : AnalysisBaseTemplate<Tensor>(system), z0(_z0), dz(_dz)
    {
    }
    ...
    void updateAverage(Tensor res)
    {
        ...
        if (nMeasurements > 0)
        {
            ...
            else
            {
                ...
                {
                    newVariance[i] =
                        lastVariance[i] + (res[i] - lastAverage[i]) * (res[i] - newAverage[i]);
                }
                lastAverage = newAverage;
