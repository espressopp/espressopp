    ...
    : storage(_storage), globalTuples()
{
    ...
    con1 = storage->beforeSendParticles.connect(std::bind(
        &FixedTupleList::beforeSendParticles, this, std::placeholders::_1, std::placeholders::_2));
    con2 = storage->afterRecvParticles.connect(std::bind(
        &FixedTupleList::afterRecvParticles, this, std::placeholders::_1, std::placeholders::_2));
    con3 =
        storage->onParticlesChanged.connect(std::bind(&FixedTupleList::onParticlesChanged, this));
}
...
bool FixedTupleList::addTuple(boost::python::list& tuple)
{
    bool returnVal = true;
...
void FixedTupleList::beforeSendParticles(ParticleList& pl, OutBuffer& buf)
{
...
void FixedTupleList::afterRecvParticles(ParticleList& pl, InBuffer& buf)
{
