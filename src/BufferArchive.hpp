...
public:
    ...
    void read(Particle& p) { ar >> p; }

    void read(ParticleForce& f) { ar >> f; }

    void read(std::vector<longint>& v) { ar >> v; }

    void recv(longint sender, int tag) { comm.recv(sender, tag, ar); }
};
...
public:
    ...
    void write(ParticleForce& f) { ar << f; }

    void write(Particle& p) { ar << p; }

    void write(std::vector<longint>& v) { ar << v; }

    void send(longint receiver, int tag) { comm.send(receiver, tag, ar); }
};
