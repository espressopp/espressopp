...
##################################################################################################
#                                                                                                                                                                             #
#  ESPResSo++ Python script for an H-AdResS simulation of water/Hamonic potential using Multiple Time Stepping  #
#                                                                                                                                                                             #
##################################################################################################
...
# Performs an H-AdResS adaptive resolution simulation using a RESPA multiple time stepping scheme.
...
# while the coarse-grained interaction potential is a simple truncated Hamonic potential.
print 'Performing an ESPResSo++ H-AdResS simulation with multiple time stepping.\n'
...
interaction_cutoff_cg = 0.4  # cutoff coarse-grained potential
...
ex_size = 2.0 # radius of atomistic region
hy_size = 2.0 # width of hybrid region
...
# call the gromacs parser for processing the top file (and included files) and the conf file
...
# set up atomistic nonbonded interactions. This interaction template handles both the Lannard Jones and the Reaction Field within H-AdResS
interNBat = espressopp.interaction.VerletListHadressATLenJonesReacFieldGen(verletlist, ftpl)
...
# set up coarse-grained potential (a truncated harmonic potential)
potCG = espressopp.interaction.Harmonic(K=500.0, r0=interaction_cutoff_cg, cutoff=interaction_cutoff_cg)
interNBcg = espressopp.interaction.VerletListHadressCGHarmonic(verletlist, ftpl)
...
# add Free Energy Compensation
fec = espressopp.integrator.FreeEnergyCompensation(system, center=[Lx/2, Ly/2, Lz/2], slow=correction_on_slow_timescale)
fec.addForce(itype=3, filename="table_fec.dat", type=typeCG)
...
print ''
print "time, temperature, E_bonds, E_angles, E_nonbonded, E_kinetic, E_correction, E_total"
fmt = '%5.5f %15.8g %15.10g %15.10g %15.10g %15.10g %15.10g %15.10g\n'
...
E_kinetic = 0.5 * temp * (3 * num_particles_AT) / 120.27239
E_correction = fec.computeCompEnergy()
E_total = E_kinetic + E_bonds + E_angles + E_nonbonded + E_correction
outfile.write(fmt%(0, temp, E_bonds, E_angles, E_nonbonded, E_kinetic, E_correction, E_total))
