...
class TestPIAdResS(ParametrizedTestCase):
    def test_PIAdResS(self):
        ...
        for pid_atom in range(num_atoms):
            ...
            allParticles.append([pid_atom + num_Trotter_beads + 1,
                                 ...
                                 Real3D(0,
                                        ...
                                        0),
                                 0,
                                 types[pid_atom * nTrotter],
                                 masses[pid_atom * nTrotter],
                                 0])
            for pid_trotter in range(nTrotter):
                pid = pid_atom * nTrotter + pid_trotter
        ...
        potOOqm = espressopp.interaction.Tabulated(
            itype=3, filename=tabOW_OW, cutoff=potential_cutoff)
        potHOqm = espressopp.interaction.Tabulated(
            itype=3, filename=tabHW_OW, cutoff=potential_cutoff)
        potHHqm = espressopp.interaction.Tabulated(
            itype=3, filename=tabHW_HW, cutoff=potential_cutoff)
        if nb_forcefield_setup == 1:
            ...
            potOOcl = espressopp.interaction.LennardJones(
                epsilon=temp, sigma=0.25, shift='auto', cutoff=1.122462048309373 * 0.25)
        ...
        potBondHH = espressopp.interaction.Tabulated(
            itype=3, filename=tabBondHH)
        potBondOH = espressopp.interaction.Tabulated(
            itype=3, filename=tabBondOH)
        ...
        if KTI:
            for i in range(1, num_Trotter_beads + num_atoms + 1):
                ...
                system.storage.modifyParticle(
                    i, 'varmass', clmassmultiplier * system.storage.getParticle(i).mass)
            system.storage.decompose()
...
if __name__ == '__main__':
    ...
    suite.addTest(ParametrizedTestCase.parametrize(
        TestPIAdResS, param=parameter_dict_test10))
    ...
    suite.addTest(ParametrizedTestCase.parametrize(
        TestPIAdResS, param=parameter_dict_test11))
