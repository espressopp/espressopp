...
class TestTDforce(unittest.TestCase):
    ...
    def test_slab(self):
        ...
        # run checks (only one particle is in hybrid region, should feel the
        # thermodynamic force, and should hence move along the x direction)
    ...
    def test_fixed_sphere(self):
        ...
        # run checks (as test before, just that particles should move in
        # y-direction given the new setup and the spherical adaptive resolution
        # geometry)
        self.assertEqual(before[0], after[0])
        self.assertAlmostEqual(after[1], 6.596913, places=5)
        self.assertEqual(before[2], after[2])
    ...
    def test_single_moving_sphere(self):
        ...
        tuples = [(1, 6), (2, 7), (3, 8), (4, 9), (5, 10)]
        self.system.storage.addParticles(
            particle_list, 'id', 'type', 'q', 'pos', 'v', 'mass', 'adrat')
        ftpl = espressopp.FixedTupleListAdress(self.system.storage)
        ftpl.addTuples(tuples)
        self.system.storage.setFixedTuplesAdress(ftpl)
        self.system.storage.decompose()
        ...
        # run checks (first particle moves and defines the AdResS region,
        # second particle is in the corresponding hybrid region, others are in
    ...
    def test_many_moving_spheres(self):
        ...
        # set up TD force
        thdforce = espressopp.integrator.TDforce(
            self.system, vl, startdist=0.9, enddist=2.1, edgeweightmultiplier=20)
        thdforce.addForce(itype=3, filename="table_tf.tab", type=1)
        integrator.addExtension(thdforce)
        ...
        # run checks (first two particles move and together define the AdResS
        # region, third particle is in the corresponding hybrid region, others
        # are in coarse-grained region)
        self.assertAlmostEqual(after[0], 5.100000, places=5)
    ...
    def test_many_moving_spheres_fewupdates(self):
        ...
        integrator = espressopp.integrator.VelocityVerlet(self.system)
        integrator.dt = 0.01
        adress = espressopp.integrator.Adress(
            self.system, vl, ftpl, regionupdates=5)
        integrator.addExtension(adress)
        espressopp.tools.AdressDecomp(self.system, integrator)
        ...
        # y coordinates of particles after integration (as test before, but
        # more integration steps and the AdResS region is updated only every 5
        # steps)
        after = [self.system.storage.getParticle(
            i).pos[1] for i in range(1, 6)]
