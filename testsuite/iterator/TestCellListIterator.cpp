...
BOOST_AUTO_TEST_CASE(ListOfEmptyCells)
{
    ...
    CellList cl;
    for (int i = 0; i < NCELL; i++) cells.push_back(cell);

    for (int i = 0; i < NCELL; i++) cl.push_back(&cells[i]);
...
BOOST_AUTO_TEST_CASE(CellListWithHole)
{
    ...
    for (int i = 0; i < NCELL; ++i)
    {
        cells.push_back(cell);
        // create holes
        if (i == 0 || i == 3) cells.push_back(cell);
        for (int j = 0; j < NP; ++j)
        {
            p.id() = i * NP + j;
            cells.back().particles.push_back(p);
    ...
    // create cell list
    CellList cl;
    for (int i = 0; i < cells.size(); ++i) cl.push_back(&cells[i]);
