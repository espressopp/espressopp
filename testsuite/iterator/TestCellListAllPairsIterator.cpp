...
BOOST_AUTO_TEST_CASE(emptyCellsNoNeighbors)
{
    ...
    for (int i = 0; i < NCELL; ++i) cl.push_back(&cell[i]);
...
BOOST_AUTO_TEST_CASE(emptyCellsEmptyNeighbors)
{
    ...
    for (int i = 0; i < NCELL; ++i)
    {
        for (int j = 0; j < NNCELL; ++j)
            cell[i].neighborCells.push_back(NeighborCellInfo(neighbor[i], true));
        cl.push_back(&cell[i]);
    }
...
BOOST_AUTO_TEST_CASE(emptyCellsFullNeighbors)
{
    ...
    {
        for (int j = 0; j < NNP; j++) neighbor[i].particles.push_back(p);
    }
    ...
    for (int pid1 = 0; pid1 < NP * NCELL; ++pid1)
    {
        for (int pid2 = 0; pid2 < NP * NCELL; ++pid2)
        {
            ...
            if (pid1 == pid2)
            {
                ...
                BOOST_CHECK_MESSAGE(occ == 0, "occupancy(" << pid1 << ", " << pid2 << ") == " << occ
                                                           << ", should be 0");
            }
            else if (cid1 == cid2)
            {
                // pairs in the same cell should be touched once
                BOOST_CHECK_MESSAGE(occ == 1, "occupancy(" << pid1 << ", " << pid2
                                                           << ") (same cell) == " << occ
                                                           << ", should be 1");
            }
            else
            {
                ...
                BOOST_CHECK_MESSAGE(occ == 0, "occupancy(" << pid1 << ", " << pid2
                                                           << ") (diff. cells) == " << occ
                                                           << ", should be 0");
            }
        };
...
BOOST_AUTO_TEST_CASE(fullCellsFullNeighbors)
{
    ...
    for (int cid = 0; cid < NCELL; ++cid)
    {
        ...
        for (int j = 1; j <= NN; j++)
        {
            ...
            nbcell = (cid + j) % NCELL;
            cell[cid].neighborCells.push_back(NeighborCellInfo(cell[nbcell], true));

            nbcell = cid - j;
            if (nbcell < 0) nbcell += NCELL;
            cell[cid].neighborCells.push_back(NeighborCellInfo(cell[nbcell], false));
        }
    }
    ...
    // and check the results
    int expected = NCELL * (((NP * (NP - 1)) / 2)  // sum of self pairs
    ...
    for (int pid1 = 0; pid1 < NP * NCELL; ++pid1)
    {
        for (int pid2 = 0; pid2 < NP * NCELL; ++pid2)
        {
            ...
            else if (dcid <= NN)
            {
                ...
                BOOST_CHECK_MESSAGE(occ == 1, "occupancy(" << pid1 << ", " << pid2 << ", cid1=="
                                                           << cid1 << ", cid2==" << cid2
                                                           << ") == " << occ << ", should be 1");
            }
            else
            {
                ...
                BOOST_CHECK_MESSAGE(occ == 0, "occupancy(" << pid1 << ", " << pid2 << ", cid1=="
                                                           << cid1 << ", cid2==" << cid2
                                                           << ") == " << occ << ", should be 0");
            }
        };
