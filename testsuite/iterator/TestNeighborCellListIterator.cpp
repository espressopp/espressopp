...
BOOST_AUTO_TEST_CASE(ListOfEmptyCells)
{
    ...
    Cell cell;
    LocalCellList cl;
    for (int i = 0; i < NCELL; i++) cl.push_back(cell);
    BOOST_TEST_CHECKPOINT("after CellList init");

    NeighborCellList ncl;
    for (int i = 0; i < NCELL; i++) ncl.push_back(NeighborCellInfo(cl[i], (i % 2 == 0)));
...
BOOST_AUTO_TEST_CASE(FullCellList)
{
    ...
    NeighborCellList ncl;
    for (int i = 0; i < NCELL; ++i) ncl.push_back(NeighborCellInfo(cells[i], (i % 2 == 0)));
