...
class TestCaseMeanSquareInternalDist(unittest.TestCase):
    ...
    def test_MSID(self):
        ...
        system1.storage.addParticles([[cid * 10 + k, espressopp.Real3D(k, cid, 0)]
                                     for cid in range(10) for k in range(10)], 'id', 'pos')
        system1.storage.decompose()
        msid1 = espressopp.analysis.MeanSquareInternalDist(
            system1, chainlength=10, start_pid=0)
        msid1.gather()
        res1 = msid1.compute()

        # add 10 straight chains, particle ids are numbered from 1 to 100
        system2.storage.addParticles([[cid * 10 + k + 1, espressopp.Real3D(k, cid, 0)]
                                     for cid in range(10) for k in range(10)], 'id', 'pos')
        system2.storage.decompose()
        msid2 = espressopp.analysis.MeanSquareInternalDist(
            system2, chainlength=10, start_pid=1)
        msid2.gather()
        res2 = msid2.compute()
