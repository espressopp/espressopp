...
def generate_md(use_vec=True, vec_mode=""):
    ...
    if use_vec:
        vec = espressopp.vectorization.Vectorization(
            system, integrator, mode=vec_mode)
    ...
    if use_vec:
        ...
        interLJ = espressopp.vectorization.interaction.VerletListLennardJones(
            vl)
        potLJ = espressopp.vectorization.interaction.LennardJones(
            epsilon=1.0, sigma=1.0, cutoff=rc, shift=0)
    else:
    ...
    print("verletlist          = ", ".".join(
        [vl.__class__.__module__, vl.__class__.__name__]))
    print("interaction         = ", ".".join(
        [interLJ.__class__.__module__, interLJ.__class__.__name__]))
    print('')
    ...
    configurations = espressopp.analysis.Configurations(
        system, pos=True, vel=True, force=True)
