...
class TestThermoIntegration(unittest.TestCase):
    ...
    def test_potential_decoupling(self):
        ...
        interactionLJ = espressopp.interaction.VerletListAdressLennardJonesSoftcoreTI(
            vl, ftpl)
        potLJ = espressopp.interaction.LennardJonesSoftcoreTI(
            epsilonA=1.0,
            sigmaA=0.2,
            epsilonB=0.0,
            sigmaB=0.2,
        ...
        self.assertAlmostEqual(
            interactionQQ.computeEnergyDeriv(), -1.627412, places=5)
        self.assertAlmostEqual(
            interactionLJ.computeEnergyDeriv(),
            0.330739,
            places=5)
        self.assertAlmostEqual(
            interactionQQ.computeEnergy(), -1.213441, places=5)
        self.assertAlmostEqual(
            interactionLJ.computeEnergy(), -0.545769, places=5)
    ...
    def test_potential_annihilation(self):
        ...
        self.assertAlmostEqual(
            interactionQQ.computeEnergyDeriv(),
            0.725217,
            places=5)
        self.assertAlmostEqual(
            interactionLJ.computeEnergyDeriv(),
            ...
            places=5)
        self.assertAlmostEqual(
            interactionQQ.computeEnergy(), -0.507652, places=5)
        self.assertAlmostEqual(
            interactionLJ.computeEnergy(), -0.447031, places=5)
