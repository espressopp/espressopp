...
struct Fixture
{
    Fixture()
    {
        ...
        domdec = std::make_shared<DomainDecomposition>(esppSystem, nodeGrid, cellGrid, halfCellInt);
        esutil::RNG rng;
        ...
        BOOST_TEST_MESSAGE("number of particles in storage =  " << domdec->getNRealParticles());
    }
};
...
struct LatticeFixture : DomainFixture
{
    LatticeFixture() : DomainFixture(1.0)
    {
        ...
        BOOST_TEST_MESSAGE(
            "number of lattice particles in storage = " << domdec->getNRealParticles());
...
BOOST_FIXTURE_TEST_CASE(moveParticles, LatticeFixture)
{
    ...
    std::shared_ptr<MDIntegrator> integrator = std::make_shared<VelocityVerlet>(esppSystem);
