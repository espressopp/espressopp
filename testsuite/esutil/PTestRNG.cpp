...
BOOST_AUTO_TEST_CASE(different_tasks)
{
    ...
    else
    {
        ...
        for (size_t i = 0; i < rs.size(); i++)
            for (size_t j = i + 1; j < rs.size(); j++) BOOST_CHECK_NE(rs[i], rs[j]);
    }
}
...
BOOST_AUTO_TEST_CASE(reproducible)
{
    ...
    for (int i = 0; i < 1000; i++) BOOST_CHECK_EQUAL(rng(), rng2());
...
BOOST_AUTO_TEST_CASE(uniformOnSphere)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        BOOST_CHECK_CLOSE(rs[0] * rs[0] + rs[1] * rs[1] + rs[2] * rs[2], static_cast<real>(1.0),
                          1.0);
    }
}
