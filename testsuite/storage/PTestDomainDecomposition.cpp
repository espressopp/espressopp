...
BOOST_AUTO_TEST_CASE(constructDomainDecomposition)
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        Int3D cellGrid(1);
        nodeGrid[i] = 0;
        BOOST_CHECK_THROW(DomainDecomposition(system, nodeGrid, cellGrid, halfCellInt),
                          NodeGridIllegal);
    }
...
BOOST_FIXTURE_TEST_CASE(fetchParticles, Fixture)
{
    ...
    DomainDecomposition domdec2(system, nodeGrid, cellGrid, halfCellInt);
...
BOOST_FIXTURE_TEST_CASE(checkGhosts, Fixture)
{
    ...
    for (ESPPIterator<CellList> it(domdec->getLocalCells()); it.isValid(); ++it)
    {
        ...
        if (cnt == 1)
        {
            ...
            for (int i = 0; i < 3; ++i)
            {
                ...
                int ip = ipos[i] - cGrid.getFrameWidth() +
                         nGrid.getNodePosition(i) * cGrid.getGridSize(i);
            ...
            if (failed)
            {
                BOOST_TEST_MESSAGE("error at particle: expected " << pos << " type " << type
                                                                  << ", got " << pl[0].position()
                                                                  << " type " << pl[0].type());
            }
        }
        if (failed)
        {
            BOOST_TEST_MESSAGE("error at cell: " << ipos << " on node " << nGrid.getNodePosition(0)
    ...
    for (ESPPIterator<CellList> it(domdec->getRealCells()); it.isValid(); ++it)
    {
        ...
        if (cnt == 1)
        {
            ...
            for (int i = 0; i < 3; ++i)
            {
                real ap = ipos[i] - cGrid.getFrameWidth() +
                          nGrid.getNodePosition(i) * cGrid.getGridSize(i);
                force[i] = ap * ghostCnt;
            }
            ...
            if (dst > 1e-10)
            {
                BOOST_TEST_MESSAGE("error at cell: "
                                   << ipos << " on node " << nGrid.getNodePosition(0) << " "
                                   << nGrid.getNodePosition(1) << " " << nGrid.getNodePosition(2)
                                   << " expect force " << force << " got force " << pl[0].force());
            }
        }
    }
