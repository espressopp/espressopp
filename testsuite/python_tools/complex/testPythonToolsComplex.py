...
class TestPythonTools(unittest.TestCase):
    ...
    def test_boresch_restraints(self):
        ...
        # A,B,C in ligand, a,b,c in protein, original atomistic indices
        restraintAtoms = {'A': 1, 'B': 2, 'C': 3, 'a': 4, 'b': 5, 'c': 6}
        restraintK = {
            'aA': 4184,
            'baA': 41.84,
            'aAB': 41.84,
        ...
        restraint_interactions = espressopp.tools.applyBoreschRestraints(
            self.system, restraintAtoms, restraintK, restraintR0)
        dhdlRstr = 0.0
        for rt in list(restraint_interactions.values()):
            dhdlRstr += rt.computeEnergy()  # energyDeriv = energy for restraints
        self.assertAlmostEqual(dhdlRstr, 141.92348724, places=5)

    def test_self_excl_energy(self):

        exclusions = [(1, 2), (1, 3), (4, 5), (4, 6)]
        energy = espressopp.tools.energy.getSelfExclEnergyReactionField(
            self.system,
            exclusions,
            prefactor=1.0,
            epsilon1=1,
