...
def calc_dihedral(self, quadrupleslist):
    ...
    # returns torsional angle according to IUPAC convention (same convention
    # as used in espressopp, dlpoly, gromacs, vmd, etc.)
...
class TestDihedralHarmonic(unittest.TestCase):
    def setUp(self):
        ...
        cellGrid = espressopp.tools.decomp.cellGrid(
            box, nodeGrid, cutoff, skin)
        system.storage = espressopp.storage.DomainDecomposition(
            system, nodeGrid, cellGrid)
        self.system = system

    def test_phi0(self):
        # create system with three torsions, and for each one set a potential
        # with a different phi0 values, to check that the interaction potential
        # works for all combinations of positive and negative phi and phi0
        ...
        for i in range(3):
            ...
            interaction = espressopp.interaction.FixedQuadrupleListDihedralHarmonic(
                self.system, fql, potential=espressopp.interaction.DihedralHarmonic(
        ...
        interaction = espressopp.interaction.FixedPairListHarmonic(
            self.system, fpl, potential=espressopp.interaction.Harmonic(K=1.0, r0=1.0))
        self.system.addInteraction(interaction)
        ...
        self.assertAlmostEqual(
            interactions[0].computeEnergy(),
            0.747885,
            places=5)
        self.assertAlmostEqual(
            interactions[1].computeEnergy(),
            0.099570,
            places=5)
        self.assertAlmostEqual(
            interactions[2].computeEnergy(),
            0.099570,
            places=5)
