...
handle<> object_without_skeleton_exception;

void translate_obj_wo_skel_exception(const object_without_skeleton & e)
{
  PyErr_SetObject(object_without_skeleton_exception.get(), object(e).ptr());
}
...
void export_skeleton_and_content(class_<communicator>& comm)
{
  ...
  object_without_skeleton_exception = handle<>(PyErr_NewException(
      "boostmpi.ObjectWithoutSkeletonError", 0, 0));
  scope().attr("ObjectWithoutSkeletonError") = object_without_skeleton_exception;

  register_exception_translator<object_without_skeleton>(&translate_obj_wo_skel_exception);

  class_<object_without_skeleton>
    ("ObjectWithoutSkeletonErrorData", object_without_skeleton_docstring, no_init)
